...
import {
  ...
  OCCC_PHILOSOPHY,
  OCCC_VISION,
  parseOrganizationalCompoundingCenter,
  type OrganizationalCompoundingCenter,
} from "@/lib/organizational-compounding-center";
...
const HEALTH_STYLES: Record<string, string> = {
  ...
  interrupted: "border-rose-200 bg-rose-50",
};
...
export function OrganizationalCompoundingCenterPanel({ labels }: Props) {
  const [center, setCenter] = useState<OrganizationalCompoundingCenter | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/organizational-compounding/center");
    if (res.ok) setCenter(parseOrganizationalCompoundingCenter(await res.json()));
    setLoading(false);
  }, []);
  ...
  const postAction = async (payload: Record<string, unknown>) => {
    await fetch("/api/organizational-compounding/action", {
      method: "POST",
  ...
  const healthStyle = HEALTH_STYLES[dash?.compounding_health_label ?? "healthy"] ?? HEALTH_STYLES.healthy;
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      <div className="flex flex-wrap gap-3 text-sm">
        ...
        {center?.links?.organizational_transformation && (
          <Link href={center.links.organizational_transformation} className="text-slate-600 hover:underline">
      ...
      <div>
        ...
        <p className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900">
          {labels.corePrinciple}: {OCCC_CORE_PRINCIPLE}
        </p>
        <p className="mt-2 text-sm text-gray-600">
          {labels.philosophyTitle}: {OCCC_PHILOSOPHY}
        </p>
        <p className="mt-1 text-sm text-gray-600">
          {labels.visionTitle}: {OCCC_VISION}
        </p>
      ...
      {dash && (
        <section className={`rounded-2xl border p-5 shadow-sm ${healthStyle}`}>
          ...
          <p className="mt-1 text-sm text-gray-600">
            {labels.healthLabels[dash.compounding_health_label] ?? dash.compounding_health_label} ·{" "}
            {labels.compoundingScore}: {dash.compounding_score}/100 · {labels.positiveMomentum}:{" "}
            {dash.positive_momentum_pct}%
          </p>
          <dl className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Metric label={labels.metrics.consistency} value={`${dash.consistency_execution_pct}%`} />
            ...
            <Metric label={labels.metrics.relationships} value={`${dash.relationship_stability_pct}%`} />
            <Metric label={labels.metrics.sustainability} value={`${dash.improvement_sustainability_pct}%`} />
            <Metric label={labels.metrics.improvement} value={`${dash.long_term_improvement_pct}%`} />
          ...
          {center?.can_manage && (
            <div className="mt-4 flex flex-wrap gap-2">
              <ActionBtn label={labels.generateReport} onClick={() => void postAction({ action: "generate_compounding_report" })} />
              ...
              <ActionBtn
                ...
                variant="muted"
                onClick={() => void postAction({ action: "export_progress_snapshot", period_label: "Current quarter" })}
              />
      ...
      <Section title={labels.signalsTitle} empty={center?.compounding_signals.length === 0} emptyLabel={labels.emptySection}>
        {center?.compounding_signals.map((sig) => (
      ...
      <Section title={labels.leverageTitle} empty={center?.leverage_prompts.length === 0} emptyLabel={labels.emptySection}>
        {center?.leverage_prompts.map((lev) => (
          <li key={lev.leverage_key} className="rounded-xl border border-indigo-100 bg-indigo-50/20 p-4 text-sm">
            <p className="text-xs text-gray-500">{labels.leverageTypes[lev.leverage_type] ?? lev.leverage_type}</p>
            <p className="mt-1 font-medium text-gray-900">{lev.title}</p>
            <p className="mt-2 text-gray-700">{lev.summary}</p>
          </li>
        ))}
      </Section>

      <Section title={labels.initiativesTitle} empty={center?.compounding_initiatives.length === 0} emptyLabel={labels.emptySection}>
        {center?.compounding_initiatives.map((ini) => (
      ...
      <Section title={labels.reviewsTitle} empty={center?.compounding_reviews.length === 0} emptyLabel={labels.emptySection}>
        {center?.compounding_reviews.map((rev) => (
      ...
      <Section title={labels.milestonesTitle} empty={center?.compounding_milestones.length === 0} emptyLabel={labels.emptySection}>
        {center?.compounding_milestones.map((ms) => (
        ...
        {center?.can_manage && (
          <li className="pt-2">
            <ActionBtn
              ...
              onClick={() =>
                void postAction({
                  action: "archive_compounding_milestone",
                  title: "Compounding milestone",
                  summary: "Compounding milestone archived via Compounding Center.",
                })
              }
            />
          </li>
          ...
          <li key={snap.snapshot_key} className="rounded-xl border border-gray-100 p-3 text-sm">
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              <span className="text-xs text-indigo-700">
                {labels.compoundingScore}: {snap.compounding_score}
              </span>
            </div>
      ...
      {center?.executive_view && (
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            <ExecField label={labels.executiveFields.value} value={center.executive_view.long_term_value} />
            <ExecField label={labels.executiveFields.consistency} value={center.executive_view.leadership_consistency} />
            <ExecField label={labels.executiveFields.growth} value={center.executive_view.growth_trends} />
            <ExecField label={labels.executiveFields.patience} value={center.executive_view.patience_opportunities} />
          </dl>
        </section>
      )}
      ...
      <Section title={labels.sessionsTitle} empty={center?.compounding_sessions.length === 0} emptyLabel={labels.emptySection}>
        {center?.compounding_sessions.map((sess) => (

...
import {
  parseHumanWonderExplorationEngineDashboard,
  type AbosSuccessCriterion,
  type BlueprintObjective,
  type ExecutiveReview,
  type HumanWonderExplorationEngineDashboard,
  type IntegrationLink,
  type ReflectionEntry,
  type ScaffoldNote,
} from "@/lib/aipify/human-wonder-exploration-engine";
...
export function HumanWonderExplorationEngineDashboardPanel({ labels }: Props) {
  const [dashboard, setDashboard] = useState<HumanWonderExplorationEngineDashboard | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/aipify/human-wonder-exploration-engine/dashboard");
    if (res.ok) setDashboard(parseHumanWonderExplorationEngineDashboard(await res.json()));
    setLoading(false);
  }, []);
  ...
  const integrationLinks = dashboard.hwebp179_integration_links?.length
    ? dashboard.hwebp179_integration_links
    : dashboard.integration_links;
  const eraOpener = dashboard.hwebp179_era_opener_summary?.length
    ? dashboard.hwebp179_era_opener_summary
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-xl border border-indigo-200 bg-indigo-50/50 p-6">
        ...
        <p className="mt-2 text-2xl font-bold text-indigo-900">{dashboard.human_wonder_exploration_score ?? 0}</p>
        <p className="text-xs text-indigo-700">{labels.scoreLabel}</p>
        {dashboard.human_wonder_exploration_mission ? <p className="mt-2 text-sm font-medium text-indigo-900">{dashboard.human_wonder_exploration_mission}</p> : null}
        ...
        {dashboard.human_wonder_exploration_vision ? <p className="mt-2 text-xs italic text-indigo-800">{dashboard.human_wonder_exploration_vision}</p> : null}
        <div className="mt-4 flex flex-wrap gap-4 text-sm">
          <div><span className="font-medium">{labels.modeLabel}:</span> {dashboard.wonder_mode}</div>
          <div><span className="font-medium">{labels.readinessLabel}:</span> {dashboard.wonder_readiness_level ?? 1}</div>
      ...
      {dashboard.human_wonder_exploration_objectives?.length ? (
        <section className="space-y-3">
          <h2 className="text-lg font-semibold text-gray-900">{labels.blueprintObjectives}</h2>
          <div className="grid gap-2 sm:grid-cols-2">
            {dashboard.human_wonder_exploration_objectives.map((o) => <ObjectiveCard key={o.key ?? o.label} objective={o} />)}
          </div>
        </section>
      ) : null}
      ...
      {dashboard.human_wonder_exploration_success_criteria?.length ? (
        <section className="space-y-2">
          <h2 className="text-lg font-semibold text-gray-900">{labels.successCriteria}</h2>
          {dashboard.human_wonder_exploration_success_criteria.map((c) => (
            <SuccessCriterionRow key={c.key ?? c.label} criterion={c} metLabel={labels.criterionMet} pendingLabel={labels.criterionPending} />
          ))}
        </section>
      ) : null}

      {dashboard.human_wonder_exploration_privacy_note ? <p className="text-xs text-gray-500">{dashboard.human_wonder_exploration_privacy_note}</p> : null}
    </div>
  );
}

...
import {
  ...
  OSLC_PHILOSOPHY,
  OSLC_VISION,
  parseOrganizationalSustainabilityCenter,
  type OrganizationalSustainabilityCenter,
} from "@/lib/organizational-sustainability-center";
...
type PanelLabels = {
  ...
  concernTypes: Record<string, string>;
  concernTones: Record<string, string>;
  growthTypes: Record<string, string>;
...
const HEALTH_STYLES: Record<string, string> = {
  ...
  attention_recommended: "border-amber-200 bg-amber-50",
  unsustainable_risk: "border-rose-200 bg-rose-50",
};
...
export function OrganizationalSustainabilityCenterPanel({ labels }: Props) {
  const [center, setCenter] = useState<OrganizationalSustainabilityCenter | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/organizational-sustainability/center");
    if (res.ok) setCenter(parseOrganizationalSustainabilityCenter(await res.json()));
    setLoading(false);
  }, []);
  ...
  const postAction = async (payload: Record<string, unknown>) => {
    await fetch("/api/organizational-sustainability/action", {
      method: "POST",
  ...
  const healthStyle = HEALTH_STYLES[dash?.sustainability_health_label ?? "healthy"] ?? HEALTH_STYLES.healthy;
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      <div className="flex flex-wrap gap-3 text-sm">
        ...
        {center?.links?.organizational_renewal && <Link href={center.links.organizational_renewal} className="text-slate-600 hover:underline">{labels.organizationalRenewalLink}</Link>}
      ...
      <div>
        ...
        <p className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900">{labels.corePrinciple}: {OSLC_CORE_PRINCIPLE}</p>
        <p className="mt-2 text-sm text-gray-600">{labels.philosophyTitle}: {OSLC_PHILOSOPHY}</p>
        <p className="mt-1 text-sm text-gray-600">{labels.visionTitle}: {OSLC_VISION}</p>
      ...
      {dash && (
        <section className={`rounded-2xl border p-5 shadow-sm ${healthStyle}`}>
          ...
          <p className="mt-1 text-sm text-gray-600">
            {labels.healthLabels[dash.sustainability_health_label] ?? dash.sustainability_health_label} · {labels.sustainabilityScore}: {dash.sustainability_score}/100 · {labels.sustainabilityTrend}: {dash.sustainability_trend_pct}%
          </p>
          <dl className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Metric label={labels.metrics.workforce} value={`${dash.workforce_resilience_pct}%`} />
            <Metric label={labels.metrics.operational} value={`${dash.operational_maintainability_pct}%`} />
            <Metric label={labels.metrics.strategic} value={`${dash.strategic_consistency_pct}%`} />
            <Metric label={labels.metrics.leadership} value={`${dash.leadership_preparedness_pct}%`} />
            <Metric label={labels.metrics.financial} value={`${dash.financial_stability_pct}%`} />
            <Metric label={labels.metrics.resilience} value={`${dash.resilience_measures_pct}%`} />
          ...
          {center?.can_manage && (
            <div className="mt-4 flex flex-wrap gap-2">
              <ActionBtn label={labels.generateReport} onClick={() => void postAction({ action: "generate_sustainability_report" })} />
              <ActionBtn label={labels.printSummary} variant="muted" onClick={() => void postAction({ action: "print_executive_summary" })} />
              <ActionBtn label={labels.exportSnapshot} variant="muted" onClick={() => void postAction({ action: "export_sustainability_snapshot", period_label: "Current quarter" })} />
      ...
      <Section title={labels.concernsTitle} empty={center?.sustainability_concerns.length === 0} emptyLabel={labels.emptySection}>
        {center?.sustainability_concerns.map((con) => (
          <li key={con.concern_key} className={`rounded-xl border p-4 text-sm ${TONE_STYLES[con.concern_tone] ?? TONE_STYLES.neutral}`}>
            <p className="text-xs text-gray-500">{labels.domains[con.domain] ?? con.domain} · {labels.concernTypes[con.concern_type] ?? con.concern_type}</p>
            <p className="mt-1 font-medium text-gray-900">{con.title}</p>
            <p className="mt-2 text-gray-700">{con.summary}</p>
          </li>
        ))}
      </Section>

      <Section title={labels.growthTitle} empty={center?.growth_prompts.length === 0} emptyLabel={labels.emptySection}>
        {center?.growth_prompts.map((grw) => (
          <li key={grw.growth_key} className="rounded-xl border border-indigo-100 bg-indigo-50/20 p-4 text-sm">
            <p className="text-xs text-gray-500">{labels.growthTypes[grw.growth_type] ?? grw.growth_type}</p>
            <p className="mt-1 font-medium text-gray-900">{grw.title}</p>
            <p className="mt-2 text-gray-700">{grw.summary}</p>
          </li>
        ))}
      </Section>

      <Section title={labels.initiativesTitle} empty={center?.sustainability_initiatives.length === 0} emptyLabel={labels.emptySection}>
        {center?.sustainability_initiatives.map((ini) => (
      ...
      <Section title={labels.reviewsTitle} empty={center?.sustainability_reviews.length === 0} emptyLabel={labels.emptySection}>
        {center?.sustainability_reviews.map((rev) => (
      ...
      <Section title={labels.milestonesTitle} empty={center?.sustainability_milestones.length === 0} emptyLabel={labels.emptySection}>
        {center?.sustainability_milestones.map((ms) => (
        ...
        {center?.can_manage && (
          <li className="pt-2">
            <ActionBtn label={labels.archiveMilestone} onClick={() => void postAction({ action: "archive_sustainability_milestone", title: "Sustainability milestone", summary: "Sustainability milestone archived via Sustainability Center." })} />
          </li>
        )}
      </Section>
        ...
        {center?.snapshots.map((snap) => (
          <li key={snap.snapshot_key} className="rounded-xl border border-gray-100 p-3 text-sm">
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              <span className="text-xs text-indigo-700">{labels.sustainabilityScore}: {snap.sustainability_score}</span>
            </div>
      ...
      {center?.executive_view && (
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            <ExecField label={labels.executiveFields.viability} value={center.executive_view.long_term_viability} />
            <ExecField label={labels.executiveFields.leadership} value={center.executive_view.leadership_preparedness} />
            <ExecField label={labels.executiveFields.resilience} value={center.executive_view.resilience_trends} />
            <ExecField label={labels.executiveFields.opportunities} value={center.executive_view.sustainability_opportunities} />
          </dl>
        </section>
      )}
      ...
      <Section title={labels.sessionsTitle} empty={center?.sustainability_sessions.length === 0} emptyLabel={labels.emptySection}>
        {center?.sustainability_sessions.map((sess) => (

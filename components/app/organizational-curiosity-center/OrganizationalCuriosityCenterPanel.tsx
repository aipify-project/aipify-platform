...
import {
  ...
  OCC_PHILOSOPHY,
  OCC_VISION,
  parseOrganizationalCuriosityCenter,
  type OrganizationalCuriosityCenter,
} from "@/lib/organizational-curiosity-center";
...
type PanelLabels = {
  ...
  executiveFields: {
    ...
    explorationTrends: string;
    innovationOpportunities: string;
    organizationalInquiry: string;
  };
...
const HEALTH_STYLES: Record<string, string> = {
  ...
  curiosity_improvement_recommended: "border-rose-200 bg-rose-50",
};
...
export function OrganizationalCuriosityCenterPanel({ labels }: Props) {
  const [center, setCenter] = useState<OrganizationalCuriosityCenter | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/organizational-curiosity/center");
    if (res.ok) setCenter(parseOrganizationalCuriosityCenter(await res.json()));
    setLoading(false);
  }, []);
  ...
  const postAction = async (payload: Record<string, unknown>) => {
    await fetch("/api/organizational-curiosity/action", {
      method: "POST",
  ...
  const healthStyle = HEALTH_STYLES[dash?.curiosity_health_label ?? "healthy"] ?? HEALTH_STYLES.healthy;
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      <div className="flex flex-wrap gap-3 text-sm">
        ...
        {center?.links?.organizational_harmony && (
          <Link href={center.links.organizational_harmony} className="text-slate-600 hover:underline">
      ...
      <div>
        ...
        <p className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900">
          {labels.corePrinciple}: {OCC_CORE_PRINCIPLE}
        </p>
        <p className="mt-2 text-sm text-gray-600">
          {labels.philosophyTitle}: {OCC_PHILOSOPHY}
        </p>
        <p className="mt-1 text-sm text-gray-600">
          {labels.visionTitle}: {OCC_VISION}
        </p>
      ...
      {dash && (
        <section className={`rounded-2xl border p-5 shadow-sm ${healthStyle}`}>
          ...
          <p className="mt-1 text-sm text-gray-600">
            {labels.healthLabels[dash.curiosity_health_label] ?? dash.curiosity_health_label} ·{" "}
            {labels.curiosityScore}: {dash.curiosity_score}/100 · {labels.learningEngagement}:{" "}
            {dash.learning_engagement_pct}%
          </p>
          <dl className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Metric label={labels.metrics.learningParticipation} value={`${dash.learning_participation_pct}%`} />
            <Metric label={labels.metrics.reflectionEngagement} value={`${dash.reflection_engagement_pct}%`} />
            <Metric label={labels.metrics.crossFunctionalExploration} value={`${dash.cross_functional_exploration_pct}%`} />
            <Metric label={labels.metrics.knowledgeSharing} value={`${dash.knowledge_sharing_pct}%`} />
            <Metric label={labels.metrics.innovationDiscipline} value={`${dash.innovation_discipline_pct}%`} />
            <Metric label={labels.metrics.explorationInitiatives} value={`${dash.exploration_initiatives_pct}%`} />
          ...
          {center?.can_manage && (
            <div className="mt-4 flex flex-wrap gap-2">
              <ActionBtn label={labels.generateReport} onClick={() => void postAction({ action: "generate_curiosity_report" })} />
              ...
              <ActionBtn
                ...
                variant="muted"
                onClick={() => void postAction({ action: "export_curiosity_snapshot", period_label: "Current quarter" })}
              />
              <ActionBtn label={labels.coordinateReview} variant="muted" onClick={() => void postAction({ action: "coordinate_exploration_workshop" })} />
            </div>
          )}
        </section>
      )}

      <Section title={labels.signalsTitle} empty={center?.curiosity_signals.length === 0} emptyLabel={labels.emptySection}>
        {center?.curiosity_signals.map((sig) => (
      ...
      <Section title={labels.questionTitle} empty={center?.question_prompts.length === 0} emptyLabel={labels.emptySection}>
        {center?.question_prompts.map((que) => (
          <li key={que.question_key} className="rounded-xl border border-indigo-100 bg-indigo-50/20 p-4 text-sm">
            <p className="text-xs text-gray-500">{labels.questionTypes[que.question_type] ?? que.question_type}</p>
      ...
      <Section title={labels.discoveryTitle} empty={center?.discovery_highlights.length === 0} emptyLabel={labels.emptySection}>
        {center?.discovery_highlights.map((dis) => (
          <li key={dis.discovery_key} className="rounded-xl border border-violet-100 bg-violet-50/20 p-4 text-sm">
            <p className="text-xs text-gray-500">{labels.discoveryTypes[dis.discovery_type] ?? dis.discovery_type}</p>
            <p className="mt-1 font-medium text-gray-900">{dis.title}</p>
            <p className="mt-2 text-gray-700">{dis.summary}</p>
          </li>
        ))}
      </Section>

      <Section title={labels.initiativesTitle} empty={center?.curiosity_initiatives.length === 0} emptyLabel={labels.emptySection}>
        {center?.curiosity_initiatives.map((ini) => (
      ...
      <Section title={labels.reviewsTitle} empty={center?.curiosity_reviews.length === 0} emptyLabel={labels.emptySection}>
        {center?.curiosity_reviews.map((rev) => (
      ...
      <Section title={labels.milestonesTitle} empty={center?.curiosity_milestones.length === 0} emptyLabel={labels.emptySection}>
        {center?.curiosity_milestones.map((ms) => (
        ...
        {center?.can_manage && (
          <li className="pt-2">
            <ActionBtn
              ...
              onClick={() =>
                void postAction({
                  action: "archive_curiosity_milestone",
                  title: "Curiosity milestone",
                  summary: "Curiosity milestone archived via Curiosity Center.",
                })
              }
            />
          </li>
          ...
          <li key={snap.snapshot_key} className="rounded-xl border border-gray-100 p-3 text-sm">
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              <span className="text-xs text-indigo-700">
                {labels.curiosityScore}: {snap.curiosity_score}
              </span>
            </div>
      ...
      {center?.executive_view && (
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            ...
            <ExecField label={labels.executiveFields.explorationTrends} value={center.executive_view.exploration_trends} />
            <ExecField label={labels.executiveFields.innovationOpportunities} value={center.executive_view.innovation_opportunities} />
            <ExecField label={labels.executiveFields.organizationalInquiry} value={center.executive_view.organizational_inquiry} />
          </dl>
        </section>
      )}
      ...
      <Section title={labels.sessionsTitle} empty={center?.curiosity_sessions.length === 0} emptyLabel={labels.emptySection}>
        {center?.curiosity_sessions.map((sess) => (

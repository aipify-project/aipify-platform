...
import {
  ...
  OFLC_PHILOSOPHY,
  OFLC_VISION,
  parseOrganizationalFlourishingCenter,
  type OrganizationalFlourishingCenter,
} from "@/lib/organizational-flourishing-center";
...
type PanelLabels = {
  ...
  humansDecide: string;
  privacyNote: string;
  flourishingScore: string;
  positiveMomentum: string;
  domains: Record<string, string>;
  conditionTypes: Record<string, string>;
  conditionTones: Record<string, string>;
  meaningTypes: Record<string, string>;
...
const HEALTH_STYLES: Record<string, string> = {
  ...
  support_recommended: "border-rose-200 bg-rose-50",
};
...
export function OrganizationalFlourishingCenterPanel({ labels }: Props) {
  const [center, setCenter] = useState<OrganizationalFlourishingCenter | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/organizational-flourishing/center");
    if (res.ok) setCenter(parseOrganizationalFlourishingCenter(await res.json()));
    setLoading(false);
  }, []);
  ...
  const postAction = async (payload: Record<string, unknown>) => {
    await fetch("/api/organizational-flourishing/action", {
      method: "POST",
  ...
  const healthStyle = HEALTH_STYLES[dash?.flourishing_health_label ?? "healthy"] ?? HEALTH_STYLES.healthy;
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      <div className="flex flex-wrap gap-3 text-sm">
        ...
        {center?.links?.organizational_confidence && <Link href={center.links.organizational_confidence} className="text-slate-600 hover:underline">{labels.organizationalConfidenceLink}</Link>}
      ...
      <div>
        ...
        <p className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900">{labels.corePrinciple}: {OFLC_CORE_PRINCIPLE}</p>
        <p className="mt-2 text-sm text-gray-600">{labels.philosophyTitle}: {OFLC_PHILOSOPHY}</p>
        <p className="mt-1 text-sm text-gray-600">{labels.visionTitle}: {OFLC_VISION}</p>
      ...
      {dash && (
        <section className={`rounded-2xl border p-5 shadow-sm ${healthStyle}`}>
          ...
          <p className="mt-1 text-sm text-gray-600">
            {labels.healthLabels[dash.flourishing_health_label] ?? dash.flourishing_health_label} · {labels.flourishingScore}: {dash.flourishing_score}/100 · {labels.positiveMomentum}: {dash.positive_momentum_pct}%
          </p>
          <dl className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Metric label={labels.metrics.learning} value={`${dash.learning_participation_pct}%`} />
            <Metric label={labels.metrics.collaboration} value={`${dash.collaboration_effectiveness_pct}%`} />
            <Metric label={labels.metrics.leadership} value={`${dash.leadership_consistency_pct}%`} />
            <Metric label={labels.metrics.resilience} value={`${dash.organizational_resilience_pct}%`} />
            <Metric label={labels.metrics.purpose} value={`${dash.purpose_alignment_pct}%`} />
            <Metric label={labels.metrics.sustainability} value={`${dash.sustainability_pct}%`} />
          ...
          {center?.can_manage && (
            <div className="mt-4 flex flex-wrap gap-2">
              <ActionBtn label={labels.generateReport} onClick={() => void postAction({ action: "generate_flourishing_report" })} />
              <ActionBtn label={labels.printSummary} variant="muted" onClick={() => void postAction({ action: "print_executive_summary" })} />
              <ActionBtn label={labels.exportSnapshot} variant="muted" onClick={() => void postAction({ action: "export_flourishing_snapshot", period_label: "Current quarter" })} />
      ...
      <Section title={labels.conditionsTitle} empty={center?.flourishing_conditions.length === 0} emptyLabel={labels.emptySection}>
        {center?.flourishing_conditions.map((con) => (
          <li key={con.condition_key} className={`rounded-xl border p-4 text-sm ${TONE_STYLES[con.condition_tone] ?? TONE_STYLES.neutral}`}>
            <p className="text-xs text-gray-500">{labels.domains[con.domain] ?? con.domain} · {labels.conditionTypes[con.condition_type] ?? con.condition_type}</p>
      ...
      <Section title={labels.meaningTitle} empty={center?.meaning_prompts.length === 0} emptyLabel={labels.emptySection}>
        {center?.meaning_prompts.map((mean) => (
          <li key={mean.meaning_key} className="rounded-xl border border-indigo-100 bg-indigo-50/20 p-4 text-sm">
            <p className="text-xs text-gray-500">{labels.meaningTypes[mean.meaning_type] ?? mean.meaning_type}</p>
            <p className="mt-1 font-medium text-gray-900">{mean.title}</p>
            <p className="mt-2 text-gray-700">{mean.summary}</p>
          </li>
        ))}
      </Section>

      <Section title={labels.initiativesTitle} empty={center?.flourishing_initiatives.length === 0} emptyLabel={labels.emptySection}>
        {center?.flourishing_initiatives.map((ini) => (
      ...
      <Section title={labels.reviewsTitle} empty={center?.flourishing_reviews.length === 0} emptyLabel={labels.emptySection}>
        {center?.flourishing_reviews.map((rev) => (
      ...
      <Section title={labels.milestonesTitle} empty={center?.flourishing_milestones.length === 0} emptyLabel={labels.emptySection}>
        {center?.flourishing_milestones.map((ms) => (
        ...
        {center?.can_manage && (
          <li className="pt-2">
            <ActionBtn label={labels.archiveMilestone} onClick={() => void postAction({ action: "archive_flourishing_milestone", title: "Flourishing milestone", summary: "Flourishing milestone archived via Flourishing Center." })} />
          </li>
        )}
      </Section>
        ...
        {center?.snapshots.map((snap) => (
          <li key={snap.snapshot_key} className="rounded-xl border border-gray-100 p-3 text-sm">
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              <span className="text-xs text-indigo-700">{labels.flourishingScore}: {snap.flourishing_score}</span>
            </div>
      ...
      {center?.executive_view && (
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            <ExecField label={labels.executiveFields.sustainability} value={center.executive_view.sustainability_indicators} />
            <ExecField label={labels.executiveFields.resilience} value={center.executive_view.resilience_measures} />
            <ExecField label={labels.executiveFields.leadership} value={center.executive_view.leadership_participation} />
            <ExecField label={labels.executiveFields.opportunities} value={center.executive_view.flourishing_opportunities} />
          </dl>
        </section>
      )}
      ...
      <Section title={labels.sessionsTitle} empty={center?.flourishing_sessions.length === 0} emptyLabel={labels.emptySection}>
        {center?.flourishing_sessions.map((sess) => (

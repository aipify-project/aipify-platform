...
import {
  ...
  OCEH_PHILOSOPHY,
  OCEH_VISION,
  parseOrganizationalHopeCenter,
  type OrganizationalHopeCenter,
} from "@/lib/organizational-hope-center";
...
type PanelLabels = {
  ...
  executiveFields: {
    ...
    organizationalResilience: string;
...
const HEALTH_STYLES: Record<string, string> = {
  ...
  hope_reinforcement_recommended: "border-rose-200 bg-rose-50",
};
...
export function OrganizationalHopeCenterPanel({ labels }: Props) {
  const [center, setCenter] = useState<OrganizationalHopeCenter | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/organizational-hope/center");
    if (res.ok) setCenter(parseOrganizationalHopeCenter(await res.json()));
    setLoading(false);
  }, []);
  ...
  const postAction = async (payload: Record<string, unknown>) => {
    await fetch("/api/organizational-hope/action", {
      method: "POST",
  ...
  const healthStyle = HEALTH_STYLES[dash?.hope_health_label ?? "healthy"] ?? HEALTH_STYLES.healthy;
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      <div className="flex flex-wrap gap-3 text-sm">
        ...
        {center?.links?.organizational_courage && (
          <Link href={center.links.organizational_courage} className="text-slate-600 hover:underline">
      ...
      <div>
        ...
        <p className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900">
          {labels.corePrinciple}: {OCEH_CORE_PRINCIPLE}
        </p>
        <p className="mt-2 text-sm text-gray-600">
          {labels.philosophyTitle}: {OCEH_PHILOSOPHY}
        </p>
        <p className="mt-1 text-sm text-gray-600">
          {labels.visionTitle}: {OCEH_VISION}
        </p>
      ...
      {dash && (
        <section className={`rounded-2xl border p-5 shadow-sm ${healthStyle}`}>
          ...
          <p className="mt-1 text-sm text-gray-600">
            {labels.healthLabels[dash.hope_health_label] ?? dash.hope_health_label} ·{" "}
            {labels.hopeScore}: {dash.hope_score}/100 · {labels.progressIndicators}:{" "}
            {dash.progress_indicators_pct}%
          </p>
          <dl className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Metric label={labels.metrics.progressRecognitionMetric} value={`${dash.progress_recognition_pct}%`} />
            <Metric label={labels.metrics.leadershipCommunication} value={`${dash.leadership_communication_pct}%`} />
            <Metric label={labels.metrics.progressRecognitionMetric} value={`${dash.purpose_alignment_pct}%`} />
            <Metric label={labels.metrics.learningParticipation} value={`${dash.learning_participation_pct}%`} />
            <Metric label={labels.metrics.organizationalResilienceMetric} value={`${dash.organizational_resilience_pct}%`} />
            <Metric label={labels.metrics.futureConfidence} value={`${dash.future_confidence_pct}%`} />
          ...
          {center?.can_manage && (
            <div className="mt-4 flex flex-wrap gap-2">
              <ActionBtn label={labels.generateReport} onClick={() => void postAction({ action: "generate_hope_report" })} />
              ...
              <ActionBtn
                ...
                variant="muted"
                onClick={() => void postAction({ action: "export_hope_snapshot", period_label: "Current quarter" })}
              />
      ...
      <Section title={labels.signalsTitle} empty={center?.hope_signals.length === 0} emptyLabel={labels.emptySection}>
        {center?.hope_signals.map((sig) => (
      ...
      <Section title={labels.progressTitle} empty={center?.progress_prompts.length === 0} emptyLabel={labels.emptySection}>
        {center?.progress_prompts.map((que) => (
          <li key={que.progress_key} className="rounded-xl border border-indigo-100 bg-indigo-50/20 p-4 text-sm">
            <p className="text-xs text-gray-500">{labels.progressTypes[que.progress_type] ?? que.progress_type}</p>
      ...
      <Section title={labels.initiativesTitle} empty={center?.hope_initiatives.length === 0} emptyLabel={labels.emptySection}>
        {center?.hope_initiatives.map((ini) => (
      ...
      <Section title={labels.reviewsTitle} empty={center?.hope_reviews.length === 0} emptyLabel={labels.emptySection}>
        {center?.hope_reviews.map((rev) => (
      ...
      <Section title={labels.milestonesTitle} empty={center?.hope_milestones.length === 0} emptyLabel={labels.emptySection}>
        {center?.hope_milestones.map((ms) => (
        ...
        {center?.can_manage && (
          <li className="pt-2">
            <ActionBtn
              ...
              onClick={() =>
                void postAction({
                  action: "archive_hope_milestone",
                  title: "Hope milestone",
                  summary: "Hope milestone archived via Hope Center.",
                })
              }
            />
          </li>
          ...
          <li key={snap.snapshot_key} className="rounded-xl border border-gray-100 p-3 text-sm">
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              <span className="text-xs text-indigo-700">
                {labels.hopeScore}: {snap.hope_score}
              </span>
            </div>
      ...
      {center?.executive_view && (
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            <ExecField label={labels.executiveFields.leadershipConfidence} value={center.executive_view.leadership_confidence} />
            <ExecField label={labels.executiveFields.organizationalResilience} value={center.executive_view.organizational_resilience} />
            <ExecField label={labels.executiveFields.progressRecognition} value={center.executive_view.progress_recognition} />
            <ExecField label={labels.executiveFields.futureOpportunity} value={center.executive_view.future_opportunity} />
          </dl>
        </section>
      )}
      ...
      <Section title={labels.sessionsTitle} empty={center?.hope_sessions.length === 0} emptyLabel={labels.emptySection}>
        {center?.hope_sessions.map((sess) => (

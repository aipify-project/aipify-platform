...
import {
  ...
  OEXC_PHILOSOPHY,
  OEXC_VISION,
  parseOrganizationalExcellenceCenter,
  type OrganizationalExcellenceCenter,
} from "@/lib/organizational-excellence-center";
...
const HEALTH_STYLES: Record<string, string> = {
  ...
  improvement_recommended: "border-rose-200 bg-rose-50",
};
...
export function OrganizationalExcellenceCenterPanel({ labels }: Props) {
  const [center, setCenter] = useState<OrganizationalExcellenceCenter | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/organizational-excellence/center");
    if (res.ok) setCenter(parseOrganizationalExcellenceCenter(await res.json()));
    setLoading(false);
  }, []);
  ...
  const postAction = async (payload: Record<string, unknown>) => {
    await fetch("/api/organizational-excellence/action", {
      method: "POST",
  ...
  const healthStyle = HEALTH_STYLES[dash?.excellence_health_label ?? "healthy"] ?? HEALTH_STYLES.healthy;
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      <div className="flex flex-wrap gap-3 text-sm">
        ...
        {center?.links?.organizational_continuity && <Link href={center.links.organizational_continuity} className="text-slate-600 hover:underline">{labels.organizationalContinuityLink}</Link>}
      ...
      <div>
        ...
        <p className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900">{labels.corePrinciple}: {OEXC_CORE_PRINCIPLE}</p>
        <p className="mt-2 text-sm text-gray-600">{labels.philosophyTitle}: {OEXC_PHILOSOPHY}</p>
        <p className="mt-1 text-sm text-gray-600">{labels.visionTitle}: {OEXC_VISION}</p>
      ...
      {dash && (
        <section className={`rounded-2xl border p-5 shadow-sm ${healthStyle}`}>
          ...
          <p className="mt-1 text-sm text-gray-600">
            {labels.healthLabels[dash.excellence_health_label] ?? dash.excellence_health_label} · {labels.excellenceScore}: {dash.excellence_score}/100 · {labels.improvementMomentum}: {dash.improvement_momentum_pct}%
          </p>
          <dl className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Metric label={labels.metrics.continuousImprovement} value={`${dash.continuous_improvement_pct}%`} />
            <Metric label={labels.metrics.execution} value={`${dash.execution_consistency_pct}%`} />
            <Metric label={labels.metrics.leadership} value={`${dash.leadership_maturity_pct}%`} />
            <Metric label={labels.metrics.customer} value={`${dash.customer_satisfaction_trend_pct}%`} />
            ...
            <Metric label={labels.metrics.strengths} value={dash.capability_strengths} />
            <Metric label={labels.metrics.confidence} value={`${dash.executive_confidence}/5`} />
          </dl>
          {center?.can_manage && (
            <div className="mt-4 flex flex-wrap gap-2">
              <ActionBtn label={labels.generateReport} onClick={() => void postAction({ action: "generate_excellence_report" })} />
              <ActionBtn label={labels.printSummary} variant="muted" onClick={() => void postAction({ action: "print_executive_summary" })} />
              <ActionBtn label={labels.exportSnapshot} variant="muted" onClick={() => void postAction({ action: "export_excellence_snapshot", period_label: "Current period" })} />
              <ActionBtn label={labels.coordinateReview} variant="muted" onClick={() => void postAction({ action: "coordinate_cross_functional_review" })} />
            </div>
          )}
        </section>
      )}

      <Section title={labels.signalsTitle} empty={center?.excellence_signals.length === 0} emptyLabel={labels.emptySection}>
        {center?.excellence_signals.map((sig) => (
      ...
      <Section title={labels.bestPracticesTitle} empty={center?.best_practices.length === 0} emptyLabel={labels.emptySection}>
        {center?.best_practices.map((bp) => (
          <li key={bp.practice_key} className="rounded-xl border border-gray-100 p-4 text-sm">
            <div className="flex flex-wrap items-start justify-between gap-2">
              <p className="text-xs text-gray-500">{labels.domains[bp.domain] ?? bp.domain} · {labels.practiceTypes[bp.practice_type] ?? bp.practice_type}</p>
              <span className="text-xs font-medium text-indigo-700">{labels.practiceStatuses[bp.status] ?? bp.status}</span>
            </div>
            <p className="mt-1 font-medium text-gray-900">{bp.title}</p>
            <p className="mt-2 text-gray-700">{bp.summary}</p>
            {bp.status === "highlighted" && center?.can_manage && (
              <div className="mt-3">
                <ActionBtn label={labels.scalePractice} onClick={() => void postAction({ action: "scale_best_practice", practice_key: bp.practice_key })} />
              </div>
            )}
          </li>
        ))}
      ...
      <Section title={labels.initiativesTitle} empty={center?.excellence_initiatives.length === 0} emptyLabel={labels.emptySection}>
        {center?.excellence_initiatives.map((ini) => (
      ...
      <Section title={labels.reviewsTitle} empty={center?.excellence_reviews.length === 0} emptyLabel={labels.emptySection}>
        {center?.excellence_reviews.map((rev) => (
          <li key={rev.review_key} className="rounded-xl border border-gray-100 p-4 text-sm">
            ...
            {rev.status !== "completed" && center?.can_manage && (
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <ActionBtn label={labels.scheduleWorkshop} variant="muted" onClick={() => void postAction({ action: "schedule_improvement_workshop", review_key: rev.review_key })} />
              </div>
            )}
          </li>
        ))}
      ...
      <Section title={labels.milestonesTitle} empty={center?.excellence_milestones.length === 0} emptyLabel={labels.emptySection}>
        {center?.excellence_milestones.map((ms) => (
        ...
        {center?.can_manage && (
          <li className="pt-2">
            <ActionBtn label={labels.archiveMilestone} onClick={() => void postAction({ action: "archive_excellence_milestone", title: "Excellence milestone", summary: "Excellence milestone archived via Excellence Center." })} />
          </li>
        )}
      </Section>
        ...
        {center?.snapshots.map((snap) => (
          <li key={snap.snapshot_key} className="rounded-xl border border-gray-100 p-3 text-sm">
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              <span className="text-xs text-indigo-700">{labels.excellenceScore}: {snap.excellence_score}</span>
            </div>
      ...
      {center?.executive_view && (
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            ...
            <ExecField label={labels.executiveFields.leadership} value={center.executive_view.leadership_maturity} />
            <ExecField label={labels.executiveFields.opportunities} value={center.executive_view.strategic_opportunities} />
          </dl>
        </section>
      )}
      ...
      <Section title={labels.sessionsTitle} empty={center?.excellence_sessions.length === 0} emptyLabel={labels.emptySection}>
        {center?.excellence_sessions.map((sess) => (
          <li key={sess.session_key} className="rounded-xl border border-gray-100 p-4 text-sm">
            ...
            {sess.status !== "completed" && center?.can_manage && (
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <ActionBtn label={labels.scheduleWorkshop} variant="muted" onClick={() => void postAction({ action: "schedule_improvement_workshop", session_key: sess.session_key })} />
              </div>
            )}
          </li>
        ))}

...
import {
  parseAipifyBusinessContinuityCrisisManagementEngineDashboard,
  type AbosSuccessCriterion,
  type BlueprintObjective,
  type ExecutiveReview,
  type AipifyBusinessContinuityCrisisManagementEngineDashboard,
  type IntegrationLink,
  type ReflectionEntry,
  type ScaffoldNote,
} from "@/lib/aipify/aipify-business-continuity-crisis-management-engine";
...
export function AipifyBusinessContinuityCrisisManagementEngineDashboardPanel({ labels }: Props) {
  const [dashboard, setDashboard] = useState<AipifyBusinessContinuityCrisisManagementEngineDashboard | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/aipify/aipify-business-continuity-crisis-management-engine/dashboard");
    if (res.ok) setDashboard(parseAipifyBusinessContinuityCrisisManagementEngineDashboard(await res.json()));
    setLoading(false);
  }, []);
  ...
  const integrationLinks = dashboard.abccebp227_integration_links?.length
    ? dashboard.abccebp227_integration_links
    : dashboard.integration_links;
  const eraOpener = dashboard.abccebp227_era_opener_summary?.length
    ? dashboard.abccebp227_era_opener_summary
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-xl border border-indigo-200 bg-indigo-50/50 p-6">
        ...
        <p className="mt-2 text-2xl font-bold text-indigo-900">{dashboard.aipify_business_continuity_crisis_management_score ?? 0}</p>
        <p className="text-xs text-indigo-700">{labels.scoreLabel}</p>
        {dashboard.aipify_business_continuity_crisis_management_mission ? <p className="mt-2 text-sm font-medium text-indigo-900">{dashboard.aipify_business_continuity_crisis_management_mission}</p> : null}
        ...
        {dashboard.aipify_business_continuity_crisis_management_vision ? <p className="mt-2 text-xs italic text-indigo-800">{dashboard.aipify_business_continuity_crisis_management_vision}</p> : null}
        <div className="mt-4 flex flex-wrap gap-4 text-sm">
          <div><span className="font-medium">{labels.modeLabel}:</span> {dashboard.crisis_management_mode}</div>
          <div><span className="font-medium">{labels.readinessLabel}:</span> {dashboard.continuity_preparedness_level ?? 1}</div>
      ...
      {dashboard.aipify_business_continuity_crisis_management_objectives?.length ? (
        <section className="space-y-3">
          <h2 className="text-lg font-semibold text-gray-900">{labels.blueprintObjectives}</h2>
          <div className="grid gap-2 sm:grid-cols-2">
            {dashboard.aipify_business_continuity_crisis_management_objectives.map((o) => <ObjectiveCard key={o.key ?? o.label} objective={o} />)}
          </div>
        </section>
      ) : null}
      ...
      {dashboard.aipify_business_continuity_crisis_management_success_criteria?.length ? (
        <section className="space-y-2">
          <h2 className="text-lg font-semibold text-gray-900">{labels.successCriteria}</h2>
          {dashboard.aipify_business_continuity_crisis_management_success_criteria.map((c) => (
            <SuccessCriterionRow key={c.key ?? c.label} criterion={c} metLabel={labels.criterionMet} pendingLabel={labels.criterionPending} />
          ))}
        </section>
      ) : null}

      {dashboard.aipify_business_continuity_crisis_management_privacy_note ? <p className="text-xs text-gray-500">{dashboard.aipify_business_continuity_crisis_management_privacy_note}</p> : null}
    </div>
  );
}

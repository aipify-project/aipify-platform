...
import {
  ...
  OIC_PHILOSOPHY,
  OIC_VISION,
  parseOrganizationalIntentionalityCenter,
  type OrganizationalIntentionalityCenter,
} from "@/lib/organizational-intentionality-center";
...
export function OrganizationalIntentionalityCenterPanel({ labels }: Props) {
  const [center, setCenter] = useState<OrganizationalIntentionalityCenter | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/organizational-intentionality/center");
    if (res.ok) setCenter(parseOrganizationalIntentionalityCenter(await res.json()));
    setLoading(false);
  }, []);
  ...
  const postAction = async (payload: Record<string, unknown>) => {
    await fetch("/api/organizational-intentionality/action", {
      method: "POST",
  ...
  const healthStyle = HEALTH_STYLES[dash?.intentionality_health_label ?? "healthy"] ?? HEALTH_STYLES.healthy;
    ...
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      ...
      <div>
        ...
        <p className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900">
          {labels.corePrinciple}: {OIC_CORE_PRINCIPLE}
        </p>
        <p className="mt-2 text-sm text-gray-600">
          {labels.philosophyTitle}: {OIC_PHILOSOPHY}
        </p>
        <p className="mt-1 text-sm text-gray-600">
          {labels.visionTitle}: {OIC_VISION}
        </p>
      ...
      {dash && (
        <section className={`rounded-2xl border p-5 shadow-sm ${healthStyle}`}>
          ...
          <p className="mt-1 text-sm text-gray-600">
            {labels.healthLabels[dash.intentionality_health_label] ?? dash.intentionality_health_label} ·{" "}
            {labels.intentionalityScore}: {dash.intentionality_score}/100 · {labels.priorityAlignment}:{" "}
            {dash.priority_alignment_pct}%
          </p>
          <dl className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Metric label={labels.metrics.resourceAllocation} value={`${dash.resource_allocation_pct}%`} />
            <Metric label={labels.metrics.purposeIntegration} value={`${dash.purpose_integration_pct}%`} />
            <Metric label={labels.metrics.strategicDiscipline} value={`${dash.strategic_discipline_pct}%`} />
          ...
          {center?.can_manage && (
            <div className="mt-4 flex flex-wrap gap-2">
              <ActionBtn label={labels.generateReport} onClick={() => void postAction({ action: "generate_intentionality_report" })} />
              ...
              <ActionBtn
                ...
                variant="muted"
                onClick={() => void postAction({ action: "export_intentionality_snapshot", period_label: "Current quarter" })}
              />
      ...
      <Section title={labels.signalsTitle} empty={center?.intentionality_signals.length === 0} emptyLabel={labels.emptySection}>
        {center?.intentionality_signals.map((sig) => (
      ...
      <Section title={labels.purposeTitle} empty={center?.purpose_prompts.length === 0} emptyLabel={labels.emptySection}>
        {center?.purpose_prompts.map((per) => (
          <li key={per.purpose_key} className="rounded-xl border border-indigo-100 bg-indigo-50/20 p-4 text-sm">
            <p className="text-xs text-gray-500">{labels.purposeTypes[per.purpose_type] ?? per.purpose_type}</p>
      ...
      <Section title={labels.initiativesTitle} empty={center?.intentionality_initiatives.length === 0} emptyLabel={labels.emptySection}>
        {center?.intentionality_initiatives.map((ini) => (
      ...
      <Section title={labels.reviewsTitle} empty={center?.intentionality_reviews.length === 0} emptyLabel={labels.emptySection}>
        {center?.intentionality_reviews.map((rev) => (
      ...
      <Section title={labels.milestonesTitle} empty={center?.intentionality_milestones.length === 0} emptyLabel={labels.emptySection}>
        {center?.intentionality_milestones.map((ms) => (
        ...
        {center?.can_manage && (
          <li className="pt-2">
            <ActionBtn
              ...
              onClick={() =>
                void postAction({
                  action: "archive_intentionality_milestone",
                  title: "Intentionality milestone",
                  summary: "Intentionality milestone archived via Intentionality Center.",
                })
              }
            />
          </li>
          ...
          <li key={snap.snapshot_key} className="rounded-xl border border-gray-100 p-3 text-sm">
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              <span className="text-xs text-indigo-700">
                {labels.intentionalityScore}: {snap.intentionality_score}
              </span>
            </div>
      ...
      {center?.executive_view && (
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            <ExecField label={labels.executiveFields.strategicDiscipline} value={center.executive_view.strategic_discipline} />
            <ExecField label={labels.executiveFields.purposeAlignment} value={center.executive_view.purpose_alignment} />
            <ExecField label={labels.executiveFields.leadershipReflection} value={center.executive_view.leadership_reflection} />
            <ExecField label={labels.executiveFields.opportunityPrioritization} value={center.executive_view.opportunity_prioritization} />
          </dl>
        </section>
      )}
      ...
      <Section title={labels.sessionsTitle} empty={center?.intentionality_sessions.length === 0} emptyLabel={labels.emptySection}>
        {center?.intentionality_sessions.map((sess) => (

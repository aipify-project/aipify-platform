...
import {
  ...
  OCB_PHILOSOPHY,
  OCB_VISION,
  parseOrganizationalBalanceCenter,
  type OrganizationalBalanceCenter,
} from "@/lib/organizational-balance-center";
...
type PanelLabels = {
  ...
  recalibrationTypes: Record<string, string>;
  ...
  executiveFields: {
    sustainabilityIndicators: string;
    strategicPacing: string;
    leadershipConsistency: string;
    equilibriumOpportunities: string;
  };
};
...
const HEALTH_STYLES: Record<string, string> = {
  ...
  rebalancing_recommended: "border-rose-200 bg-rose-50",
};
...
export function OrganizationalBalanceCenterPanel({ labels }: Props) {
  const [center, setCenter] = useState<OrganizationalBalanceCenter | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/organizational-balance/center");
    if (res.ok) setCenter(parseOrganizationalBalanceCenter(await res.json()));
    setLoading(false);
  }, []);
  ...
  const postAction = async (payload: Record<string, unknown>) => {
    await fetch("/api/organizational-balance/action", {
      method: "POST",
  ...
  const healthStyle = HEALTH_STYLES[dash?.balance_health_label ?? "healthy"] ?? HEALTH_STYLES.healthy;
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      <div className="flex flex-wrap gap-3 text-sm">
        ...
        {center?.links?.organizational_adaptive_intelligence && (
          <Link href={center.links.organizational_adaptive_intelligence} className="text-slate-600 hover:underline">
      ...
      <div>
        ...
        <p className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900">
          {labels.corePrinciple}: {OCB_CORE_PRINCIPLE}
        </p>
        <p className="mt-2 text-sm text-gray-600">
          {labels.philosophyTitle}: {OCB_PHILOSOPHY}
        </p>
        <p className="mt-1 text-sm text-gray-600">
          {labels.visionTitle}: {OCB_VISION}
        </p>
      ...
      {dash && (
        <section className={`rounded-2xl border p-5 shadow-sm ${healthStyle}`}>
          ...
          <p className="mt-1 text-sm text-gray-600">
            {labels.healthLabels[dash.balance_health_label] ?? dash.balance_health_label} ·{" "}
            {labels.balanceScore}: {dash.balance_score}/100 · {labels.sustainabilityIndicators}:{" "}
            {dash.sustainability_indicators_pct}%
          </p>
          <dl className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Metric label={labels.metrics.sustainabilityIndicators} value={`${dash.sustainability_indicators_pct}%`} />
            <Metric label={labels.metrics.strategicPacing} value={`${dash.strategic_pacing_pct}%`} />
            <Metric label={labels.metrics.leadershipConsistency} value={`${dash.leadership_consistency_pct}%`} />
            <Metric label={labels.metrics.workforceResilience} value={`${dash.workforce_resilience_pct}%`} />
            <Metric label={labels.metrics.governanceEffectiveness} value={`${dash.governance_effectiveness_pct}%`} />
          ...
          {center?.can_manage && (
            <div className="mt-4 flex flex-wrap gap-2">
              <ActionBtn label={labels.generateReport} onClick={() => void postAction({ action: "generate_balance_report" })} />
              ...
              <ActionBtn
                ...
                variant="muted"
                onClick={() => void postAction({ action: "export_balance_snapshot", period_label: "Current quarter" })}
              />
      ...
      <Section title={labels.signalsTitle} empty={center?.balance_signals.length === 0} emptyLabel={labels.emptySection}>
        {center?.balance_signals.map((sig) => (
      ...
      <Section title={labels.recalibrationTitle} empty={center?.recalibration_prompts.length === 0} emptyLabel={labels.emptySection}>
        {center?.recalibration_prompts.map((que) => (
          <li key={que.application_key} className="rounded-xl border border-indigo-100 bg-indigo-50/20 p-4 text-sm">
            <p className="text-xs text-gray-500">{labels.recalibrationTypes[que.application_type] ?? que.application_type}</p>
      ...
      <Section title={labels.initiativesTitle} empty={center?.balance_initiatives.length === 0} emptyLabel={labels.emptySection}>
        {center?.balance_initiatives.map((ini) => (
      ...
      <Section title={labels.reviewsTitle} empty={center?.balance_reviews.length === 0} emptyLabel={labels.emptySection}>
        {center?.balance_reviews.map((rev) => (
      ...
      <Section title={labels.milestonesTitle} empty={center?.balance_milestones.length === 0} emptyLabel={labels.emptySection}>
        {center?.balance_milestones.map((ms) => (
        ...
        {center?.can_manage && (
          <li className="pt-2">
            <ActionBtn
              ...
              onClick={() =>
                void postAction({
                  action: "archive_balance_milestone",
                  title: "Adaptive Intelligence milestone",
                  summary: "Adaptive Intelligence milestone archived via Balance Center.",
                })
              }
            />
          </li>
          ...
          <li key={snap.snapshot_key} className="rounded-xl border border-gray-100 p-3 text-sm">
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              <span className="text-xs text-indigo-700">
                {labels.balanceScore}: {snap.balance_score}
              </span>
            </div>
      ...
      {center?.executive_view && (
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            <ExecField label={labels.executiveFields.sustainabilityIndicators} value={center.executive_view.sustainability_indicators} />
            <ExecField label={labels.executiveFields.strategicPacing} value={center.executive_view.strategic_pacing} />
            <ExecField label={labels.executiveFields.leadershipConsistency} value={center.executive_view.leadership_consistency} />
            <ExecField label={labels.executiveFields.equilibriumOpportunities} value={center.executive_view.equilibrium_opportunities} />
          </dl>
        </section>
      )}
      ...
      <Section title={labels.sessionsTitle} empty={center?.balance_sessions.length === 0} emptyLabel={labels.emptySection}>
        {center?.balance_sessions.map((sess) => (

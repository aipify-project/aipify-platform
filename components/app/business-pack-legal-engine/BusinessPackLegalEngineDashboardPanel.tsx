...
import {
  packLegalRoute,
  parseBusinessPackLegalEngineDashboard,
  type BusinessPackLegalEngineDashboard,
} from "@/lib/aipify/business-pack-legal-engine";

type Props = { labels: Record<string, string> };

export function BusinessPackLegalEngineDashboardPanel({ labels }: Props) {
  const [dashboard, setDashboard] = useState<BusinessPackLegalEngineDashboard | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/aipify/business-pack-legal-engine/dashboard");
    if (res.ok) setDashboard(parseBusinessPackLegalEngineDashboard(await res.json()));
    setLoading(false);
  }, []);
    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-3 sm:grid-cols-4">
        {[
          ...
          ["legalDocuments", summary.legal_documents],
          ["acceptanceRecords", summary.acceptance_records],
      ...
      {dashboard.mandatory_documents && dashboard.mandatory_documents.length > 0 && (
        <section className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          <h3 className="text-sm font-semibold text-gray-900">{labels.mandatoryDocuments}</h3>
          <ul className="mt-3 flex flex-wrap gap-2">
            {dashboard.mandatory_documents.map((doc) => (
              <li key={doc} className="rounded-lg border border-gray-100 bg-gray-50 px-2.5 py-1 text-xs capitalize text-gray-700">
                {doc.replace(/_/g, " ")}
              </li>
            ))}
          </ul>
          ...
          <article key={String(def.pack_key)} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
            <div>
              ...
              <p className="text-xs text-gray-500">
                {String(def.governing_law)} · {String(def.document_count)} {labels.documents}
              </p>
            </div>
            <Link
              href={String(def.legal_center_route ?? packLegalRoute(String(def.pack_key)))}

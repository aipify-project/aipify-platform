...
import {
  ...
  OCNC_PHILOSOPHY,
  OCNC_VISION,
  parseOrganizationalContinuityCenter,
  type OrganizationalContinuityCenter,
} from "@/lib/organizational-continuity-center";
...
type PanelLabels = {
  ...
  addressDependency: string;
  advanceSuccession: string;
  generateReport: string;
  printSummary: string;
  exportSnapshot: string;
  coordinateTransfer: string;
  ...
  successionTypes: Record<string, string>;
  successionStatuses: Record<string, string>;
...
const HEALTH_STYLES: Record<string, string> = {
  ...
  vulnerable: "border-rose-200 bg-rose-50",
};
...
export function OrganizationalContinuityCenterPanel({ labels }: Props) {
  const [center, setCenter] = useState<OrganizationalContinuityCenter | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/organizational-continuity/center");
    if (res.ok) setCenter(parseOrganizationalContinuityCenter(await res.json()));
    setLoading(false);
  }, []);
  ...
  const postAction = async (payload: Record<string, unknown>) => {
    await fetch("/api/organizational-continuity/action", {
      method: "POST",
  ...
  const healthStyle = HEALTH_STYLES[dash?.continuity_health_label ?? "stable"] ?? HEALTH_STYLES.stable;
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      <div className="flex flex-wrap gap-3 text-sm">
        ...
        {center?.links?.organizational_coherence && <Link href={center.links.organizational_coherence} className="text-slate-600 hover:underline">{labels.organizationalCoherenceLink}</Link>}
      ...
      <div>
        ...
        <p className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900">{labels.corePrinciple}: {OCNC_CORE_PRINCIPLE}</p>
        <p className="mt-2 text-sm text-gray-600">{labels.philosophyTitle}: {OCNC_PHILOSOPHY}</p>
        <p className="mt-1 text-sm text-gray-600">{labels.visionTitle}: {OCNC_VISION}</p>
      ...
      {dash && (
        <section className={`rounded-2xl border p-5 shadow-sm ${healthStyle}`}>
          ...
          <p className="mt-1 text-sm text-gray-600">
            {labels.healthLabels[dash.continuity_health_label] ?? dash.continuity_health_label} · {labels.continuityScore}: {dash.continuity_score}/100
          </p>
          <dl className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Metric label={labels.metrics.leadership} value={`${dash.leadership_preparedness_pct}%`} />
            <Metric label={labels.metrics.knowledge} value={`${dash.knowledge_continuity_pct}%`} />
            <Metric label={labels.metrics.operational} value={`${dash.operational_resilience_pct}%`} />
            <Metric label={labels.metrics.strategic} value={`${dash.strategic_stability_pct}%`} />
            <Metric label={labels.metrics.succession} value={`${dash.succession_readiness_pct}%`} />
            <Metric label={labels.metrics.documentation} value={`${dash.documentation_maturity_pct}%`} />
            <Metric label={labels.metrics.dependencies} value={dash.dependency_risks} />
            <Metric label={labels.metrics.confidence} value={`${dash.executive_confidence}/5`} />
          </dl>
          {center?.can_manage && (
            <div className="mt-4 flex flex-wrap gap-2">
              <ActionBtn label={labels.generateReport} onClick={() => void postAction({ action: "generate_continuity_report" })} />
              <ActionBtn label={labels.printSummary} variant="muted" onClick={() => void postAction({ action: "print_preparedness_summary" })} />
              <ActionBtn label={labels.exportSnapshot} variant="muted" onClick={() => void postAction({ action: "export_continuity_snapshot", period_label: "Current period" })} />
              <ActionBtn label={labels.coordinateTransfer} variant="muted" onClick={() => void postAction({ action: "coordinate_knowledge_transfer" })} />
            </div>
          )}
        </section>
      )}

      <Section title={labels.dependencyTitle} empty={center?.dependency_signals.length === 0} emptyLabel={labels.emptySection}>
        {center?.dependency_signals.map((sig) => (
          <li key={sig.signal_key} className={`rounded-xl border p-4 text-sm ${TONE_STYLES[sig.signal_tone] ?? TONE_STYLES.neutral}`}>
            ...
            {sig.status === "open" && sig.signal_tone === "attention" && center?.can_manage && (
              <div className="mt-3">
                <ActionBtn label={labels.addressDependency} onClick={() => void postAction({ action: "address_dependency", signal_key: sig.signal_key })} />
              </div>
            )}
          </li>
        ))}
      ...
      <Section title={labels.successionTitle} empty={center?.succession_items.length === 0} emptyLabel={labels.emptySection}>
        {center?.succession_items.map((item) => (
          <li key={item.succession_key} className="rounded-xl border border-gray-100 p-4 text-sm">
            <div className="flex flex-wrap items-start justify-between gap-2">
              <p className="text-xs text-gray-500">{labels.domains[item.domain] ?? item.domain} · {labels.successionTypes[item.succession_type] ?? item.succession_type}</p>
              <span className="text-xs font-medium text-indigo-700">{labels.successionStatuses[item.status] ?? item.status}</span>
            </div>
            <p className="mt-1 font-medium text-gray-900">{item.title}</p>
            <p className="mt-2 text-gray-700">{item.summary}</p>
            {item.status !== "completed" && center?.can_manage && (
              <div className="mt-3">
                <ActionBtn label={labels.advanceSuccession} onClick={() => void postAction({ action: "advance_succession", succession_key: item.succession_key })} />
              </div>
            )}
          </li>
        ))}
      ...
      <Section title={labels.reviewsTitle} empty={center?.continuity_reviews.length === 0} emptyLabel={labels.emptySection}>
        {center?.continuity_reviews.map((rev) => (
          <li key={rev.review_key} className="rounded-xl border border-gray-100 p-4 text-sm">
            ...
            {rev.status !== "completed" && center?.can_manage && (
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <ActionBtn label={labels.scheduleDiscussion} variant="muted" onClick={() => void postAction({ action: "schedule_succession_discussion", review_key: rev.review_key })} />
              </div>
            )}
          </li>
        ))}
      ...
      <Section title={labels.milestonesTitle} empty={center?.continuity_milestones.length === 0} emptyLabel={labels.emptySection}>
        {center?.continuity_milestones.map((ms) => (
        ...
        {center?.can_manage && (
          <li className="pt-2">
            <ActionBtn label={labels.archiveMilestone} onClick={() => void postAction({ action: "archive_continuity_milestone", title: "Continuity milestone", summary: "Continuity milestone archived via Continuity Center." })} />
          </li>
        )}
      </Section>
        ...
        {center?.snapshots.map((snap) => (
          <li key={snap.snapshot_key} className="rounded-xl border border-gray-100 p-3 text-sm">
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              <span className="text-xs text-indigo-700">{labels.continuityScore}: {snap.continuity_score}</span>
            </div>
      ...
      {center?.executive_view && (
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            <ExecField label={labels.executiveFields.leadership} value={center.executive_view.leadership_continuity} />
            <ExecField label={labels.executiveFields.consistency} value={center.executive_view.strategic_consistency} />
            <ExecField label={labels.executiveFields.knowledge} value={center.executive_view.knowledge_preservation} />
            <ExecField label={labels.executiveFields.opportunities} value={center.executive_view.resilience_opportunities} />
          </dl>
        </section>
      )}
      ...
      <Section title={labels.sessionsTitle} empty={center?.continuity_sessions.length === 0} emptyLabel={labels.emptySection}>
        {center?.continuity_sessions.map((sess) => (
          <li key={sess.session_key} className="rounded-xl border border-gray-100 p-4 text-sm">
            ...
            {sess.status !== "completed" && center?.can_manage && (
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <ActionBtn label={labels.scheduleDiscussion} variant="muted" onClick={() => void postAction({ action: "schedule_succession_discussion", session_key: sess.session_key })} />
              </div>
            )}
          </li>
        ))}

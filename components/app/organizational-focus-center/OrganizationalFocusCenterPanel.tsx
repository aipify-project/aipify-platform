...
type PanelLabels = {
  ...
  executiveFields: {
    priorityAlignment: string;
    strategicConcentration: string;
    leadershipReinforcement: string;
    focusOpportunities: string;
  };
};
...
const HEALTH_STYLES: Record<string, string> = {
  ...
  focus_reinforcement_recommended: "border-rose-200 bg-rose-50",
};
...
export function OrganizationalFocusCenterPanel({ labels }: Props) {
  ...
  const healthStyle = HEALTH_STYLES[dash?.focus_health_label ?? "healthy"] ?? HEALTH_STYLES.healthy;
    ...
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      ...
      <div>
        ...
        <p className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900">
          {labels.corePrinciple}: {OFC_CORE_PRINCIPLE}
        </p>
        <p className="mt-2 text-sm text-gray-600">
          {labels.philosophyTitle}: {OFC_PHILOSOPHY}
        </p>
        <p className="mt-1 text-sm text-gray-600">
          {labels.visionTitle}: {OFC_VISION}
        </p>
      ...
      {dash && (
        <section className={`rounded-2xl border p-5 shadow-sm ${healthStyle}`}>
          ...
          <p className="mt-1 text-sm text-gray-600">
            {labels.healthLabels[dash.focus_health_label] ?? dash.focus_health_label} · {labels.focusScore}:{" "}
            {dash.focus_score}/100 · {labels.priorityAlignment}: {dash.priority_alignment_pct}%
          </p>
          <dl className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Metric label={labels.metrics.priorityAlignment} value={`${dash.priority_alignment_pct}%`} />
            <Metric label={labels.metrics.initiativeConcentration} value={`${dash.initiative_concentration_pct}%`} />
            <Metric label={labels.metrics.executionClarity} value={`${dash.execution_clarity_pct}%`} />
            <Metric label={labels.metrics.priorityClarity} value={`${dash.priority_clarity_pct}%`} />
            <Metric label={labels.metrics.initiativeOverloadRisk} value={`${dash.initiative_overload_risk_pct}%`} />
            <Metric label={labels.metrics.leadershipConsistency} value={`${dash.leadership_consistency_pct}%`} />
            <Metric label={labels.metrics.resourceConcentration} value={`${dash.resource_concentration_pct}%`} />
          ...
          {center?.can_manage && (
            <div className="mt-4 flex flex-wrap gap-2">
              <ActionBtn label={labels.generateReport} onClick={() => void postAction({ action: "generate_focus_report" })} />
              ...
              <ActionBtn
                ...
                variant="muted"
                onClick={() => void postAction({ action: "export_focus_snapshot", period_label: "Current quarter" })}
              />
      ...
      <Section title={labels.signalsTitle} empty={center?.focus_signals.length === 0} emptyLabel={labels.emptySection}>
        {center?.focus_signals.map((sig) => (
      ...
      <Section title={labels.priorityTitle} empty={center?.priority_prompts.length === 0} emptyLabel={labels.emptySection}>
        {center?.priority_prompts.map((que) => (
          <li key={que.question_key} className="rounded-xl border border-indigo-100 bg-indigo-50/20 p-4 text-sm">
            <p className="text-xs text-gray-500">{labels.priorityTypes[que.question_type] ?? que.question_type}</p>
      ...
      <Section title={labels.initiativesTitle} empty={center?.focus_initiatives.length === 0} emptyLabel={labels.emptySection}>
        {center?.focus_initiatives.map((ini) => (
      ...
      <Section title={labels.milestonesTitle} empty={center?.focus_milestones.length === 0} emptyLabel={labels.emptySection}>
        {center?.focus_milestones.map((ms) => (
        ...
        {center?.can_manage && (
          <li className="pt-2">
            <ActionBtn
              ...
              onClick={() =>
                void postAction({
                  action: "archive_focus_milestone",
          ...
          <li key={snap.snapshot_key} className="rounded-xl border border-gray-100 p-3 text-sm">
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              <span className="text-xs text-indigo-700">
                {labels.focusScore}: {snap.focus_score}
              </span>
            </div>
      ...
      {center?.executive_view && (
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            <ExecField label={labels.executiveFields.priorityAlignment} value={center.executive_view.priority_alignment} />
            <ExecField label={labels.executiveFields.strategicConcentration} value={center.executive_view.strategic_concentration} />
            <ExecField label={labels.executiveFields.leadershipReinforcement} value={center.executive_view.leadership_reinforcement} />
            <ExecField label={labels.executiveFields.focusOpportunities} value={center.executive_view.focus_opportunities} />
          </dl>
        </section>
      )}
      ...
      <Section title={labels.sessionsTitle} empty={center?.focus_sessions.length === 0} emptyLabel={labels.emptySection}>
        {center?.focus_sessions.map((sess) => (

...
import {
  ...
  OCAI_PHILOSOPHY,
  OCAI_VISION,
  parseOrganizationalAdaptiveIntelligenceCenter,
  type OrganizationalAdaptiveIntelligenceCenter,
} from "@/lib/organizational-adaptive-intelligence-center";
...
type PanelLabels = {
  ...
  signalsTitle: string;
  learningApplicationTitle: string;
  ...
  humansDecide: string;
  privacyNote: string;
  adaptiveIntelligenceScore: string;
  capabilityEvolutionMetric: string;
  domains: Record<string, string>;
  signalTypes: Record<string, string>;
  signalTones: Record<string, string>;
  learningApplicationTypes: Record<string, string>;
  ...
  executiveFields: {
    leadershipLearning: string;
    strategicResponsiveness: string;
...
const HEALTH_STYLES: Record<string, string> = {
  ...
  adaptation_opportunities_identified: "border-rose-200 bg-rose-50",
};
...
export function OrganizationalAdaptiveIntelligenceCenterPanel({ labels }: Props) {
  const [center, setCenter] = useState<OrganizationalAdaptiveIntelligenceCenter | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/organizational-adaptive-intelligence/center");
    if (res.ok) setCenter(parseOrganizationalAdaptiveIntelligenceCenter(await res.json()));
    setLoading(false);
  }, []);
  ...
  const postAction = async (payload: Record<string, unknown>) => {
    await fetch("/api/organizational-adaptive-intelligence/action", {
      method: "POST",
  ...
  const healthStyle = HEALTH_STYLES[dash?.adaptive_intelligence_health_label ?? "healthy"] ?? HEALTH_STYLES.healthy;
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      <div className="flex flex-wrap gap-3 text-sm">
        ...
        {center?.links?.organizational_wisdom_transfer && (
          <Link href={center.links.organizational_wisdom_transfer} className="text-slate-600 hover:underline">
      ...
      <div>
        ...
        <p className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900">
          {labels.corePrinciple}: {OCAI_CORE_PRINCIPLE}
        </p>
        <p className="mt-2 text-sm text-gray-600">
          {labels.philosophyTitle}: {OCAI_PHILOSOPHY}
        </p>
        <p className="mt-1 text-sm text-gray-600">
          {labels.visionTitle}: {OCAI_VISION}
        </p>
      ...
      {dash && (
        <section className={`rounded-2xl border p-5 shadow-sm ${healthStyle}`}>
          ...
          <p className="mt-1 text-sm text-gray-600">
            {labels.healthLabels[dash.adaptive_intelligence_health_label] ?? dash.adaptive_intelligence_health_label} ·{" "}
            {labels.adaptiveIntelligenceScore}: {dash.adaptive_intelligence_score}/100 · {labels.capabilityEvolutionMetric}:{" "}
            {dash.capability_evolution_pct}%
          </p>
          <dl className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Metric label={labels.metrics.learningEffectiveness} value={`${dash.learning_effectiveness_pct}%`} />
            <Metric label={labels.metrics.reflectionParticipation} value={`${dash.reflection_participation_pct}%`} />
            <Metric label={labels.metrics.strategicResponsivenessMetric} value={`${dash.strategic_responsiveness_pct}%`} />
            <Metric label={labels.metrics.capabilityEvolutionMetric} value={`${dash.capability_evolution_pct}%`} />
            <Metric label={labels.metrics.decisionAdaptability} value={`${dash.decision_adaptability_pct}%`} />
            <Metric label={labels.metrics.responsivenessTrends} value={`${dash.responsiveness_trends_pct}%`} />
          ...
          {center?.can_manage && (
            <div className="mt-4 flex flex-wrap gap-2">
              <ActionBtn label={labels.generateReport} onClick={() => void postAction({ action: "generate_adaptive_intelligence_report" })} />
              ...
              <ActionBtn
                ...
                variant="muted"
                onClick={() => void postAction({ action: "export_adaptive_intelligence_snapshot", period_label: "Current quarter" })}
              />
      ...
      <Section title={labels.signalsTitle} empty={center?.adaptive_intelligence_signals.length === 0} emptyLabel={labels.emptySection}>
        {center?.adaptive_intelligence_signals.map((sig) => (
      ...
      <Section title={labels.learningApplicationTitle} empty={center?.learning_application_prompts.length === 0} emptyLabel={labels.emptySection}>
        {center?.learning_application_prompts.map((que) => (
          <li key={que.application_key} className="rounded-xl border border-indigo-100 bg-indigo-50/20 p-4 text-sm">
            <p className="text-xs text-gray-500">{labels.learningApplicationTypes[que.application_type] ?? que.application_type}</p>
      ...
      <Section title={labels.initiativesTitle} empty={center?.adaptive_intelligence_initiatives.length === 0} emptyLabel={labels.emptySection}>
        {center?.adaptive_intelligence_initiatives.map((ini) => (
      ...
      <Section title={labels.reviewsTitle} empty={center?.adaptive_intelligence_reviews.length === 0} emptyLabel={labels.emptySection}>
        {center?.adaptive_intelligence_reviews.map((rev) => (
      ...
      <Section title={labels.milestonesTitle} empty={center?.adaptive_intelligence_milestones.length === 0} emptyLabel={labels.emptySection}>
        {center?.adaptive_intelligence_milestones.map((ms) => (
        ...
        {center?.can_manage && (
          <li className="pt-2">
            <ActionBtn
              ...
              onClick={() =>
                void postAction({
                  action: "archive_adaptive_intelligence_milestone",
                  title: "Adaptive Intelligence milestone",
                  summary: "Adaptive Intelligence milestone archived via Adaptive Intelligence Center.",
                })
              }
            />
          </li>
          ...
          <li key={snap.snapshot_key} className="rounded-xl border border-gray-100 p-3 text-sm">
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              <span className="text-xs text-indigo-700">
                {labels.adaptiveIntelligenceScore}: {snap.adaptive_intelligence_score}
              </span>
            </div>
      ...
      {center?.executive_view && (
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            <ExecField label={labels.executiveFields.leadershipLearning} value={center.executive_view.leadership_learning} />
            <ExecField label={labels.executiveFields.strategicResponsiveness} value={center.executive_view.strategic_responsiveness} />
            <ExecField label={labels.executiveFields.capabilityEvolution} value={center.executive_view.capability_evolution} />
            <ExecField label={labels.executiveFields.futureReadiness} value={center.executive_view.future_readiness} />
          </dl>
        </section>
      )}
      ...
      <Section title={labels.sessionsTitle} empty={center?.adaptive_intelligence_sessions.length === 0} emptyLabel={labels.emptySection}>
        {center?.adaptive_intelligence_sessions.map((sess) => (

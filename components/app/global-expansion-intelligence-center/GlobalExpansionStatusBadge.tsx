...
export function GlobalExpansionStatusBadge({
  statusKey,
  labels,
}: {
  statusKey: string;
  labels: GlobalExpansionIntelligenceCenterLabels["status"];
}) {
  const def = getOperationsStatusDefinition(statusKey);
  const label = labels[def.labelKey as keyof GlobalExpansionIntelligenceCenterLabels["status"]] ?? def.labelKey;
  return (

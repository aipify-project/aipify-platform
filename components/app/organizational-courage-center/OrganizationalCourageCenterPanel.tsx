...
import {
  ...
  OCCE_PHILOSOPHY,
  OCCE_VISION,
  parseOrganizationalCourageCenter,
  type OrganizationalCourageCenter,
} from "@/lib/organizational-courage-center";
...
type PanelLabels = {
  ...
  executiveFields: {
    leadershipIntegrity: string;
    innovationConfidence: string;
    ethicalConsistency: string;
    valuesBasedDecisions: string;
  };
};
...
const HEALTH_STYLES: Record<string, string> = {
  ...
  courage_development_recommended: "border-rose-200 bg-rose-50",
};
...
export function OrganizationalCourageCenterPanel({ labels }: Props) {
  const [center, setCenter] = useState<OrganizationalCourageCenter | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/organizational-courage/center");
    if (res.ok) setCenter(parseOrganizationalCourageCenter(await res.json()));
    setLoading(false);
  }, []);
  ...
  const postAction = async (payload: Record<string, unknown>) => {
    await fetch("/api/organizational-courage/action", {
      method: "POST",
  ...
  const healthStyle = HEALTH_STYLES[dash?.courage_health_label ?? "healthy"] ?? HEALTH_STYLES.healthy;
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      <div className="flex flex-wrap gap-3 text-sm">
        ...
        {center?.links?.organizational_curiosity && (
          <Link href={center.links.organizational_curiosity} className="text-slate-600 hover:underline">
      ...
      <div>
        ...
        <p className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900">
          {labels.corePrinciple}: {OCCE_CORE_PRINCIPLE}
        </p>
        <p className="mt-2 text-sm text-gray-600">
          {labels.philosophyTitle}: {OCCE_PHILOSOPHY}
        </p>
        <p className="mt-1 text-sm text-gray-600">
          {labels.visionTitle}: {OCCE_VISION}
        </p>
      ...
      {dash && (
        <section className={`rounded-2xl border p-5 shadow-sm ${healthStyle}`}>
          ...
          <p className="mt-1 text-sm text-gray-600">
            {labels.healthLabels[dash.courage_health_label] ?? dash.courage_health_label} ·{" "}
            {labels.courageScore}: {dash.courage_score}/100 · {labels.valuesAlignedDecisions}:{" "}
            {dash.values_aligned_decisions_pct}%
          </p>
          <dl className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Metric label={labels.metrics.leadershipTransparency} value={`${dash.leadership_transparency_pct}%`} />
            <Metric label={labels.metrics.reflectionParticipation} value={`${dash.reflection_participation_pct}%`} />
            <Metric label={labels.metrics.ethicalConsistencyMetric} value={`${dash.ethical_consistency_pct}%`} />
            <Metric label={labels.metrics.learningIntegration} value={`${dash.learning_integration_pct}%`} />
            <Metric label={labels.metrics.responsibleInnovation} value={`${dash.responsible_innovation_pct}%`} />
            <Metric label={labels.metrics.leadershipReflection} value={`${dash.leadership_reflection_pct}%`} />
          ...
          {center?.can_manage && (
            <div className="mt-4 flex flex-wrap gap-2">
              <ActionBtn label={labels.generateReport} onClick={() => void postAction({ action: "generate_courage_report" })} />
              ...
              <ActionBtn
                ...
                variant="muted"
                onClick={() => void postAction({ action: "export_courage_snapshot", period_label: "Current quarter" })}
              />
      ...
      <Section title={labels.signalsTitle} empty={center?.courage_signals.length === 0} emptyLabel={labels.emptySection}>
        {center?.courage_signals.map((sig) => (
      ...
      <Section title={labels.conversationTitle} empty={center?.conversation_prompts.length === 0} emptyLabel={labels.emptySection}>
        {center?.conversation_prompts.map((que) => (
          <li key={que.conversation_key} className="rounded-xl border border-indigo-100 bg-indigo-50/20 p-4 text-sm">
            <p className="text-xs text-gray-500">{labels.conversationTypes[que.conversation_type] ?? que.conversation_type}</p>
      ...
      <Section title={labels.initiativesTitle} empty={center?.courage_initiatives.length === 0} emptyLabel={labels.emptySection}>
        {center?.courage_initiatives.map((ini) => (
      ...
      <Section title={labels.reviewsTitle} empty={center?.courage_reviews.length === 0} emptyLabel={labels.emptySection}>
        {center?.courage_reviews.map((rev) => (
      ...
      <Section title={labels.milestonesTitle} empty={center?.courage_milestones.length === 0} emptyLabel={labels.emptySection}>
        {center?.courage_milestones.map((ms) => (
        ...
        {center?.can_manage && (
          <li className="pt-2">
            <ActionBtn
              ...
              onClick={() =>
                void postAction({
                  action: "archive_courage_milestone",
                  title: "Courage milestone",
                  summary: "Courage milestone archived via Courage Center.",
                })
              }
            />
          </li>
          ...
          <li key={snap.snapshot_key} className="rounded-xl border border-gray-100 p-3 text-sm">
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              <span className="text-xs text-indigo-700">
                {labels.courageScore}: {snap.courage_score}
              </span>
            </div>
      ...
      {center?.executive_view && (
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            <ExecField label={labels.executiveFields.leadershipIntegrity} value={center.executive_view.leadership_integrity} />
            <ExecField label={labels.executiveFields.innovationConfidence} value={center.executive_view.innovation_confidence} />
            <ExecField label={labels.executiveFields.ethicalConsistency} value={center.executive_view.ethical_consistency} />
            <ExecField label={labels.executiveFields.valuesBasedDecisions} value={center.executive_view.values_based_decisions} />
          </dl>
        </section>
      )}
      ...
      <Section title={labels.sessionsTitle} empty={center?.courage_sessions.length === 0} emptyLabel={labels.emptySection}>
        {center?.courage_sessions.map((sess) => (

...
import {
  parseHumanWisdomAugmentedJudgmentDashboard,
  type AbosSuccessCriterion,
  type ScaffoldNote,
  type BlueprintObjective,
  type ExecutiveReview,
  type HumanWisdomAugmentedJudgmentDashboard,
  type IntegrationLink,
  type ReflectionEntry,
} from "@/lib/aipify/human-wisdom-augmented-judgment-engine";
...
function AgencyRow({ note }: { note: ScaffoldNote }) {
  return (
...
export function HumanWisdomAugmentedJudgmentEngineDashboardPanel({ labels }: Props) {
  const [dashboard, setDashboard] = useState<HumanWisdomAugmentedJudgmentDashboard | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/aipify/human-wisdom-augmented-judgment-engine/dashboard");
    if (res.ok) {
      setDashboard(parseHumanWisdomAugmentedJudgmentDashboard(await res.json()));
    }
    setLoading(false);
  }, []);
  ...
  const integrationLinks =
    dashboard.hwajbp175_integration_links?.length
      ? dashboard.hwajbp175_integration_links
      : dashboard.integration_links;

  const eraOpener =
    dashboard.hwajbp175_era_opener_summary?.length
      ? dashboard.hwajbp175_era_opener_summary
    ...
    <div className="space-y-6">
      <section className="rounded-xl border border-indigo-200 bg-indigo-50/50 p-6">
        ...
        <p className="mt-2 text-2xl font-bold text-indigo-900">
          {dashboard.human_wisdom_judgment_score ?? 0}
        </p>
        <p className="text-xs text-indigo-700">{labels.humanWisdomAugmentedJudgmentScore}</p>
        {dashboard.human_wisdom_augmented_judgment_mission ? (
        ...
        {dashboard.human_wisdom_augmented_judgment_vision ? (
          <p className="mt-2 text-xs italic text-indigo-800">{dashboard.human_wisdom_augmented_judgment_vision}</p>
        ) : null}
        <div className="mt-4 flex flex-wrap gap-4 text-sm">
          <div>
            <span className="font-medium">{labels.discoveryMode}:</span> {dashboard.judgment_mode}
          </div>
          <div>
            <span className="font-medium">{labels.meaningReadinessLevel}:</span>{" "}
            {dashboard.wisdom_readiness_level ?? 1}
          </div>
          <div>
      ...
      <MetaListSection
        title={labels.centerCapabilities}
        meta={dashboard.center_meta}
        itemsKey="capabilities"
      />

      <MetaListSection
        title={labels.reflectionEngine}
        meta={dashboard.engine_meta}
        itemsKey="reflection_questions"
      />

      <MetaListSection
        title={labels.responsibleFramework}
        meta={dashboard.framework_meta}
        itemsKey="domains"
      />

      <MetaListSection
        title={labels.executiveReviewsMeta}
        meta={dashboard.executive_reviews_meta}
        itemsKey="review_themes"
      />

      <MetaListSection
        title={labels.companionCapabilities}
        meta={dashboard.companion_meta}
        itemsKey="capabilities"
      />

      <MetaListSection
        title={labels.supportingEngine}
        meta={dashboard.sub_engine_meta}
        itemsKey="characteristics"
      />

      <MetaListSection
        title={labels.oversightEngine}
        meta={dashboard.supporting_engine_meta}
        itemsKey="protections"
      />

      {dashboard.reflections.length > 0 ? (
        <section className="space-y-3">
          <h2 className="text-lg font-semibold text-gray-900">{labels.reflectionScaffolds}</h2>
      ...
      {dashboard.scaffold_notes.length > 0 ? (
        <section className="space-y-3">
          ...
          <div className="space-y-2">
            {dashboard.scaffold_notes.map((note) => (
      ...
      {dashboard.human_wisdom_augmented_judgment_objectives?.length ? (
        <section className="space-y-3">
          <h2 className="text-lg font-semibold text-gray-900">{labels.blueprintObjectives}</h2>
          <div className="grid gap-2 sm:grid-cols-2">
            {dashboard.human_wisdom_augmented_judgment_objectives.map((objective) => (
      ...
      {dashboard.human_wisdom_augmented_judgment_success_criteria?.length ? (
        <section className="space-y-2">
          <h2 className="text-lg font-semibold text-gray-900">{labels.successCriteria}</h2>
          {dashboard.human_wisdom_augmented_judgment_success_criteria.map((criterion) => (
            <SuccessCriterionRow
      ...
      {dashboard.human_wisdom_augmented_judgment_privacy_note ? (
        <p className="text-xs text-gray-500">{dashboard.human_wisdom_augmented_judgment_privacy_note}</p>
      ) : null}
    </div>
  );
}

...
import {
  ...
  OFC_PHILOSOPHY,
  OFC_VISION,
  parseOrganizationalFuturesCenter,
  type OrganizationalFuturesCenter,
} from "@/lib/organizational-futures-center";
...
type PanelLabels = {
  ...
  readinessDimensions: Record<string, string>;
...
const READINESS_STYLES: Record<string, string> = {
  highly_prepared: "border-emerald-200 bg-emerald-50",
...
export function OrganizationalFuturesCenterPanel({ labels }: Props) {
  const [center, setCenter] = useState<OrganizationalFuturesCenter | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/organizational-futures/center");
    if (res.ok) setCenter(parseOrganizationalFuturesCenter(await res.json()));
    setLoading(false);
  }, []);
  ...
  const postAction = async (payload: Record<string, unknown>) => {
    await fetch("/api/organizational-futures/action", {
      method: "POST",
  ...
  const readinessStyle = READINESS_STYLES[dash?.readiness_label ?? "developing"] ?? READINESS_STYLES.developing;
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      ...
      {dash && (
        <section className={`rounded-2xl border p-5 shadow-sm ${readinessStyle}`}>
          ...
          <p className="mt-1 text-sm text-gray-600">
            {labels.healthLabels[dash.readiness_label] ?? dash.readiness_label} · {labels.readinessScore}: {dash.readiness_score}/100
          </p>
          <dl className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Metric label={labels.metrics.scenariosExplored} value={`${dash.scenarios_explored}/${dash.scenarios_total}`} />
            <Metric label={labels.metrics.signals} value={dash.signals_identified} />
            <Metric label={labels.metrics.preparedness} value={dash.preparedness_initiatives} />
            <Metric label={labels.metrics.reviews} value={`${dash.review_participation_pct}%`} />
            <Metric label={labels.metrics.capabilities} value={`${dash.capabilities_readiness_pct}%`} />
            <Metric label={labels.metrics.technology} value={`${dash.technology_readiness_pct}%`} />
            <Metric label={labels.metrics.flexibility} value={`${dash.strategic_flexibility_pct}%`} />
            <Metric label={labels.metrics.confidence} value={`${dash.executive_confidence}/5`} />
          </dl>
          {center?.can_manage && (
            <div className="mt-4 flex flex-wrap gap-2">
              <ActionBtn label={labels.generateReport} onClick={() => void postAction({ action: "generate_futures_report" })} />
              <ActionBtn label={labels.printSummary} variant="muted" onClick={() => void postAction({ action: "print_scenario_summary" })} />
              <ActionBtn label={labels.exportSnapshot} variant="muted" onClick={() => void postAction({ action: "export_foresight_snapshot", period_label: "Current period" })} />
              <ActionBtn label={labels.coordinateReflection} variant="muted" onClick={() => void postAction({ action: "coordinate_leadership_reflection" })} />
            </div>
          )}
        </section>
      )}

      <Section title={labels.scenariosTitle} empty={center?.scenarios.length === 0} emptyLabel={labels.emptySection}>
        {center?.scenarios.map((sc) => (
          <li key={sc.scenario_key} className="rounded-xl border border-gray-100 p-4 text-sm">
            <div className="flex flex-wrap items-start justify-between gap-2">
              <p className="text-xs text-gray-500">{labels.domains[sc.domain] ?? sc.domain} · {labels.scenarioTypes[sc.scenario_type] ?? sc.scenario_type}</p>
              <span className="text-xs font-medium text-indigo-700">{labels.scenarioStatuses[sc.status] ?? sc.status}</span>
            </div>
            <p className="mt-1 font-medium text-gray-900">{sc.title}</p>
            <p className="mt-2 text-gray-700">{sc.summary}</p>
            {sc.status !== "explored" && center?.can_manage && (
              <div className="mt-3">
                <ActionBtn label={labels.exploreScenario} onClick={() => void postAction({ action: "explore_scenario", scenario_key: sc.scenario_key })} />
              </div>
            )}
          </li>
        ))}
      ...
      <Section title={labels.signalsTitle} empty={center?.future_signals.length === 0} emptyLabel={labels.emptySection}>
        {center?.future_signals.map((sig) => (
      ...
      <Section title={labels.readinessTitle} empty={center?.readiness_items.length === 0} emptyLabel={labels.emptySection}>
        {center?.readiness_items.map((rd) => (
          <li key={rd.readiness_key} className="rounded-xl border border-gray-100 p-4 text-sm">
            <div className="flex flex-wrap items-start justify-between gap-2">
              <p className="text-xs text-gray-500">{labels.readinessDimensions[rd.dimension] ?? rd.dimension}</p>
              <span className="text-xs font-medium text-indigo-700">{labels.readinessLevels[rd.readiness_level] ?? rd.readiness_level}</span>
            </div>
            <p className="mt-1 font-medium text-gray-900">{rd.title}</p>
            <p className="mt-2 text-gray-700">{rd.summary}</p>
          </li>
        ))}
      </Section>

      <Section title={labels.reviewsTitle} empty={center?.futures_reviews.length === 0} emptyLabel={labels.emptySection}>
        {center?.futures_reviews.map((rev) => (
          <li key={rev.review_key} className="rounded-xl border border-gray-100 p-4 text-sm">
            ...
            {rev.status !== "completed" && center?.can_manage && (
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <ActionBtn label={labels.scheduleWorkshop} variant="muted" onClick={() => void postAction({ action: "schedule_executive_workshop", review_key: rev.review_key })} />
              </div>
            )}
          </li>
        ))}
      ...
      <Section title={labels.archivesTitle} empty={center?.archived_scenarios.length === 0} emptyLabel={labels.emptySection}>
        {center?.archived_scenarios.map((arch) => (
          <li key={arch.archive_key} className="rounded-xl border border-gray-100 p-3 text-sm">
            <p className="text-xs text-gray-500">{labels.domains[arch.domain] ?? arch.domain}</p>
            <p className="mt-1 font-medium text-gray-900">{arch.title}</p>
            <p className="mt-1 text-gray-700">{arch.summary}</p>
          </li>
        ))}
        {center?.can_manage && (
          <li className="pt-2">
            <ActionBtn label={labels.archiveHistory} onClick={() => void postAction({ action: "archive_scenario_history", title: "Scenario history", summary: "Scenario history archived via Futures Center." })} />
          </li>
        )}
      </Section>
        ...
        {center?.snapshots.map((snap) => (
          <li key={snap.snapshot_key} className="rounded-xl border border-gray-100 p-3 text-sm">
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              <span className="text-xs text-indigo-700">{labels.readinessScore}: {snap.readiness_score}</span>
            </div>
      ...
      {center?.executive_view && (
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            <ExecField label={labels.executiveFields.scenarioReadiness} value={center.executive_view.scenario_readiness} />
            <ExecField label={labels.executiveFields.resilience} value={center.executive_view.strategic_resilience} />
            <ExecField label={labels.executiveFields.signals} value={center.executive_view.emerging_signals} />
      ...
      <Section title={labels.sessionsTitle} empty={center?.futures_sessions.length === 0} emptyLabel={labels.emptySection}>
        {center?.futures_sessions.map((sess) => (
          <li key={sess.session_key} className="rounded-xl border border-gray-100 p-4 text-sm">
            ...
            {sess.status !== "completed" && center?.can_manage && (
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <ActionBtn label={labels.scheduleWorkshop} variant="muted" onClick={() => void postAction({ action: "schedule_executive_workshop", session_key: sess.session_key })} />
              </div>
            )}
          </li>
        ))}

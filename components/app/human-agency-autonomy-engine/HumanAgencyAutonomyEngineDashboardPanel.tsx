...
import {
  parseHumanAgencyAutonomyDashboard,
  type AbosSuccessCriterion,
  type ScaffoldNote,
  type BlueprintObjective,
  type ExecutiveReview,
  type HumanAgencyAutonomyDashboard,
  type IntegrationLink,
  type ReflectionEntry,
} from "@/lib/aipify/human-agency-autonomy-engine";
...
export function HumanAgencyAutonomyEngineDashboardPanel({ labels }: Props) {
  const [dashboard, setDashboard] = useState<HumanAgencyAutonomyDashboard | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/aipify/human-agency-autonomy-engine/dashboard");
    if (res.ok) {
      setDashboard(parseHumanAgencyAutonomyDashboard(await res.json()));
    }
    setLoading(false);
  }, []);
  ...
  const integrationLinks =
    dashboard.haarbp176_integration_links?.length
      ? dashboard.haarbp176_integration_links
      : dashboard.integration_links;

  const eraOpener =
    dashboard.haarbp176_era_opener_summary?.length
      ? dashboard.haarbp176_era_opener_summary
    ...
    <div className="space-y-6">
      <section className="rounded-xl border border-indigo-200 bg-indigo-50/50 p-6">
        ...
        <p className="mt-2 text-2xl font-bold text-indigo-900">
          {dashboard.human_agency_autonomy_score ?? 0}
        </p>
        <p className="text-xs text-indigo-700">{labels.humanAgencyAutonomyScore}</p>
        {dashboard.human_agency_autonomy_mission ? (
        ...
        {dashboard.human_agency_autonomy_vision ? (
          <p className="mt-2 text-xs italic text-indigo-800">{dashboard.human_agency_autonomy_vision}</p>
        ) : null}
        <div className="mt-4 flex flex-wrap gap-4 text-sm">
          <div>
            <span className="font-medium">{labels.discoveryMode}:</span> {dashboard.autonomy_mode}
          </div>
          <div>
            <span className="font-medium">{labels.meaningReadinessLevel}:</span>{" "}
            {dashboard.agency_readiness_level ?? 1}
          </div>
          <div>
      ...
      {dashboard.human_agency_autonomy_objectives?.length ? (
        <section className="space-y-3">
          <h2 className="text-lg font-semibold text-gray-900">{labels.blueprintObjectives}</h2>
          <div className="grid gap-2 sm:grid-cols-2">
            {dashboard.human_agency_autonomy_objectives.map((objective) => (
      ...
      {dashboard.human_agency_autonomy_success_criteria?.length ? (
        <section className="space-y-2">
          <h2 className="text-lg font-semibold text-gray-900">{labels.successCriteria}</h2>
          {dashboard.human_agency_autonomy_success_criteria.map((criterion) => (
            <SuccessCriterionRow
      ...
      {dashboard.human_agency_autonomy_privacy_note ? (
        <p className="text-xs text-gray-500">{dashboard.human_agency_autonomy_privacy_note}</p>
      ) : null}
    </div>
  );
}

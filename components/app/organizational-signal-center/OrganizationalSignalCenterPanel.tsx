...
import {
  ...
  OSIG_PHILOSOPHY,
  OSIG_VISION,
  parseOrganizationalSignalCenter,
  type OrganizationalSignalCenter,
} from "@/lib/organizational-signal-center";
...
type PanelLabels = {
  ...
  interpretationTypes: Record<string, string>;
...
const HEALTH_STYLES: Record<string, string> = {
  ...
  signal_awareness_recommended: "border-rose-200 bg-rose-50",
};
...
export function OrganizationalSignalCenterPanel({ labels }: Props) {
  const [center, setCenter] = useState<OrganizationalSignalCenter | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/organizational-signal/center");
    if (res.ok) setCenter(parseOrganizationalSignalCenter(await res.json()));
    setLoading(false);
  }, []);
  ...
  const postAction = async (payload: Record<string, unknown>) => {
    await fetch("/api/organizational-signal/action", {
      method: "POST",
  ...
  const healthStyle = HEALTH_STYLES[dash?.signal_health_label ?? "healthy"] ?? HEALTH_STYLES.healthy;
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      <div className="flex flex-wrap gap-3 text-sm">
        ...
        {center?.links?.organizational_simplicity && (
          <Link href={center.links.organizational_simplicity} className="text-slate-600 hover:underline">
      ...
      <div>
        ...
        <p className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900">
          {labels.corePrinciple}: {OSIG_CORE_PRINCIPLE}
        </p>
        <p className="mt-2 text-sm text-gray-600">
          {labels.philosophyTitle}: {OSIG_PHILOSOPHY}
        </p>
        <p className="mt-1 text-sm text-gray-600">
          {labels.visionTitle}: {OSIG_VISION}
        </p>
      ...
      {dash && (
        <section className={`rounded-2xl border p-5 shadow-sm ${healthStyle}`}>
          ...
          <p className="mt-1 text-sm text-gray-600">
            {labels.healthLabels[dash.signal_health_label] ?? dash.signal_health_label} · {labels.signalScore}:{" "}
            {dash.signal_score}/100 · {labels.emergingThemes}: {dash.emerging_themes_detected}
          </p>
          <dl className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Metric label={labels.metrics.significantTrends} value={`${dash.significant_trends_pct}%`} />
            <Metric label={labels.metrics.executiveAttention} value={`${dash.executive_attention_pct}%`} />
            <Metric label={labels.metrics.observationEffectiveness} value={`${dash.observation_effectiveness_pct}%`} />
            <Metric label={labels.metrics.reflectionParticipation} value={`${dash.reflection_participation_pct}%`} />
            <Metric label={labels.metrics.responseReadiness} value={`${dash.response_readiness_pct}%`} />
            <Metric label={labels.metrics.patternAwareness} value={`${dash.pattern_awareness_pct}%`} />
          ...
          {center?.can_manage && (
            <div className="mt-4 flex flex-wrap gap-2">
              <ActionBtn label={labels.generateReport} onClick={() => void postAction({ action: "generate_signal_report" })} />
              ...
              <ActionBtn
                ...
                variant="muted"
                onClick={() => void postAction({ action: "export_signal_snapshot", period_label: "Current quarter" })}
              />
      ...
      <Section title={labels.signalsTitle} empty={center?.organizational_signals.length === 0} emptyLabel={labels.emptySection}>
        {center?.organizational_signals.map((sig) => (
      ...
      <Section title={labels.interpretationTitle} empty={center?.interpretation_prompts.length === 0} emptyLabel={labels.emptySection}>
        {center?.interpretation_prompts.map((que) => (
          <li key={que.question_key} className="rounded-xl border border-indigo-100 bg-indigo-50/20 p-4 text-sm">
            <p className="text-xs text-gray-500">{labels.interpretationTypes[que.question_type] ?? que.question_type}</p>
      ...
      <Section title={labels.initiativesTitle} empty={center?.signal_initiatives.length === 0} emptyLabel={labels.emptySection}>
        {center?.signal_initiatives.map((ini) => (
      ...
      <Section title={labels.reviewsTitle} empty={center?.signal_reviews.length === 0} emptyLabel={labels.emptySection}>
        {center?.signal_reviews.map((rev) => (
      ...
      <Section title={labels.milestonesTitle} empty={center?.signal_milestones.length === 0} emptyLabel={labels.emptySection}>
        {center?.signal_milestones.map((ms) => (
        ...
        {center?.can_manage && (
          <li className="pt-2">
            <ActionBtn
              ...
              onClick={() =>
                void postAction({
                  action: "archive_signal_milestone",
          ...
          <li key={snap.snapshot_key} className="rounded-xl border border-gray-100 p-3 text-sm">
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              <span className="text-xs text-indigo-700">
                {labels.signalScore}: {snap.signal_score}
              </span>
            </div>
      ...
      {center?.executive_view && (
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            <ExecField label={labels.executiveFields.emergingThemes} value={center.executive_view.emerging_themes} />
            <ExecField label={labels.executiveFields.strategicObservation} value={center.executive_view.strategic_observation} />
            <ExecField label={labels.executiveFields.responseReadiness} value={center.executive_view.response_readiness} />
            <ExecField label={labels.executiveFields.opportunityAwareness} value={center.executive_view.opportunity_awareness} />
          </dl>
        </section>
      )}
      ...
      <Section title={labels.sessionsTitle} empty={center?.signal_sessions.length === 0} emptyLabel={labels.emptySection}>
        {center?.signal_sessions.map((sess) => (

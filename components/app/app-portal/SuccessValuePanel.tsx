...
import {
  ...
  type SuccessValueLevel,
} from "@/lib/app-portal/success-value";

type Props = { labels: SuccessValueLabels };

const CATEGORIES: SuccessCategory[] = [
...
const VALUE_LEVELS: SuccessValueLevel[] = ["emerging_value", "moderate_value", "significant_value", "transformational_value"];

const STATUS_STYLE: Record<SuccessStatus, string> = {
  planned: "bg-slate-100 text-slate-700",
  in_progress: "bg-indigo-100 text-indigo-900",
  measuring: "bg-amber-100 text-amber-950",
  successful: "bg-emerald-100 text-emerald-900",
  partially_successful: "bg-blue-100 text-blue-900",
  did_not_meet_expectations: "bg-rose-100 text-rose-900",
  archived: "bg-slate-100 text-slate-500",
};

export function SuccessValuePanel({ labels }: Props) {
  const [data, setData] = useState<SuccessListResponse | null>(null);
  ...
  const [formCategory, setFormCategory] = useState<SuccessCategory>("operational_value");
  const [formValueLevel, setFormValueLevel] = useState<SuccessValueLevel>("moderate_value");
  ...
  const load = useCallback(async () => {
    ...
    if (valueLevel) params.set("value_level", valueLevel);
    if (reviewFrom) params.set("review_from", reviewFrom);
    if (reviewTo) params.set("review_to", reviewTo);
    if (search.trim()) params.set("search", search.trim());
    const res = await fetch(`/api/aipify/success?${params}`);
    if (res.ok) setData(parseSuccessList(await res.json()));
    setLoading(false);
  }, [category, status, valueLevel, reviewFrom, reviewTo, search]);
  ...
  async function createItem() {
    ...
    const res = await fetch("/api/aipify/success", {
      method: "POST",
      ...
      body: JSON.stringify({
        ...
        value_level: formValueLevel,
        expected_outcomes: formExpected,
        value_hypothesis: formHypothesis,
        measurement_method: formMeasurement,
        notes: formNotes,
      }),
    });
    if (res.ok) {
      ...
      if (body.initiative?.id) {
        window.location.href = `/app/operations/success/${body.initiative.id}`;
        return;
      }
      setShowForm(false);
      void load();
    ...
    <div className="mx-auto max-w-5xl space-y-8">
      ...
      {dash ? (
        <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <Stat label={labels.dashboard.underReview} value={dash.under_review} />
          <Stat label={labels.dashboard.highestImpact} value={dash.highest_impact} />
          <Stat label={labels.dashboard.missingMeasurements} value={dash.missing_measurements} />
        </section>
      ) : null}

      {dash && dash.recently_realized.length > 0 ? (
        <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <p className="text-sm font-medium text-slate-900">{labels.dashboard.recentlyRealized}</p>
          <ul className="mt-2 flex flex-wrap gap-2 text-sm">
            {dash.recently_realized.map((i) => (
              <li key={i.id}>
                <Link href={`/app/operations/success/${i.id}`} className="text-indigo-700 hover:underline">{i.title}</Link>
              </li>
            ))}
          </ul>
        </section>
      ...
      {dash && dash.success_trends.length > 0 ? (
        <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <p className="text-sm font-medium text-slate-900">{labels.dashboard.successTrends}</p>
          <ul className="mt-2 flex flex-wrap gap-2 text-sm text-slate-600">
            {dash.success_trends.map((i) => (
      ...
      <section className="flex flex-wrap gap-3">
        ...
        <select value={valueLevel} onChange={(e) => setValueLevel(e.target.value)} className="rounded-lg border border-slate-200 px-3 py-2 text-sm">
          <option value="">{labels.filters.valueLevel}</option>
          {VALUE_LEVELS.map((v) => <option key={v} value={v}>{labels.valueLevels[v]}</option>)}
        </select>
        <input type="date" value={reviewFrom} onChange={(e) => setReviewFrom(e.target.value)} aria-label={labels.filters.reviewFrom} className="rounded-lg border border-slate-200 px-3 py-2 text-sm" />
        <input type="date" value={reviewTo} onChange={(e) => setReviewTo(e.target.value)} aria-label={labels.filters.reviewTo} className="rounded-lg border border-slate-200 px-3 py-2 text-sm" />
        {data?.can_manage ? (
      ...
      {showForm ? (
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm space-y-4">
          ...
          <select value={formCategory} onChange={(e) => setFormCategory(e.target.value as SuccessCategory)} className="rounded-lg border border-slate-200 px-3 py-2 text-sm">
            {CATEGORIES.map((c) => <option key={c} value={c}>{labels.categories[c]}</option>)}
          </select>
          <select value={formValueLevel} onChange={(e) => setFormValueLevel(e.target.value as SuccessValueLevel)} className="rounded-lg border border-slate-200 px-3 py-2 text-sm">
            {VALUE_LEVELS.map((v) => <option key={v} value={v}>{labels.valueLevels[v]}</option>)}
          </select>
          <textarea value={formExpected} onChange={(e) => setFormExpected(e.target.value)} placeholder={labels.form.expectedOutcomes} rows={2} className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm" />
          <textarea value={formHypothesis} onChange={(e) => setFormHypothesis(e.target.value)} placeholder={labels.form.valueHypothesis} rows={2} className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm" />
          <input value={formMeasurement} onChange={(e) => setFormMeasurement(e.target.value)} placeholder={labels.form.measurementMethod} className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm" />
          ...
          {data?.items.map((item) => (
            <li key={item.id} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <Link href={`/app/operations/success/${item.id}`} className="font-medium text-slate-900 hover:text-indigo-700">{item.title}</Link>
              ...
              <div className="mt-3 flex flex-wrap gap-4 text-xs text-slate-600">
                ...
                <span>{labels.card.valueLevel}: {labels.valueLevels[item.value_level]}</span>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <dl className="mt-4 space-y-3 text-sm">
          ...
          <div><dt className="font-medium">{labels.faq.whyMeasure}</dt><dd className="mt-1 text-slate-600">{labels.faq.whyMeasureAnswer}</dd></div>
          <div><dt className="font-medium">{labels.faq.autoDetermine}</dt><dd className="mt-1 text-slate-600">{labels.faq.autoDetermineAnswer}</dd></div>
        </dl>
      </section>
    </div>
  );

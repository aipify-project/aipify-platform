...
type Props = { labels: PlaybooksLabels };
...
const STATUSES: PlaybookStatus[] = ["draft", "active", "under_review", "archived"];
...
export function PlaybooksPanel({ labels }: Props) {
  const [data, setData] = useState<PlaybookListResponse | null>(null);
  ...
  const [formCategory, setFormCategory] = useState<PlaybookCategory>("custom");
  const [formNotes, setFormNotes] = useState("");
  const [formStepTitle, setFormStepTitle] = useState("");
  const [formStepDescription, setFormStepDescription] = useState("");
  ...
  const load = useCallback(async () => {
    ...
    const res = await fetch(`/api/aipify/playbooks?${params}`);
    if (res.ok) setData(parsePlaybookList(await res.json()));
    setLoading(false);
  }, [category, status, recentlyUpdated, search]);
  ...
  async function createItem() {
    ...
    const steps = formStepTitle.trim()
      ? [{ title: formStepTitle, description: formStepDescription, responsible_role: "", requires_approval: false }]
      : [];
    const res = await fetch("/api/aipify/playbooks", {
      method: "POST",
    ...
    if (res.ok) {
      const body = (await res.json()) as { playbook?: { id?: string } };
      if (body.playbook?.id) {
        window.location.href = `/app/operations/playbooks/${body.playbook.id}`;
        return;
      }
      setShowForm(false);
      void load();
    ...
    <div className="mx-auto max-w-5xl space-y-8">
      ...
      {dash ? (
        <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <Stat label={labels.dashboard.draft} value={dash.draft} />
          <Stat label={labels.dashboard.archived} value={dash.archived} />
        </section>
      ) : null}
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <ul className="mt-2 flex flex-wrap gap-2 text-sm">
            {dash.recently_updated.map((p) => (
              <li key={p.id}>
                <Link href={`/app/operations/playbooks/${p.id}`} className="text-indigo-700 hover:underline">{p.title}</Link>
              </li>
            ))}
          </ul>
        </section>
      ...
      {dash && dash.most_accessed.length > 0 ? (
        <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <p className="text-sm font-medium text-slate-900">{labels.dashboard.mostAccessed}</p>
          <ul className="mt-2 flex flex-wrap gap-2 text-sm">
            {dash.most_accessed.map((p) => (
              <li key={p.id}>
                <Link href={`/app/operations/playbooks/${p.id}`} className="text-indigo-700 hover:underline">{p.title}</Link>
              </li>
            ))}
      ...
      {showForm ? (
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm space-y-4">
          ...
          <select value={formCategory} onChange={(e) => setFormCategory(e.target.value as PlaybookCategory)} className="rounded-lg border border-slate-200 px-3 py-2 text-sm">
            {CATEGORIES.map((c) => <option key={c} value={c}>{labels.categories[c]}</option>)}
          </select>
          <input value={formStepTitle} onChange={(e) => setFormStepTitle(e.target.value)} placeholder={labels.form.stepTitle} className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm" />
          <textarea value={formStepDescription} onChange={(e) => setFormStepDescription(e.target.value)} placeholder={labels.form.stepDescription} rows={2} className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm" />
          ...
          {data?.items.map((item) => (
            <li key={item.id} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <Link href={`/app/operations/playbooks/${item.id}`} className="font-medium text-slate-900 hover:text-indigo-700">{item.title}</Link>
              ...
              <div className="mt-3 flex flex-wrap gap-4 text-xs text-slate-600">
                ...
                {item.last_reviewed_date ? <span>{labels.card.lastReviewed}: {item.last_reviewed_date}</span> : null}
              </div>
            </li>
          ))}
        </ul>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <dl className="mt-4 space-y-3 text-sm">
          ...
          <div><dt className="font-medium">{labels.faq.versions}</dt><dd className="mt-1 text-slate-600">{labels.faq.versionsAnswer}</dd></div>
          <div><dt className="font-medium">{labels.faq.autoCreate}</dt><dd className="mt-1 text-slate-600">{labels.faq.autoCreateAnswer}</dd></div>
        </dl>
      </section>
    </div>
  );

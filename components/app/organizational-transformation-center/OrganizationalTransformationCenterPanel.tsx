...
import {
  ...
  OTRC_PHILOSOPHY,
  OTRC_VISION,
  parseOrganizationalTransformationCenter,
  type OrganizationalTransformationCenter,
} from "@/lib/organizational-transformation-center";
...
const READINESS_STYLES: Record<string, string> = {
  highly_ready: "border-emerald-200 bg-emerald-50",
  prepared: "border-teal-200 bg-teal-50",
  developing: "border-slate-200 bg-slate-50",
  limited_readiness: "border-amber-200 bg-amber-50",
  transformation_risk: "border-rose-200 bg-rose-50",
};
...
export function OrganizationalTransformationCenterPanel({ labels }: Props) {
  const [center, setCenter] = useState<OrganizationalTransformationCenter | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/organizational-transformation/center");
    if (res.ok) setCenter(parseOrganizationalTransformationCenter(await res.json()));
    setLoading(false);
  }, []);
  ...
  const postAction = async (payload: Record<string, unknown>) => {
    await fetch("/api/organizational-transformation/action", {
      method: "POST",
  ...
  const readinessStyle =
    READINESS_STYLES[dash?.transformation_readiness_label ?? "developing"] ?? READINESS_STYLES.developing;
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      <div className="flex flex-wrap gap-3 text-sm">
        ...
        {center?.links?.organizational_sustainability && (
          <Link href={center.links.organizational_sustainability} className="text-slate-600 hover:underline">
            {labels.organizationalSustainabilityLink}
          </Link>
        )}
        {center?.links?.organizational_renewal && (
          <Link href={center.links.organizational_renewal} className="text-slate-600 hover:underline">
      ...
      <div>
        ...
        <p className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900">
          {labels.corePrinciple}: {OTRC_CORE_PRINCIPLE}
        </p>
        <p className="mt-2 text-sm text-gray-600">
          {labels.philosophyTitle}: {OTRC_PHILOSOPHY}
        </p>
        <p className="mt-1 text-sm text-gray-600">
          {labels.visionTitle}: {OTRC_VISION}
        </p>
        <p className="mt-3 text-sm font-medium text-indigo-900">{labels.humansDecide}</p>
      </div>

      {dash && (
        <section className={`rounded-2xl border p-5 shadow-sm ${readinessStyle}`}>
          <h2 className="text-lg font-semibold text-gray-900">{labels.dashboardTitle}</h2>
          <p className="mt-1 text-sm text-gray-600">
            {labels.readinessLabels[dash.transformation_readiness_label] ?? dash.transformation_readiness_label} ·{" "}
            {labels.readinessScore}: {dash.transformation_readiness_score}/100 · {labels.adoptionMomentum}:{" "}
            {dash.adoption_momentum_pct}%
          </p>
          <dl className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Metric label={labels.metrics.leadership} value={`${dash.leadership_commitment_pct}%`} />
            <Metric label={labels.metrics.workforce} value={`${dash.workforce_preparedness_pct}%`} />
            <Metric label={labels.metrics.capability} value={`${dash.capability_maturity_pct}%`} />
            <Metric label={labels.metrics.communication} value={`${dash.communication_effectiveness_pct}%`} />
            <Metric label={labels.metrics.governance} value={`${dash.governance_readiness_pct}%`} />
            <Metric label={labels.metrics.risks} value={dash.risks_identified} />
          ...
          {center?.can_manage && (
            <div className="mt-4 flex flex-wrap gap-2">
              <ActionBtn
                label={labels.generateReport}
                onClick={() => void postAction({ action: "generate_transformation_report" })}
              />
              <ActionBtn
                label={labels.printSummary}
                variant="muted"
                onClick={() => void postAction({ action: "print_executive_summary" })}
              />
              <ActionBtn
                label={labels.exportSnapshot}
                variant="muted"
                onClick={() =>
                  void postAction({ action: "export_transformation_snapshot", period_label: "Current quarter" })
                }
              />
              <ActionBtn
                label={labels.coordinateReview}
                variant="muted"
                onClick={() => void postAction({ action: "coordinate_stakeholder_review" })}
              />
            </div>
          )}
        </section>
      ...
      {center?.transformation_workflow.length ? (
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          <h2 className="text-lg font-semibold text-gray-900">{labels.workflowTitle}</h2>
          <ol className="mt-4 space-y-2">
            {center.transformation_workflow
              .slice()
              .sort((a, b) => a.order - b.order)
              .map((stage) => (
                <li key={stage.stage_key} className="flex items-start gap-3 text-sm">
                  <span className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-indigo-100 text-xs font-medium text-indigo-800">
                    {stage.order}
                  </span>
                  <span className="text-gray-800">{stage.label}</span>
                </li>
              ))}
          </ol>
        </section>
      ...
      <Section title={labels.signalsTitle} empty={center?.transformation_signals.length === 0} emptyLabel={labels.emptySection}>
        {center?.transformation_signals.map((sig) => (
          <li
            key={sig.signal_key}
            className={`rounded-xl border p-4 text-sm ${TONE_STYLES[sig.signal_tone] ?? TONE_STYLES.neutral}`}
          >
      ...
      <Section title={labels.adoptionTitle} empty={center?.adoption_prompts.length === 0} emptyLabel={labels.emptySection}>
        {center?.adoption_prompts.map((adp) => (
          <li key={adp.adoption_key} className="rounded-xl border border-indigo-100 bg-indigo-50/20 p-4 text-sm">
            <p className="text-xs text-gray-500">{labels.adoptionTypes[adp.adoption_type] ?? adp.adoption_type}</p>
            <p className="mt-1 font-medium text-gray-900">{adp.title}</p>
            <p className="mt-2 text-gray-700">{adp.summary}</p>
          </li>
        ))}
      </Section>
      ...
      <Section
        ...
        empty={center?.transformation_initiatives.length === 0}
        emptyLabel={labels.emptySection}
      >
        {center?.transformation_initiatives.map((ini) => (
          <li key={ini.initiative_key} className="rounded-xl border border-gray-100 p-4 text-sm">
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              <span className="text-xs font-medium text-indigo-700">
                {labels.initiativeStatuses[ini.status] ?? ini.status}
              </span>
            </div>
            ...
            {ini.status !== "completed" && center?.can_manage && (
              <div className="mt-3">
                <ActionBtn
                  label={labels.completeInitiative}
                  onClick={() => void postAction({ action: "complete_initiative", initiative_key: ini.initiative_key })}
                />
              </div>
            )}
          </li>
      ...
      <Section title={labels.reviewsTitle} empty={center?.transformation_reviews.length === 0} emptyLabel={labels.emptySection}>
        {center?.transformation_reviews.map((rev) => (
          <li key={rev.review_key} className="rounded-xl border border-gray-100 p-4 text-sm">
            ...
            {rev.status !== "completed" && center?.can_manage && (
              <div className="mt-3 flex flex-wrap gap-2">
                <ActionBtn
                  label={labels.completeReview}
                  onClick={() => void postAction({ action: "complete_review", review_key: rev.review_key })}
                />
                <ActionBtn
                  label={labels.scheduleReflection}
                  variant="muted"
                  onClick={() => void postAction({ action: "schedule_reflection_session", review_key: rev.review_key })}
                />
              </div>
            )}
          </li>
      ...
      <Section
        title={labels.milestonesTitle}
        empty={center?.transformation_milestones.length === 0}
        emptyLabel={labels.emptySection}
      >
        {center?.transformation_milestones.map((ms) => (
        ...
        {center?.can_manage && (
          <li className="pt-2">
            <ActionBtn
              ...
              onClick={() =>
                void postAction({
                  action: "archive_transformation_milestone",
                  title: "Transformation milestone",
                  summary: "Transformation milestone archived via Transformation Center.",
                })
              }
            />
          </li>
          ...
          <li key={snap.snapshot_key} className="rounded-xl border border-gray-100 p-3 text-sm">
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              <span className="text-xs text-indigo-700">
                {labels.readinessScore}: {snap.transformation_readiness_score}
              </span>
            </div>
      ...
      {center?.executive_view && (
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            ...
            <ExecField
              label={labels.executiveFields.leadership}
              value={center.executive_view.leadership_participation}
            />
            <ExecField label={labels.executiveFields.adoption} value={center.executive_view.adoption_confidence} />
            <ExecField
              label={labels.executiveFields.opportunities}
              value={center.executive_view.transformation_opportunities}
            />
          </dl>
        </section>
      )}
      ...
      <InsightSection
        ...
        items={center?.insights ?? []}
        canManage={center?.can_manage ?? false}
        dismissLabel={labels.dismiss}
        onDismiss={(key) => void postAction({ action: "dismiss_insight", insight_key: key })}
      />
      <RecommendationSection
        ...
        dismissLabel={labels.dismiss}
        onAccept={(key) => void postAction({ action: "accept_recommendation", recommendation_key: key })}
        onDismiss={(key) => void postAction({ action: "dismiss_recommendation", recommendation_key: key })}
      />

      <Section title={labels.sessionsTitle} empty={center?.transformation_sessions.length === 0} emptyLabel={labels.emptySection}>
        {center?.transformation_sessions.map((sess) => (
          <li key={sess.session_key} className="rounded-xl border border-gray-100 p-4 text-sm">
            ...
            {sess.status !== "completed" && center?.can_manage && (
              <div className="mt-3 flex flex-wrap gap-2">
                <ActionBtn
                  label={labels.completeSession}
                  onClick={() => void postAction({ action: "complete_session", session_key: sess.session_key })}
                />
                <ActionBtn
                  label={labels.scheduleWorkshop}
                  variant="muted"
                  onClick={() => void postAction({ action: "schedule_leadership_workshop", session_key: sess.session_key })}
                />
              </div>
            )}
          </li>
...
function InsightSection({
  ...
  title: string;
  items: { insight_key: string; message: string }[];
  canManage: boolean;
  dismissLabel: string;
...
function RecommendationSection({
  ...
  title: string;
  items: { recommendation_key: string; message: string }[];
  canManage: boolean;
  acceptLabel: string;
  dismissLabel: string;

...
import { parseContinuousImprovementEngineDashboard, type ContinuousImprovementEngineDashboard } from "@/lib/aipify/continuous-improvement-engine";
...
export function ContinuousImprovementEngineDashboardPanel({ labels }: Props) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/aipify/continuous-improvement-engine/dashboard");
    if (res.ok) setDashboard(parseContinuousImprovementEngineDashboard(await res.json()));
    setLoading(false);
  }, []);

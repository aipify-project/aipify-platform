...
import {
  ...
  OIMC_PHILOSOPHY,
  OIMC_VISION,
  parseOrganizationalImpactCenter,
  type OrganizationalImpactCenter,
} from "@/lib/organizational-impact-center";
...
type PanelLabels = {
  ...
  indicatorTypes: Record<string, string>;
  indicatorTones: Record<string, string>;
...
const HEALTH_STYLES: Record<string, string> = {
  ...
  meaningful: "border-slate-200 bg-slate-50",
  developing: "border-amber-200 bg-amber-50",
  reflection_recommended: "border-rose-200 bg-rose-50",
};
...
export function OrganizationalImpactCenterPanel({ labels }: Props) {
  const [center, setCenter] = useState<OrganizationalImpactCenter | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/organizational-impact/center");
    if (res.ok) setCenter(parseOrganizationalImpactCenter(await res.json()));
    setLoading(false);
  }, []);
  ...
  const postAction = async (payload: Record<string, unknown>) => {
    await fetch("/api/organizational-impact/action", {
      method: "POST",
  ...
  const healthStyle = HEALTH_STYLES[dash?.impact_health_label ?? "meaningful"] ?? HEALTH_STYLES.meaningful;
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      <div className="flex flex-wrap gap-3 text-sm">
        ...
        {center?.links?.organizational_excellence && <Link href={center.links.organizational_excellence} className="text-slate-600 hover:underline">{labels.organizationalExcellenceLink}</Link>}
      ...
      <div>
        ...
        <p className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900">{labels.corePrinciple}: {OIMC_CORE_PRINCIPLE}</p>
        <p className="mt-2 text-sm text-gray-600">{labels.philosophyTitle}: {OIMC_PHILOSOPHY}</p>
        <p className="mt-1 text-sm text-gray-600">{labels.visionTitle}: {OIMC_VISION}</p>
      ...
      {dash && (
        <section className={`rounded-2xl border p-5 shadow-sm ${healthStyle}`}>
          ...
          <p className="mt-1 text-sm text-gray-600">
            {labels.healthLabels[dash.impact_health_label] ?? dash.impact_health_label} · {labels.impactProfile}: {dash.impact_profile_score}/100 · {labels.positiveTrend}: {dash.positive_outcome_trend_pct}%
          </p>
          <dl className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Metric label={labels.metrics.customer} value={`${dash.customer_outcome_pct}%`} />
            <Metric label={labels.metrics.employee} value={`${dash.employee_development_pct}%`} />
            <Metric label={labels.metrics.community} value={`${dash.community_engagement_pct}%`} />
            <Metric label={labels.metrics.purpose} value={`${dash.purpose_alignment_pct}%`} />
            <Metric label={labels.metrics.mission} value={`${dash.mission_fulfillment_pct}%`} />
            <Metric label={labels.metrics.initiatives} value={dash.initiatives_in_progress} />
            <Metric label={labels.metrics.stakeholders} value={dash.stakeholder_summaries} />
            <Metric label={labels.metrics.confidence} value={`${dash.executive_confidence}/5`} />
          </dl>
          {center?.can_manage && (
            <div className="mt-4 flex flex-wrap gap-2">
              <ActionBtn label={labels.generateReport} onClick={() => void postAction({ action: "generate_impact_report" })} />
              <ActionBtn label={labels.printSummary} variant="muted" onClick={() => void postAction({ action: "print_stakeholder_summary" })} />
              <ActionBtn label={labels.exportSnapshot} variant="muted" onClick={() => void postAction({ action: "export_impact_snapshot", period_label: "Current quarter" })} />
      ...
      <Section title={labels.indicatorsTitle} empty={center?.impact_indicators.length === 0} emptyLabel={labels.emptySection}>
        {center?.impact_indicators.map((ind) => (
          <li key={ind.indicator_key} className={`rounded-xl border p-4 text-sm ${TONE_STYLES[ind.indicator_tone] ?? TONE_STYLES.neutral}`}>
            <p className="text-xs text-gray-500">{labels.domains[ind.domain] ?? ind.domain} · {labels.indicatorTypes[ind.indicator_type] ?? ind.indicator_type}</p>
            <p className="mt-1 font-medium text-gray-900">{ind.title}</p>
            <p className="mt-2 text-gray-700">{ind.summary}</p>
          </li>
        ))}
      </Section>

      <Section title={labels.initiativesTitle} empty={center?.impact_initiatives.length === 0} emptyLabel={labels.emptySection}>
        {center?.impact_initiatives.map((ini) => (
          <li key={ini.initiative_key} className="rounded-xl border border-gray-100 p-4 text-sm">
            ...
            {ini.status !== "completed" && ini.status !== "documented" && center?.can_manage && (
              <div className="mt-3 flex flex-wrap gap-2">
                <ActionBtn label={labels.documentInitiative} onClick={() => void postAction({ action: "document_initiative", initiative_key: ini.initiative_key })} />
                <ActionBtn label={labels.completeInitiative} variant="muted" onClick={() => void postAction({ action: "complete_initiative", initiative_key: ini.initiative_key })} />
              </div>
            )}
          </li>
        ))}
      ...
      <Section title={labels.reviewsTitle} empty={center?.impact_reviews.length === 0} emptyLabel={labels.emptySection}>
        {center?.impact_reviews.map((rev) => (
      ...
      <Section title={labels.milestonesTitle} empty={center?.impact_milestones.length === 0} emptyLabel={labels.emptySection}>
        {center?.impact_milestones.map((ms) => (
        ...
        {center?.can_manage && (
          <li className="pt-2">
            <ActionBtn label={labels.archiveMilestone} onClick={() => void postAction({ action: "archive_impact_milestone", title: "Impact milestone", summary: "Impact milestone archived via Impact Center." })} />
          </li>
        )}
      </Section>
        ...
        {center?.snapshots.map((snap) => (
          <li key={snap.snapshot_key} className="rounded-xl border border-gray-100 p-3 text-sm">
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              <span className="text-xs text-indigo-700">{labels.impactProfile}: {snap.impact_profile_score}</span>
            </div>
      ...
      {center?.executive_view && (
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            <ExecField label={labels.executiveFields.stakeholders} value={center.executive_view.stakeholder_indicators} />
            <ExecField label={labels.executiveFields.mission} value={center.executive_view.mission_fulfillment} />
            <ExecField label={labels.executiveFields.opportunities} value={center.executive_view.contribution_opportunities} />
            <ExecField label={labels.executiveFields.influence} value={center.executive_view.organizational_influence} />
          </dl>
        </section>
      )}
      ...
      <Section title={labels.sessionsTitle} empty={center?.impact_sessions.length === 0} emptyLabel={labels.emptySection}>
        {center?.impact_sessions.map((sess) => (

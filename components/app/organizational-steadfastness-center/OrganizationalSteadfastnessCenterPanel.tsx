...
import {
  ...
  OSFC_PHILOSOPHY,
  OSFC_VISION,
  parseOrganizationalSteadfastnessCenter,
  type OrganizationalSteadfastnessCenter,
} from "@/lib/organizational-steadfastness-center";
...
export function OrganizationalSteadfastnessCenterPanel({ labels }: Props) {
  const [center, setCenter] = useState<OrganizationalSteadfastnessCenter | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/organizational-steadfastness/center");
    if (res.ok) setCenter(parseOrganizationalSteadfastnessCenter(await res.json()));
    setLoading(false);
  }, []);
  ...
  const postAction = async (payload: Record<string, unknown>) => {
    await fetch("/api/organizational-steadfastness/action", {
      method: "POST",
  ...
  const healthStyle = HEALTH_STYLES[dash?.steadfastness_health_label ?? "stable"] ?? HEALTH_STYLES.stable;
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      <div className="flex flex-wrap gap-3 text-sm">
        ...
        {center?.links?.organizational_compounding && (
          <Link href={center.links.organizational_compounding} className="text-slate-600 hover:underline">
      ...
      <div>
        ...
        <p className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900">
          {labels.corePrinciple}: {OSFC_CORE_PRINCIPLE}
        </p>
        <p className="mt-2 text-sm text-gray-600">
          {labels.philosophyTitle}: {OSFC_PHILOSOPHY}
        </p>
        <p className="mt-1 text-sm text-gray-600">
          {labels.visionTitle}: {OSFC_VISION}
        </p>
      ...
      {dash && (
        <section className={`rounded-2xl border p-5 shadow-sm ${healthStyle}`}>
          ...
          <p className="mt-1 text-sm text-gray-600">
            {labels.healthLabels[dash.steadfastness_health_label] ?? dash.steadfastness_health_label} ·{" "}
            {labels.steadfastnessScore}: {dash.steadfastness_score}/100 · {labels.resilienceIndicators}:{" "}
            {dash.resilience_indicators_pct}%
          </p>
          <dl className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Metric label={labels.metrics.values} value={`${dash.values_consistency_pct}%`} />
            <Metric label={labels.metrics.leadershipReliability} value={`${dash.leadership_reliability_pct}%`} />
            <Metric label={labels.metrics.recovery} value={`${dash.recovery_effectiveness_pct}%`} />
            <Metric label={labels.metrics.strategic} value={`${dash.strategic_persistence_pct}%`} />
            <Metric label={labels.metrics.resilience} value={`${dash.organizational_resilience_pct}%`} />
            <Metric label={labels.metrics.commitment} value={`${dash.commitment_consistency_pct}%`} />
          ...
          {center?.can_manage && (
            <div className="mt-4 flex flex-wrap gap-2">
              <ActionBtn label={labels.generateReport} onClick={() => void postAction({ action: "generate_steadfastness_report" })} />
              ...
              <ActionBtn
                ...
                variant="muted"
                onClick={() => void postAction({ action: "export_resilience_snapshot", period_label: "Current quarter" })}
              />
      ...
      <Section title={labels.signalsTitle} empty={center?.steadfastness_signals.length === 0} emptyLabel={labels.emptySection}>
        {center?.steadfastness_signals.map((sig) => (
      ...
      <Section title={labels.persistenceTitle} empty={center?.persistence_prompts.length === 0} emptyLabel={labels.emptySection}>
        {center?.persistence_prompts.map((per) => (
          <li key={per.persistence_key} className="rounded-xl border border-indigo-100 bg-indigo-50/20 p-4 text-sm">
            <p className="text-xs text-gray-500">{labels.persistenceTypes[per.persistence_type] ?? per.persistence_type}</p>
      ...
      <Section title={labels.initiativesTitle} empty={center?.steadfastness_initiatives.length === 0} emptyLabel={labels.emptySection}>
        {center?.steadfastness_initiatives.map((ini) => (
      ...
      <Section title={labels.reviewsTitle} empty={center?.steadfastness_reviews.length === 0} emptyLabel={labels.emptySection}>
        {center?.steadfastness_reviews.map((rev) => (
      ...
      <Section title={labels.milestonesTitle} empty={center?.steadfastness_milestones.length === 0} emptyLabel={labels.emptySection}>
        {center?.steadfastness_milestones.map((ms) => (
        ...
        {center?.can_manage && (
          <li className="pt-2">
            <ActionBtn
              ...
              onClick={() =>
                void postAction({
                  action: "archive_steadfastness_milestone",
                  title: "Steadfastness milestone",
                  summary: "Steadfastness milestone archived via Steadfastness Center.",
                })
              }
            />
          </li>
          ...
          <li key={snap.snapshot_key} className="rounded-xl border border-gray-100 p-3 text-sm">
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              <span className="text-xs text-indigo-700">
                {labels.steadfastnessScore}: {snap.steadfastness_score}
              </span>
            </div>
      ...
      {center?.executive_view && (
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            ...
            <ExecField label={labels.executiveFields.strategicResilience} value={center.executive_view.strategic_resilience} />
            <ExecField label={labels.executiveFields.valuesContinuity} value={center.executive_view.values_continuity} />
            <ExecField label={labels.executiveFields.confidence} value={center.executive_view.confidence_opportunities} />
          </dl>
        </section>
      )}
      ...
      <Section title={labels.sessionsTitle} empty={center?.steadfastness_sessions.length === 0} emptyLabel={labels.emptySection}>
        {center?.steadfastness_sessions.map((sess) => (

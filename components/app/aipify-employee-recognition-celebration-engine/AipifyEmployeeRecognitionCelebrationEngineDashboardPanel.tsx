...
import {
  parseAipifyEmployeeRecognitionCelebrationEngineDashboard,
  type AbosSuccessCriterion,
  type BlueprintObjective,
  type ExecutiveReview,
  type AipifyEmployeeRecognitionCelebrationEngineDashboard,
  type IntegrationLink,
  type ReflectionEntry,
  type ScaffoldNote,
} from "@/lib/aipify/aipify-employee-recognition-celebration-engine";
...
export function AipifyEmployeeRecognitionCelebrationEngineDashboardPanel({ labels }: Props) {
  const [dashboard, setDashboard] = useState<AipifyEmployeeRecognitionCelebrationEngineDashboard | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/aipify/aipify-employee-recognition-celebration-engine/dashboard");
    if (res.ok) setDashboard(parseAipifyEmployeeRecognitionCelebrationEngineDashboard(await res.json()));
    setLoading(false);
  }, []);
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-xl border border-indigo-200 bg-indigo-50/50 p-6">
        ...
        <p className="mt-2 text-2xl font-bold text-indigo-900">{dashboard.aipify_employee_recognition_celebration_score ?? 0}</p>
        <p className="text-xs text-indigo-700">{labels.scoreLabel}</p>
        {dashboard.aipify_employee_recognition_celebration_mission ? <p className="mt-2 text-sm font-medium text-indigo-900">{dashboard.aipify_employee_recognition_celebration_mission}</p> : null}
      ...
      {dashboard.aipify_employee_recognition_celebration_objectives?.length ? (
        <section className="space-y-3">
          <h2 className="text-lg font-semibold text-gray-900">{labels.blueprintObjectives}</h2>
          <div className="grid gap-2 sm:grid-cols-2">
            {dashboard.aipify_employee_recognition_celebration_objectives.map((o) => <ObjectiveCard key={o.key ?? o.label} objective={o} />)}
          </div>
        </section>
      ) : null}
      ...
      {dashboard.aipify_employee_recognition_celebration_success_criteria?.length ? (
        <section className="space-y-2">
          <h2 className="text-lg font-semibold text-gray-900">{labels.successCriteria}</h2>
          {dashboard.aipify_employee_recognition_celebration_success_criteria.map((c) => (
            <SuccessCriterionRow key={c.key ?? c.label} criterion={c} metLabel={labels.criterionMet} pendingLabel={labels.criterionPending} />
          ))}
        </section>
      ) : null}

      {dashboard.aipify_employee_recognition_celebration_privacy_note ? <p className="text-xs text-gray-500">{dashboard.aipify_employee_recognition_celebration_privacy_note}</p> : null}
    </div>
  );
}

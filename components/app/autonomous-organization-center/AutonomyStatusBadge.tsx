...
export function AutonomyStatusBadge({
  statusKey,
  labels,
}: {
  statusKey: string;
  labels: AutonomousOrganizationCenterLabels["status"];
}) {
  const def = getOperationsStatusDefinition(statusKey);
  const label = labels[def.labelKey as keyof AutonomousOrganizationCenterLabels["status"]] ?? def.labelKey;
  return (

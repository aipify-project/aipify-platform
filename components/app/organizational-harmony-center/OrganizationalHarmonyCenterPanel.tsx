...
import {
  ...
  OHC_PHILOSOPHY,
  OHC_VISION,
  parseOrganizationalHarmonyCenter,
  type OrganizationalHarmonyCenter,
} from "@/lib/organizational-harmony-center";
...
type PanelLabels = {
  ...
  dialogueTypes: Record<string, string>;
...
const HEALTH_STYLES: Record<string, string> = {
  ...
  harmony_improvement_recommended: "border-rose-200 bg-rose-50",
};
...
export function OrganizationalHarmonyCenterPanel({ labels }: Props) {
  const [center, setCenter] = useState<OrganizationalHarmonyCenter | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/organizational-harmony/center");
    if (res.ok) setCenter(parseOrganizationalHarmonyCenter(await res.json()));
    setLoading(false);
  }, []);
  ...
  const postAction = async (payload: Record<string, unknown>) => {
    await fetch("/api/organizational-harmony/action", {
      method: "POST",
  ...
  const healthStyle = HEALTH_STYLES[dash?.harmony_health_label ?? "healthy"] ?? HEALTH_STYLES.healthy;
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      <div className="flex flex-wrap gap-3 text-sm">
        ...
        {center?.links?.organizational_awareness && (
          <Link href={center.links.organizational_awareness} className="text-slate-600 hover:underline">
      ...
      <div>
        ...
        <p className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900">
          {labels.corePrinciple}: {OHC_CORE_PRINCIPLE}
        </p>
        <p className="mt-2 text-sm text-gray-600">
          {labels.philosophyTitle}: {OHC_PHILOSOPHY}
        </p>
        <p className="mt-1 text-sm text-gray-600">
          {labels.visionTitle}: {OHC_VISION}
        </p>
      ...
      {dash && (
        <section className={`rounded-2xl border p-5 shadow-sm ${healthStyle}`}>
          ...
          <p className="mt-1 text-sm text-gray-600">
            {labels.healthLabels[dash.harmony_health_label] ?? dash.harmony_health_label} ·{" "}
            {labels.harmonyScore}: {dash.harmony_score}/100 · {labels.collaborationIndicators}:{" "}
            {dash.collaboration_indicators_pct}%
          </p>
          <dl className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Metric label={labels.metrics.collaborationEffectiveness} value={`${dash.collaboration_effectiveness_pct}%`} />
            <Metric label={labels.metrics.leadershipConsistency} value={`${dash.leadership_consistency_pct}%`} />
            <Metric label={labels.metrics.crossFunctionalCooperation} value={`${dash.cross_functional_cooperation_pct}%`} />
            <Metric label={labels.metrics.communicationQuality} value={`${dash.communication_quality_pct}%`} />
            <Metric label={labels.metrics.sharedOwnership} value={`${dash.shared_ownership_pct}%`} />
            <Metric label={labels.metrics.crossFunctionalAlignment} value={`${dash.cross_functional_alignment_pct}%`} />
          ...
          {center?.can_manage && (
            <div className="mt-4 flex flex-wrap gap-2">
              <ActionBtn label={labels.generateReport} onClick={() => void postAction({ action: "generate_harmony_report" })} />
              ...
              <ActionBtn
                ...
                variant="muted"
                onClick={() => void postAction({ action: "export_harmony_snapshot", period_label: "Current quarter" })}
              />
      ...
      <Section title={labels.signalsTitle} empty={center?.harmony_signals.length === 0} emptyLabel={labels.emptySection}>
        {center?.harmony_signals.map((sig) => (
      ...
      <Section title={labels.dialogueTitle} empty={center?.dialogue_prompts.length === 0} emptyLabel={labels.emptySection}>
        {center?.dialogue_prompts.map((per) => (
          <li key={per.dialogue_key} className="rounded-xl border border-indigo-100 bg-indigo-50/20 p-4 text-sm">
            <p className="text-xs text-gray-500">{labels.dialogueTypes[per.dialogue_type] ?? per.dialogue_type}</p>
      ...
      <Section title={labels.initiativesTitle} empty={center?.harmony_initiatives.length === 0} emptyLabel={labels.emptySection}>
        {center?.harmony_initiatives.map((ini) => (
      ...
      <Section title={labels.reviewsTitle} empty={center?.harmony_reviews.length === 0} emptyLabel={labels.emptySection}>
        {center?.harmony_reviews.map((rev) => (
          <li key={rev.review_key} className="rounded-xl border border-gray-100 p-4 text-sm">
            ...
            {rev.status !== "completed" && center?.can_manage && (
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <ActionBtn label={labels.scheduleWorkshop} variant="muted" onClick={() => void postAction({ action: "schedule_collaboration_workshop", review_key: rev.review_key })} />
              </div>
            )}
          </li>
        ))}
      ...
      <Section title={labels.milestonesTitle} empty={center?.harmony_milestones.length === 0} emptyLabel={labels.emptySection}>
        {center?.harmony_milestones.map((ms) => (
        ...
        {center?.can_manage && (
          <li className="pt-2">
            <ActionBtn
              ...
              onClick={() =>
                void postAction({
                  action: "archive_harmony_milestone",
                  title: "Harmony milestone",
                  summary: "Harmony milestone archived via Harmony Center.",
                })
              }
            />
          </li>
          ...
          <li key={snap.snapshot_key} className="rounded-xl border border-gray-100 p-3 text-sm">
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              <span className="text-xs text-indigo-700">
                {labels.harmonyScore}: {snap.harmony_score}
              </span>
            </div>
      ...
      {center?.executive_view && (
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            <ExecField label={labels.executiveFields.leadershipAlignment} value={center.executive_view.leadership_alignment} />
            ...
            <ExecField label={labels.executiveFields.crossFunctionalRelationships} value={center.executive_view.cross_functional_relationships} />
            <ExecField label={labels.executiveFields.cohesionOpportunities} value={center.executive_view.cohesion_opportunities} />
          </dl>
        </section>
      )}
      ...
      <Section title={labels.sessionsTitle} empty={center?.harmony_sessions.length === 0} emptyLabel={labels.emptySection}>
        {center?.harmony_sessions.map((sess) => (
          <li key={sess.session_key} className="rounded-xl border border-gray-100 p-4 text-sm">
            ...
            {sess.status !== "completed" && center?.can_manage && (
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <ActionBtn label={labels.scheduleWorkshop} variant="muted" onClick={() => void postAction({ action: "schedule_collaboration_workshop", session_key: sess.session_key })} />
              </div>
            )}
          </li>
        ))}

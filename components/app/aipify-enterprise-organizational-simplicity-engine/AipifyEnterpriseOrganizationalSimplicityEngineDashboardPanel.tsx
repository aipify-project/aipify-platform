...
import {
  parseAipifyEnterpriseOrganizationalSimplicityEngineDashboard,
  type AbosSuccessCriterion,
  type BlueprintObjective,
  type ExecutiveReview,
  type AipifyEnterpriseOrganizationalSimplicityEngineDashboard,
  type IntegrationLink,
  type ReflectionEntry,
  type ScaffoldNote,
} from "@/lib/aipify/aipify-enterprise-organizational-simplicity-engine";
...
export function AipifyEnterpriseOrganizationalSimplicityEngineDashboardPanel({ labels }: Props) {
  const [dashboard, setDashboard] = useState<AipifyEnterpriseOrganizationalSimplicityEngineDashboard | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/aipify/aipify-enterprise-organizational-simplicity-engine/dashboard");
    if (res.ok) setDashboard(parseAipifyEnterpriseOrganizationalSimplicityEngineDashboard(await res.json()));
    setLoading(false);
  }, []);
  ...
  const integrationLinks = dashboard.aeosebp277_integration_links?.length
    ? dashboard.aeosebp277_integration_links
    : dashboard.integration_links;
  const eraOpener = dashboard.aeosebp277_era_opener_summary?.length
    ? dashboard.aeosebp277_era_opener_summary
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-xl border border-indigo-200 bg-indigo-50/50 p-6">
        ...
        <p className="mt-2 text-2xl font-bold text-indigo-900">{dashboard.aipify_enterprise_organizational_simplicity_score ?? 0}</p>
        <p className="text-xs text-indigo-700">{labels.scoreLabel}</p>
        {dashboard.aipify_enterprise_organizational_simplicity_mission ? <p className="mt-2 text-sm font-medium text-indigo-900">{dashboard.aipify_enterprise_organizational_simplicity_mission}</p> : null}
        ...
        {dashboard.aipify_enterprise_organizational_simplicity_vision ? <p className="mt-2 text-xs italic text-indigo-800">{dashboard.aipify_enterprise_organizational_simplicity_vision}</p> : null}
        <div className="mt-4 flex flex-wrap gap-4 text-sm">
          <div><span className="font-medium">{labels.modeLabel}:</span> {dashboard.enterprise_organizational_simplicity_mode}</div>
          <div><span className="font-medium">{labels.readinessLabel}:</span> {dashboard.enterprise_simplicity_index_level ?? 1}</div>
      ...
      {dashboard.aipify_enterprise_organizational_simplicity_objectives?.length ? (
        <section className="space-y-3">
          <h2 className="text-lg font-semibold text-gray-900">{labels.blueprintObjectives}</h2>
          <div className="grid gap-2 sm:grid-cols-2">
            {dashboard.aipify_enterprise_organizational_simplicity_objectives.map((o) => <ObjectiveCard key={o.key ?? o.label} objective={o} />)}
          </div>
        </section>
      ) : null}
      ...
      {dashboard.aipify_enterprise_organizational_simplicity_success_criteria?.length ? (
        <section className="space-y-2">
          <h2 className="text-lg font-semibold text-gray-900">{labels.successCriteria}</h2>
          {dashboard.aipify_enterprise_organizational_simplicity_success_criteria.map((c) => (
            <SuccessCriterionRow key={c.key ?? c.label} criterion={c} metLabel={labels.criterionMet} pendingLabel={labels.criterionPending} />
          ))}
        </section>
      ) : null}

      {dashboard.aipify_enterprise_organizational_simplicity_privacy_note ? <p className="text-xs text-gray-500">{dashboard.aipify_enterprise_organizational_simplicity_privacy_note}</p> : null}
    </div>
  );
}

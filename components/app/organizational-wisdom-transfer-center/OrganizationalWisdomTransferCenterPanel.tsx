...
import {
  ...
  OCWT_PHILOSOPHY,
  OCWT_VISION,
  parseOrganizationalWisdomTransferCenter,
  type OrganizationalWisdomTransferCenter,
} from "@/lib/organizational-wisdom-transfer-center";
...
type PanelLabels = {
  ...
  executiveFields: {
    ...
    institutionalMemoryStrength: string;
    knowledgeTransferTrends: string;
...
const HEALTH_STYLES: Record<string, string> = {
  ...
  wisdom_transfer_recommended: "border-rose-200 bg-rose-50",
};
...
export function OrganizationalWisdomTransferCenterPanel({ labels }: Props) {
  const [center, setCenter] = useState<OrganizationalWisdomTransferCenter | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/organizational-wisdom-transfer/center");
    if (res.ok) setCenter(parseOrganizationalWisdomTransferCenter(await res.json()));
    setLoading(false);
  }, []);
  ...
  const postAction = async (payload: Record<string, unknown>) => {
    await fetch("/api/organizational-wisdom-transfer/action", {
      method: "POST",
  ...
  const healthStyle = HEALTH_STYLES[dash?.wisdom_transfer_health_label ?? "healthy"] ?? HEALTH_STYLES.healthy;
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      <div className="flex flex-wrap gap-3 text-sm">
        ...
        {center?.links?.organizational_hope && (
          <Link href={center.links.organizational_hope} className="text-slate-600 hover:underline">
      ...
      <div>
        ...
        <p className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900">
          {labels.corePrinciple}: {OCWT_CORE_PRINCIPLE}
        </p>
        <p className="mt-2 text-sm text-gray-600">
          {labels.philosophyTitle}: {OCWT_PHILOSOPHY}
        </p>
        <p className="mt-1 text-sm text-gray-600">
          {labels.visionTitle}: {OCWT_VISION}
        </p>
      ...
      {dash && (
        <section className={`rounded-2xl border p-5 shadow-sm ${healthStyle}`}>
          ...
          <p className="mt-1 text-sm text-gray-600">
            {labels.healthLabels[dash.wisdom_transfer_health_label] ?? dash.wisdom_transfer_health_label} ·{" "}
            {labels.wisdomTransferScore}: {dash.wisdom_transfer_score}/100 · {labels.knowledgePreservation}:{" "}
            {dash.knowledge_preservation_pct}%
          </p>
          <dl className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Metric label={labels.metrics.experienceSharing} value={`${dash.experience_sharing_pct}%`} />
            <Metric label={labels.metrics.judgmentTransfer} value={`${dash.judgment_transfer_pct}%`} />
            <Metric label={labels.metrics.institutionalMemory} value={`${dash.institutional_memory_pct}%`} />
            <Metric label={labels.metrics.learningIntegration} value={`${dash.learning_integration_pct}%`} />
            <Metric label={labels.metrics.wisdomStewardship} value={`${dash.wisdom_stewardship_pct}%`} />
            <Metric label={labels.metrics.mentorshipParticipation} value={`${dash.mentorship_participation_pct}%`} />
          ...
          {center?.can_manage && (
            <div className="mt-4 flex flex-wrap gap-2">
              <ActionBtn label={labels.generateReport} onClick={() => void postAction({ action: "generate_wisdom_transfer_report" })} />
              ...
              <ActionBtn
                ...
                variant="muted"
                onClick={() => void postAction({ action: "export_wisdom_transfer_snapshot", period_label: "Current quarter" })}
              />
      ...
      <Section title={labels.signalsTitle} empty={center?.wisdom_transfer_signals.length === 0} emptyLabel={labels.emptySection}>
        {center?.wisdom_transfer_signals.map((sig) => (
      ...
      <Section title={labels.transferTitle} empty={center?.transfer_prompts.length === 0} emptyLabel={labels.emptySection}>
        {center?.transfer_prompts.map((que) => (
          <li key={que.transfer_key} className="rounded-xl border border-indigo-100 bg-indigo-50/20 p-4 text-sm">
            <p className="text-xs text-gray-500">{labels.transferTypes[que.transfer_type] ?? que.transfer_type}</p>
      ...
      <Section title={labels.initiativesTitle} empty={center?.wisdom_transfer_initiatives.length === 0} emptyLabel={labels.emptySection}>
        {center?.wisdom_transfer_initiatives.map((ini) => (
      ...
      <Section title={labels.reviewsTitle} empty={center?.wisdom_transfer_reviews.length === 0} emptyLabel={labels.emptySection}>
        {center?.wisdom_transfer_reviews.map((rev) => (
      ...
      <Section title={labels.milestonesTitle} empty={center?.wisdom_transfer_milestones.length === 0} emptyLabel={labels.emptySection}>
        {center?.wisdom_transfer_milestones.map((ms) => (
        ...
        {center?.can_manage && (
          <li className="pt-2">
            <ActionBtn
              ...
              onClick={() =>
                void postAction({
                  action: "archive_wisdom_transfer_milestone",
                  title: "Wisdom Transfer milestone",
                  summary: "Wisdom Transfer milestone archived via Wisdom Transfer Center.",
                })
              }
            />
          </li>
          ...
          <li key={snap.snapshot_key} className="rounded-xl border border-gray-100 p-3 text-sm">
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              <span className="text-xs text-indigo-700">
                {labels.wisdomTransferScore}: {snap.wisdom_transfer_score}
              </span>
            </div>
      ...
      {center?.executive_view && (
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            <ExecField label={labels.executiveFields.leadershipStewardship} value={center.executive_view.leadership_stewardship} />
            <ExecField label={labels.executiveFields.institutionalMemoryStrength} value={center.executive_view.institutional_memory_strength} />
            <ExecField label={labels.executiveFields.knowledgeTransferTrends} value={center.executive_view.knowledge_transfer_trends} />
            <ExecField label={labels.executiveFields.successionReadiness} value={center.executive_view.succession_readiness} />
          </dl>
        </section>
      )}
      ...
      <Section title={labels.sessionsTitle} empty={center?.wisdom_transfer_sessions.length === 0} emptyLabel={labels.emptySection}>
        {center?.wisdom_transfer_sessions.map((sess) => (

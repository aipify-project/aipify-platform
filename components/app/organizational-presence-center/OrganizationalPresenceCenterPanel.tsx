...
import {
  ...
  OCP_PHILOSOPHY,
  OCP_VISION,
  parseOrganizationalPresenceCenter,
  type OrganizationalPresenceCenter,
} from "@/lib/organizational-presence-center";
...
type PanelLabels = {
  ...
  attentivenessTypes: Record<string, string>;
...
const HEALTH_STYLES: Record<string, string> = {
  ...
  presence_reinforcement_recommended: "border-rose-200 bg-rose-50",
};
...
export function OrganizationalPresenceCenterPanel({ labels }: Props) {
  const [center, setCenter] = useState<OrganizationalPresenceCenter | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/organizational-presence/center");
    if (res.ok) setCenter(parseOrganizationalPresenceCenter(await res.json()));
    setLoading(false);
  }, []);
  ...
  const postAction = async (payload: Record<string, unknown>) => {
    await fetch("/api/organizational-presence/action", {
      method: "POST",
  ...
  const healthStyle = HEALTH_STYLES[dash?.presence_health_label ?? "healthy"] ?? HEALTH_STYLES.healthy;
    ...
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      ...
      <div>
        ...
        <p className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900">
          {labels.corePrinciple}: {OCP_CORE_PRINCIPLE}
        </p>
        <p className="mt-2 text-sm text-gray-600">
          {labels.philosophyTitle}: {OCP_PHILOSOPHY}
        </p>
        <p className="mt-1 text-sm text-gray-600">
          {labels.visionTitle}: {OCP_VISION}
        </p>
      ...
      {dash && (
        <section className={`rounded-2xl border p-5 shadow-sm ${healthStyle}`}>
          ...
          <p className="mt-1 text-sm text-gray-600">
            {labels.healthLabels[dash.presence_health_label] ?? dash.presence_health_label} ·{" "}
            {labels.presenceScore}: {dash.presence_score}/100 · {labels.engagementIndicators}:{" "}
            {dash.engagement_indicators_pct}%
          </p>
          <dl className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Metric label={labels.metrics.engagementIndicators} value={`${dash.engagement_indicators_pct}%`} />
            <Metric label={labels.metrics.leadershipAttentiveness} value={`${dash.leadership_attentiveness_pct}%`} />
            <Metric label={labels.metrics.customerResponsiveness} value={`${dash.customer_responsiveness_pct}%`} />
          ...
          {center?.can_manage && (
            <div className="mt-4 flex flex-wrap gap-2">
              <ActionBtn label={labels.generateReport} onClick={() => void postAction({ action: "generate_presence_report" })} />
              ...
              <ActionBtn
                ...
                variant="muted"
                onClick={() => void postAction({ action: "export_presence_snapshot", period_label: "Current quarter" })}
              />
      ...
      <Section title={labels.signalsTitle} empty={center?.presence_signals.length === 0} emptyLabel={labels.emptySection}>
        {center?.presence_signals.map((sig) => (
      ...
      <Section title={labels.attentivenessTitle} empty={center?.attentiveness_prompts.length === 0} emptyLabel={labels.emptySection}>
        {center?.attentiveness_prompts.map((que) => (
          <li key={que.application_key} className="rounded-xl border border-indigo-100 bg-indigo-50/20 p-4 text-sm">
            <p className="text-xs text-gray-500">{labels.attentivenessTypes[que.application_type] ?? que.application_type}</p>
      ...
      <Section title={labels.initiativesTitle} empty={center?.presence_initiatives.length === 0} emptyLabel={labels.emptySection}>
        {center?.presence_initiatives.map((ini) => (
      ...
      <Section title={labels.reviewsTitle} empty={center?.presence_reviews.length === 0} emptyLabel={labels.emptySection}>
        {center?.presence_reviews.map((rev) => (
      ...
      <Section title={labels.milestonesTitle} empty={center?.presence_milestones.length === 0} emptyLabel={labels.emptySection}>
        {center?.presence_milestones.map((ms) => (
        ...
        {center?.can_manage && (
          <li className="pt-2">
            <ActionBtn
              ...
              onClick={() =>
                void postAction({
                  action: "archive_presence_milestone",
                  title: "Adaptive Intelligence milestone",
                  summary: "Adaptive Intelligence milestone archived via Presence Center.",
                })
              }
            />
          </li>
          ...
          <li key={snap.snapshot_key} className="rounded-xl border border-gray-100 p-3 text-sm">
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              <span className="text-xs text-indigo-700">
                {labels.presenceScore}: {snap.presence_score}
              </span>
            </div>
      ...
      {center?.executive_view && (
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            <ExecField label={labels.executiveFields.leadershipEngagement} value={center.executive_view.leadership_engagement} />
            ...
            <ExecField label={labels.executiveFields.relationshipQuality} value={center.executive_view.relationship_quality} />
            <ExecField label={labels.executiveFields.connectionOpportunities} value={center.executive_view.connection_opportunities} />
          </dl>
        </section>
      )}
      ...
      <Section title={labels.sessionsTitle} empty={center?.presence_sessions.length === 0} emptyLabel={labels.emptySection}>
        {center?.presence_sessions.map((sess) => (

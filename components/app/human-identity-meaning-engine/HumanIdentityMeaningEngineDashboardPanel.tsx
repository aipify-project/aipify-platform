...
import {
  parseHumanIdentityMeaningDashboard,
  type AbosSuccessCriterion,
  type AgencyNote,
  type BlueprintObjective,
  type ExecutiveHumanityReview,
  type HumanIdentityMeaningDashboard,
  type IntegrationLink,
  type MeaningReflection,
} from "@/lib/aipify/human-identity-meaning-engine";
...
function ReviewRow({ review }: { review: ExecutiveHumanityReview }) {
  return (
...
function ReflectionRow({ reflection }: { reflection: MeaningReflection }) {
  return (
...
function AgencyRow({ note }: { note: AgencyNote }) {
  return (
...
export function HumanIdentityMeaningEngineDashboardPanel({ labels }: Props) {
  const [dashboard, setDashboard] = useState<HumanIdentityMeaningDashboard | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/aipify/human-identity-meaning-engine/dashboard");
    if (res.ok) {
      setDashboard(parseHumanIdentityMeaningDashboard(await res.json()));
    }
    setLoading(false);
  }, []);
  ...
  const integrationLinks =
    dashboard.himpbp173_integration_links?.length
      ? dashboard.himpbp173_integration_links
      : dashboard.integration_links;

  const eraOpener =
    dashboard.himpbp173_era_opener_summary?.length
      ? dashboard.himpbp173_era_opener_summary
    ...
    <div className="space-y-6">
      <section className="rounded-xl border border-indigo-200 bg-indigo-50/50 p-6">
        ...
        <p className="mt-2 text-2xl font-bold text-indigo-900">
          {dashboard.human_identity_meaning_score ?? 0}
        </p>
        <p className="text-xs text-indigo-700">{labels.humanIdentityMeaningScore}</p>
        {dashboard.human_identity_meaning_mission ? (
        ...
        {dashboard.human_identity_meaning_vision ? (
          <p className="mt-2 text-xs italic text-indigo-800">{dashboard.human_identity_meaning_vision}</p>
        ) : null}
        <div className="mt-4 flex flex-wrap gap-4 text-sm">
          <div>
            <span className="font-medium">{labels.discoveryMode}:</span> {dashboard.discovery_mode}
          </div>
          <div>
            <span className="font-medium">{labels.meaningReadinessLevel}:</span>{" "}
            {dashboard.meaning_readiness_level ?? 1}
          </div>
          <div>
      ...
      <MetaListSection
        title={labels.meaningIdentityCenter}
        meta={dashboard.meaning_identity_center_meta}
        itemsKey="capabilities"
      />

      <MetaListSection
        title={labels.humanIdentityEngine}
        meta={dashboard.human_identity_engine_meta}
        itemsKey="reflection_questions"
      />

      <MetaListSection
        title={labels.meaningPreservationFramework}
        meta={dashboard.meaning_preservation_framework_meta}
        itemsKey="domains"
      />

      <MetaListSection
        title={labels.executiveHumanityReviews}
        meta={dashboard.executive_humanity_reviews_meta}
        itemsKey="review_themes"
      />

      <MetaListSection
        title={labels.meaningCompanion}
        meta={dashboard.meaning_companion_meta}
        itemsKey="capabilities"
      />
      ...
      <MetaListSection
        title={labels.agencyPreservationEngine}
        meta={dashboard.agency_preservation_engine_meta}
        itemsKey="protections"
      />

      {dashboard.meaning_reflections.length > 0 ? (
        <section className="space-y-3">
          <h2 className="text-lg font-semibold text-gray-900">{labels.meaningReflections}</h2>
          <div className="space-y-2">
            {dashboard.meaning_reflections.map((reflection) => (
      ...
      {dashboard.agency_notes.length > 0 ? (
        <section className="space-y-3">
          <h2 className="text-lg font-semibold text-gray-900">{labels.agencyNotes}</h2>
          <div className="space-y-2">
            {dashboard.agency_notes.map((note) => (
              <AgencyRow key={note.id} note={note} />
            ))}
          </div>
        </section>
      ) : null}
      ...
      {dashboard.human_identity_meaning_objectives?.length ? (
        <section className="space-y-3">
          <h2 className="text-lg font-semibold text-gray-900">{labels.blueprintObjectives}</h2>
          <div className="grid gap-2 sm:grid-cols-2">
            {dashboard.human_identity_meaning_objectives.map((objective) => (
      ...
      {dashboard.human_identity_meaning_success_criteria?.length ? (
        <section className="space-y-2">
          <h2 className="text-lg font-semibold text-gray-900">{labels.successCriteria}</h2>
          {dashboard.human_identity_meaning_success_criteria.map((criterion) => (
            <SuccessCriterionRow
      ...
      {dashboard.human_identity_meaning_privacy_note ? (
        <p className="text-xs text-gray-500">{dashboard.human_identity_meaning_privacy_note}</p>
      ) : null}
    </div>
  );
}

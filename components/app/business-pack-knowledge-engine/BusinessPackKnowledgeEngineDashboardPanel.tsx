...
import {
  packKnowledgeRoute,
  parseBusinessPackKnowledgeEngineDashboard,
  type BusinessPackKnowledgeEngineDashboard,
} from "@/lib/aipify/business-pack-knowledge-engine";

type Props = { labels: Record<string, string> };

export function BusinessPackKnowledgeEngineDashboardPanel({ labels }: Props) {
  const [dashboard, setDashboard] = useState<BusinessPackKnowledgeEngineDashboard | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/aipify/business-pack-knowledge-engine/dashboard");
    if (res.ok) setDashboard(parseBusinessPackKnowledgeEngineDashboard(await res.json()));
    setLoading(false);
  }, []);
    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-3 sm:grid-cols-5">
        {[
          ...
          ["knowledgeArticles", summary.knowledge_articles],
          ["feedbackRecords", summary.feedback_records],
          ["openGaps", summary.open_knowledge_gaps],
      ...
      {dashboard.knowledge_structure && dashboard.knowledge_structure.length > 0 && (
        <section className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          <h3 className="text-sm font-semibold text-gray-900">{labels.knowledgeStructure}</h3>
          <ol className="mt-3 space-y-1 text-sm text-gray-700">
            {dashboard.knowledge_structure.map((item) => (
      ...
      {(dashboard.top_searches ?? []).length > 0 && (
        <section className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          <h3 className="text-sm font-semibold text-gray-900">{labels.topSearches}</h3>
          <ul className="mt-2 space-y-1 text-sm text-gray-700">
            {dashboard.top_searches!.map((item) => (
              <li key={item.query}>{item.query} ({item.count})</li>
            ))}
          </ul>
        </section>
      )}
        ...
        {(dashboard.definitions ?? []).map((def) => (
          <article key={String(def.pack_key)} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
            <div>
              ...
              <p className="text-xs text-gray-500">{String(def.article_count)} {labels.articles}</p>
            </div>
            <Link
              href={String(def.knowledge_center_route ?? packKnowledgeRoute(String(def.pack_key)))}

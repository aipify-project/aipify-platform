...
import {
  parseAipifyStudioCreativeIntelligenceEngineDashboard,
  type AbosSuccessCriterion,
  type BlueprintObjective,
  type ExecutiveReview,
  type AipifyStudioCreativeIntelligenceEngineDashboard,
  type IntegrationLink,
  type ReflectionEntry,
  type ScaffoldNote,
} from "@/lib/aipify/aipify-studio-creative-intelligence-engine";
...
export function AipifyStudioCreativeIntelligenceEngineDashboardPanel({ labels }: Props) {
  const [dashboard, setDashboard] = useState<AipifyStudioCreativeIntelligenceEngineDashboard | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/aipify/aipify-studio-creative-intelligence-engine/dashboard");
    if (res.ok) setDashboard(parseAipifyStudioCreativeIntelligenceEngineDashboard(await res.json()));
    setLoading(false);
  }, []);
  ...
  const integrationLinks = dashboard.asciebp229_integration_links?.length
    ? dashboard.asciebp229_integration_links
    : dashboard.integration_links;
  const eraOpener = dashboard.asciebp229_era_opener_summary?.length
    ? dashboard.asciebp229_era_opener_summary
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-xl border border-indigo-200 bg-indigo-50/50 p-6">
        ...
        <p className="mt-2 text-2xl font-bold text-indigo-900">{dashboard.aipify_studio_creative_intelligence_score ?? 0}</p>
        <p className="text-xs text-indigo-700">{labels.scoreLabel}</p>
        {dashboard.aipify_studio_creative_intelligence_mission ? <p className="mt-2 text-sm font-medium text-indigo-900">{dashboard.aipify_studio_creative_intelligence_mission}</p> : null}
        ...
        {dashboard.aipify_studio_creative_intelligence_vision ? <p className="mt-2 text-xs italic text-indigo-800">{dashboard.aipify_studio_creative_intelligence_vision}</p> : null}
        <div className="mt-4 flex flex-wrap gap-4 text-sm">
          <div><span className="font-medium">{labels.modeLabel}:</span> {dashboard.creative_intelligence_mode}</div>
          <div><span className="font-medium">{labels.readinessLabel}:</span> {dashboard.creative_maturity_level ?? 1}</div>
      ...
      {dashboard.aipify_studio_creative_intelligence_objectives?.length ? (
        <section className="space-y-3">
          <h2 className="text-lg font-semibold text-gray-900">{labels.blueprintObjectives}</h2>
          <div className="grid gap-2 sm:grid-cols-2">
            {dashboard.aipify_studio_creative_intelligence_objectives.map((o) => <ObjectiveCard key={o.key ?? o.label} objective={o} />)}
          </div>
        </section>
      ) : null}
      ...
      {dashboard.aipify_studio_creative_intelligence_success_criteria?.length ? (
        <section className="space-y-2">
          <h2 className="text-lg font-semibold text-gray-900">{labels.successCriteria}</h2>
          {dashboard.aipify_studio_creative_intelligence_success_criteria.map((c) => (
            <SuccessCriterionRow key={c.key ?? c.label} criterion={c} metLabel={labels.criterionMet} pendingLabel={labels.criterionPending} />
          ))}
        </section>
      ) : null}

      {dashboard.aipify_studio_creative_intelligence_privacy_note ? <p className="text-xs text-gray-500">{dashboard.aipify_studio_creative_intelligence_privacy_note}</p> : null}
    </div>
  );
}

...
import {
  ...
  OAC_PHILOSOPHY,
  OAC_VISION,
  parseOrganizationalAwarenessCenter,
  type OrganizationalAwarenessCenter,
} from "@/lib/organizational-awareness-center";
...
type PanelLabels = {
  ...
  observationTypes: Record<string, string>;
  ...
  executiveFields: {
    leadershipAttentiveness: string;
    strategicObservation: string;
    environmentalAwareness: string;
    insightOpportunities: string;
  };
};
...
const HEALTH_STYLES: Record<string, string> = {
  ...
  awareness_improvement_recommended: "border-rose-200 bg-rose-50",
};
...
export function OrganizationalAwarenessCenterPanel({ labels }: Props) {
  const [center, setCenter] = useState<OrganizationalAwarenessCenter | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/organizational-awareness/center");
    if (res.ok) setCenter(parseOrganizationalAwarenessCenter(await res.json()));
    setLoading(false);
  }, []);
  ...
  const postAction = async (payload: Record<string, unknown>) => {
    await fetch("/api/organizational-awareness/action", {
      method: "POST",
  ...
  const healthStyle = HEALTH_STYLES[dash?.awareness_health_label ?? "healthy"] ?? HEALTH_STYLES.healthy;
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      <div className="flex flex-wrap gap-3 text-sm">
        ...
        {center?.links?.organizational_intentionality && (
          <Link href={center.links.organizational_intentionality} className="text-slate-600 hover:underline">
      ...
      <div>
        ...
        <p className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900">
          {labels.corePrinciple}: {OAC_CORE_PRINCIPLE}
        </p>
        <p className="mt-2 text-sm text-gray-600">
          {labels.philosophyTitle}: {OAC_PHILOSOPHY}
        </p>
        <p className="mt-1 text-sm text-gray-600">
          {labels.visionTitle}: {OAC_VISION}
        </p>
      ...
      {dash && (
        <section className={`rounded-2xl border p-5 shadow-sm ${healthStyle}`}>
          ...
          <p className="mt-1 text-sm text-gray-600">
            {labels.healthLabels[dash.awareness_health_label] ?? dash.awareness_health_label} ·{" "}
            {labels.awarenessScore}: {dash.awareness_score}/100 · {labels.emergingThemes}:{" "}
            {dash.emerging_themes_pct}%
          </p>
          <dl className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Metric label={labels.metrics.environmentalScanning} value={`${dash.environmental_scanning_pct}%`} />
            <Metric label={labels.metrics.insightUtilization} value={`${dash.insight_utilization_pct}%`} />
            <Metric label={labels.metrics.leadershipResponsiveness} value={`${dash.leadership_responsiveness_pct}%`} />
            <Metric label={labels.metrics.strategicObservations} value={`${dash.strategic_observations_pct}%`} />
          ...
          {center?.can_manage && (
            <div className="mt-4 flex flex-wrap gap-2">
              <ActionBtn label={labels.generateReport} onClick={() => void postAction({ action: "generate_awareness_report" })} />
              ...
              <ActionBtn
                ...
                variant="muted"
                onClick={() => void postAction({ action: "export_awareness_snapshot", period_label: "Current quarter" })}
              />
              <ActionBtn label={labels.coordinateReview} variant="muted" onClick={() => void postAction({ action: "coordinate_strategic_discussion" })} />
            </div>
          )}
        </section>
      )}

      <Section title={labels.signalsTitle} empty={center?.awareness_signals.length === 0} emptyLabel={labels.emptySection}>
        {center?.awareness_signals.map((sig) => (
      ...
      <Section title={labels.observationTitle} empty={center?.observation_prompts.length === 0} emptyLabel={labels.emptySection}>
        {center?.observation_prompts.map((per) => (
          <li key={per.observation_key} className="rounded-xl border border-indigo-100 bg-indigo-50/20 p-4 text-sm">
            <p className="text-xs text-gray-500">{labels.observationTypes[per.observation_type] ?? per.observation_type}</p>
      ...
      <Section title={labels.initiativesTitle} empty={center?.awareness_initiatives.length === 0} emptyLabel={labels.emptySection}>
        {center?.awareness_initiatives.map((ini) => (
      ...
      <Section title={labels.reviewsTitle} empty={center?.awareness_reviews.length === 0} emptyLabel={labels.emptySection}>
        {center?.awareness_reviews.map((rev) => (
      ...
      <Section title={labels.milestonesTitle} empty={center?.awareness_milestones.length === 0} emptyLabel={labels.emptySection}>
        {center?.awareness_milestones.map((ms) => (
        ...
        {center?.can_manage && (
          <li className="pt-2">
            <ActionBtn
              ...
              onClick={() =>
                void postAction({
                  action: "archive_awareness_milestone",
                  title: "Awareness milestone",
                  summary: "Awareness milestone archived via Awareness Center.",
                })
              }
            />
          </li>
          ...
          <li key={snap.snapshot_key} className="rounded-xl border border-gray-100 p-3 text-sm">
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              <span className="text-xs text-indigo-700">
                {labels.awarenessScore}: {snap.awareness_score}
              </span>
            </div>
      ...
      {center?.executive_view && (
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            <ExecField label={labels.executiveFields.leadershipAttentiveness} value={center.executive_view.leadership_attentiveness} />
            <ExecField label={labels.executiveFields.strategicObservation} value={center.executive_view.strategic_observation} />
            <ExecField label={labels.executiveFields.environmentalAwareness} value={center.executive_view.environmental_awareness} />
            <ExecField label={labels.executiveFields.insightOpportunities} value={center.executive_view.insight_opportunities} />
          </dl>
        </section>
      )}
      ...
      <Section title={labels.sessionsTitle} empty={center?.awareness_sessions.length === 0} emptyLabel={labels.emptySection}>
        {center?.awareness_sessions.map((sess) => (

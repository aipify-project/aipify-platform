...
import {
  PARTNERS_PORTAL_HOME_ROUTE,
  PARTNERS_PORTAL_NAV_GROUPS,
  getPartnersActiveNavId,
...
export function PartnersPortalShell({
  portalBadge,
  portalTitle,
  portalSubtitle,
  signOutLabel,
  ...
  navLabels,
  children,
}: PartnersPortalShellProps) {
  const pathname = usePathname();
  const activeId = getPartnersActiveNavId(pathname);
    ...
    <div className="min-h-screen bg-gradient-to-b from-emerald-50/50 to-slate-50">
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6">
          <div>
            <Link href={PARTNERS_PORTAL_HOME_ROUTE} className="text-xs font-semibold uppercase tracking-wide text-emerald-700">
              {portalBadge}
            </Link>

...
import {
  ...
  parseGrowthPortalDashboard,
  type GrowthPortalDashboard,
  type GrowthPortalLabels,
} from "@/lib/growth-portal";

type GrowthPortalDashboardPanelProps = {
  labels: GrowthPortalLabels["dashboard"];
...
export function GrowthPortalDashboardPanel({
  labels,
  navLabels,
}: GrowthPortalDashboardPanelProps) {
  const [dashboard, setDashboard] = useState<GrowthPortalDashboard | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/growth-portal/dashboard");
    if (res.ok) setDashboard(parseGrowthPortalDashboard(await res.json()));
    setLoading(false);
  }, []);

...
type GrowthPortalAuthGuardProps = {
  loadingLabel: string;
  children: ReactNode;
};

export function GrowthPortalAuthGuard({ loadingLabel, children }: GrowthPortalAuthGuardProps) {
  ...
  useEffect(() => {
    ...
    async function verifyAccess() {
      ...
      if (error || !user) {
        router.replace("/login?next=/growth");
        return;
      }

      const res = await fetch("/api/growth-portal/access");

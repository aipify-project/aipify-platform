...
import { parseDeveloperCenter, type DeveloperCenter } from "@/lib/developer-center-engine/parse";
import type { Bp602Section } from "@/lib/developer-center-engine/config";
import { bp602SectionToRpc } from "@/lib/developer-center-engine/config";
import type { buildDeveloperCenterLabels } from "@/lib/developer-center-engine/labels";

type Labels = ReturnType<typeof buildDeveloperCenterLabels>;

type Props = {
  labels: Labels;
  section: Bp602Section;
};
...
export function DeveloperCenterPanel({ labels, section }: Props) {
  const [center, setCenter] = useState<DeveloperCenter | null>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const rpcSection = bp602SectionToRpc(section);
    const res = await fetch(`/api/platform/developer-center/center?section=${rpcSection}`);
    if (res.ok) setCenter(parseDeveloperCenter(await res.json()));
    else setCenter(null);
    setLoading(false);
  }, [section]);
  ...
  return (
    <div className="space-y-6">
      ...
      {(section === "overview" || section === "reports") && Object.keys(exec).length > 0 && (
        <section className="rounded-2xl border border-violet-100 bg-gradient-to-br from-violet-50/80 to-white p-6 shadow-sm">
          ...
          <dl className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <StatCard label={labels.executive.registeredDevelopers} value={exec.registered_developers ?? 0} />
            <StatCard label={labels.executive.activeProjects} value={exec.active_projects ?? 0} />
            <StatCard label={labels.executive.publishedPacks} value={exec.published_packs ?? 0} />
            <StatCard label={labels.executive.certificationsGranted} value={exec.certifications_granted ?? 0} />
            <StatCard label={labels.executive.pendingValidations} value={exec.pending_validations ?? 0} />
            <StatCard label={labels.executive.marketplaceReady} value={exec.marketplace_ready ?? 0} />
            <StatCard label={labels.executive.platformUsage} value={exec.platform_usage ?? stats.api_keys ?? 0} />
          </dl>
        </section>
      )}
      ...
      {section === "overview" && (
        <>
          <dl className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <StatCard label={labels.stats.developers} value={stats.developers ?? 0} />
            <StatCard label={labels.stats.projects} value={stats.projects ?? 0} />
            <StatCard label={labels.stats.manifests} value={stats.manifests ?? 0} />
            <StatCard label={labels.stats.sandboxes} value={stats.sandboxes ?? 0} />
          </dl>

...
export default function ModuleShowcase({ title, subtitle, modules }: ModuleShowcaseProps) {
  return (
    <section id="modules" className="scroll-mt-20 border-y border-white/10 bg-[#0c1018]" aria-labelledby="modules-title">
      <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 lg:py-20">
        <div className="mx-auto max-w-2xl text-center">
          <h2 id="modules-title" className="text-3xl font-bold tracking-tight text-white sm:text-4xl">
            {title}
          </h2>
        ...
        <ul className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
          {modules.map((mod, i) => (
            <li
              key={mod.name}
              className={`group rounded-2xl border border-white/10 bg-white/[0.03] p-5 transition duration-300 hover:-translate-y-1 hover:bg-white/[0.05] hover:shadow-lg ${MODULE_ACCENTS[i % MODULE_ACCENTS.length]}`}
            >

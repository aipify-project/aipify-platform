...
export default function ProductCommandBriefSection({ commandBrief }: ProductCommandBriefSectionProps) {
  return (
      ...
      <div className={`${PublicMarketingClasses.container} py-14 sm:py-16`}>
        <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)] lg:items-start lg:gap-14">
          <div className="max-w-xl">
            ...
            {commandBrief.subtitle ? (
              <p className={PublicMarketingClasses.sectionSubtitle}>{commandBrief.subtitle}</p>
            ) : null}

            <ul className="mt-8 space-y-5">
              {commandBrief.points.map((point) => (
          ...
          <div>
            <CommandBriefMockup labels={commandBrief} />
          </div>
        </div>
      </div>
    </section>

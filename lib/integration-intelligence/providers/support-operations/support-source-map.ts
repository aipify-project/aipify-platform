...
export const SUPPORT_OPERATIONS_SOURCE_MAP: readonly SupportOperationsSourceDefinition[] = [
  {
    ...
    available_fields: [
      ...
      "open_cases[].priority",
      "open_cases[].channel",
      "open_cases[].created_at",
      "open_cases[].escalated_at",
    ],
    required_permission: "support.view",
    status: "partial",
    read_only: true,
    limitations: [
      "Queue counts from Support AI dashboard — organization_support_cases only.",
    ],
  },
  {
  ...
  },
  {
    ...
    available_fields: [
      "sla.source_exact",
      "sla.policy_configured",
      "sla.sla_at_risk",
      "sla.breached",
      "open_cases[].sla_status",
      "open_cases[].first_response_due_at",
      "open_cases[].resolution_due_at",
      "settings.sla_warning_minutes_before_due",
      "settings.sla_at_risk_minutes_before_due",
    ],
    required_permission: "support.view_metrics",
    status: "live",
    read_only: true,
    limitations: [
      "SLA deadlines derive from stored aso_settings policy only — unavailable when policy is not configured.",
    ],
  },
  {

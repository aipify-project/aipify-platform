import type {
  CommandBriefPanel,
  CommandBriefRelatedAction,
  CommandBriefSignalSeverity,
  CommandBriefSignalType,
  CommandBriefSourceTier,
} from "./types";

export type CommandBriefCatalogEntry = {
  signal_key: string;
  signal_type: CommandBriefSignalType;
  category: string;
  title_key: string;
  summary_key: string;
  default_severity: CommandBriefSignalSeverity;
  source_tier: CommandBriefSourceTier;
  related_capability: string | null;
  required_permission: string | null;
  related_action: CommandBriefRelatedAction | null;
  panels: readonly CommandBriefPanel[];
};

const CORE_PREFIX = "customerApp.companionPlatformKnowledge.commandBriefCore";
const VERIFICATION_PREFIX = "customerApp.companionPlatformKnowledge.verification.commandBrief";
const BOOKING_PREFIX = "customerApp.companionPlatformKnowledge.booking.commandBrief";
const SUPPORT_PREFIX = "customerApp.companionPlatformKnowledge.support.commandBrief";
const HOSTS_PREFIX = "customerApp.companionPlatformKnowledge.hosts.commandBrief";
const DIRECTORY_EMPLOYEE_PREFIX =
  "customerApp.companionPlatformKnowledge.directory.appEmployee.commandBrief";
const DIRECTORY_CRM_PREFIX =
  "customerApp.companionPlatformKnowledge.directory.crm.commandBrief";
const DIRECTORY_SUPPLIER_PREFIX =
  "customerApp.companionPlatformKnowledge.directory.supplier.commandBrief";

function signal(
  signalKey: string,
  input: Omit<CommandBriefCatalogEntry, "signal_key">,
): CommandBriefCatalogEntry {
  return { signal_key: signalKey, ...input };
}

function domainSignal(
  signalKey: string,
  category: string,
  signalType: CommandBriefSignalType,
  permission: string,
  capability: string,
  href: string,
): CommandBriefCatalogEntry {
  return signal(signalKey, {
    signal_type: signalType,
    category,
    title_key: `${CORE_PREFIX}.signals.${signalKey}.title`,
    summary_key: `${CORE_PREFIX}.signals.${signalKey}.summary`,
    default_severity: "medium",
    source_tier: "compatible_live",
    related_capability: capability,
    required_permission: permission,
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href,
      capability_key: capability,
      executable: true,
    },
    panels: ["app"],
  });
}

export const COMMAND_BRIEF_SIGNAL_CATALOG: readonly CommandBriefCatalogEntry[] = [
  domainSignal("activity_change", "community", "attention", "customer_community.view", "community.read", "/app/community"),
  domainSignal("new_members", "community", "opportunity", "customer_community.view", "member.read", "/app/community"),
  domainSignal("reward_milestone", "community", "opportunity", "customer_community.view", "community.read", "/app/community"),
  domainSignal("pending_moderation", "moderation", "attention", "moderation.view", "moderation.read", "/app/moderation"),
  domainSignal("reports_requiring_attention", "moderation", "risk", "moderation.view", "moderation.read", "/app/moderation"),
  domainSignal("listing_review", "marketplace", "attention", "marketplace.view", "listing.read", "/app/marketplace"),
  signal("pending_verification", {
    signal_type: "attention",
    category: "verification",
    title_key: `${VERIFICATION_PREFIX}.pendingVerification`,
    summary_key: `${VERIFICATION_PREFIX}.pendingVerification`,
    default_severity: "high",
    source_tier: "exact_live",
    related_capability: "verification.read",
    required_permission: "verification.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/verification",
      capability_key: "verification.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("verification_needs_information", {
    signal_type: "attention",
    category: "verification",
    title_key: `${VERIFICATION_PREFIX}.needsInformation`,
    summary_key: `${VERIFICATION_PREFIX}.needsInformation`,
    default_severity: "medium",
    source_tier: "exact_live",
    related_capability: "verification.read",
    required_permission: "verification.view",
    related_action: {
      kind: "view_details",
      label_key: `${CORE_PREFIX}.actions.viewDetails`,
      href: "/app/verification",
      capability_key: "verification.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("verification_high_priority", {
    signal_type: "risk",
    category: "verification",
    title_key: `${VERIFICATION_PREFIX}.highPriority`,
    summary_key: `${VERIFICATION_PREFIX}.highPriority`,
    default_severity: "high",
    source_tier: "exact_live",
    related_capability: "verification.read",
    required_permission: "verification.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/verification",
      capability_key: "verification.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("verification_oldest_pending", {
    signal_type: "attention",
    category: "verification",
    title_key: `${VERIFICATION_PREFIX}.oldestPending`,
    summary_key: `${VERIFICATION_PREFIX}.oldestPending`,
    default_severity: "medium",
    source_tier: "exact_live",
    related_capability: "verification.read",
    required_permission: "verification.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/verification",
      capability_key: "verification.read",
      executable: true,
    },
    panels: ["app"],
  }),
  ...(
    [
      ["upcoming_bookings", "upcomingBookings"],
      ["unassigned_booking", "unassignedBooking"],
      ["booking_conflict", "bookingConflict"],
      ["availability_gap", "availabilityGap"],
      ["absence_coverage_needed", "absenceCoverageNeeded"],
      ["vacation_return_schedule", "vacationReturnSchedule"],
      ["booking_requiring_attention", "bookingRequiringAttention"],
      ["cancellation_change", "cancellationChange"],
    ] as const
  ).map(([signalKey, i18nKey]) =>
    signal(signalKey, {
      signal_type:
        signalKey.includes("conflict") || signalKey.includes("absence") ? "risk" : "attention",
      category: "booking",
      title_key: `${BOOKING_PREFIX}.${i18nKey}`,
      summary_key: `${BOOKING_PREFIX}.${i18nKey}`,
      default_severity: signalKey.includes("conflict") ? "high" : "medium",
      source_tier: "exact_live",
      related_capability: "booking.read",
      required_permission: "appointments.view",
      related_action: {
        kind: "open_page",
        label_key: `${CORE_PREFIX}.actions.openDetails`,
        href: "/app/appointments",
        capability_key: "booking.read",
        executable: true,
      },
      panels: ["app"],
    }),
  ),
  domainSignal("new_lead", "sales", "opportunity", "sales.view", "lead.read", "/app/sales"),
  domainSignal("lead_without_follow_up", "sales", "follow_up", "sales.view", "lead.read", "/app/sales"),
  domainSignal("recommended_follow_up", "sales", "recommendation", "sales.view", "lead.read", "/app/sales"),
  domainSignal("deal_status_change", "sales", "attention", "sales.view", "deal.read", "/app/sales"),
  domainSignal("sales_target_deviation", "sales", "forecast_warning", "sales.view", "sales.read", "/app/sales"),
  domainSignal("customer_health_warning", "sales", "health_score", "sales.view", "customer.read", "/app/sales"),
  domainSignal("churn_risk", "sales", "risk", "sales.view", "customer.read", "/app/sales"),
  domainSignal("conversion_deviation", "sales", "anomaly", "sales.view", "sales.read", "/app/sales"),
  domainSignal("overdue_invoice", "finance", "attention", "billing.view", "invoice.read", "/app/settings/billing"),
  domainSignal("subscription_change", "finance", "attention", "billing.view", "subscription.read", "/app/settings/billing"),
  domainSignal("forecast_warning", "finance", "forecast_warning", "billing.view", "finance.read", "/app/finance"),
  domainSignal("revenue_deviation", "finance", "anomaly", "billing.view", "finance.read", "/app/finance"),
  domainSignal("payment_failure", "finance", "alert", "billing.view", "payment.read", "/app/settings/billing"),
  domainSignal("payout_attention", "finance", "attention", "billing.view", "payout.read", "/app/finance"),
  domainSignal("reconciliation_issue", "finance", "risk", "billing.view", "finance.read", "/app/finance"),
  domainSignal("verification_pending", "security", "attention", "security.view", "verification.read", "/app/settings/security"),
  domainSignal("access_review_required", "security", "attention", "security.view", "access.read", "/app/settings/security"),
  domainSignal("security_incident", "security", "alert", "security.view", "incident.read", "/app/settings/security"),
  domainSignal("failed_login_2fa_issue", "security", "risk", "security.view", "auth.read", "/app/settings/security"),
  domainSignal("expiring_certificate", "security", "attention", "security.view", "certificate.read", "/app/settings/security"),
  domainSignal("compliance_attention", "security", "attention", "security.view", "compliance.read", "/app/settings/security"),
  domainSignal("unresolved_risk_signal", "security", "risk", "security.view", "risk.read", "/app/settings/security"),
  domainSignal("permission_anomaly", "security", "anomaly", "security.view", "permission.read", "/app/settings/security"),
  domainSignal("onboarding_in_progress", "hr", "attention", "hr.view", "onboarding.read", "/app/hr"),
  domainSignal("absence_upcoming", "hr", "attention", "hr.view", "absence.read", "/app/absence"),
  domainSignal("expiring_certifications", "hr", "attention", "hr.view", "certification.read", "/app/hr"),
  domainSignal("training_pending", "hr", "attention", "hr.view", "training.read", "/app/hr"),
  domainSignal("pending_invitations", "hr", "attention", "hr.view", "team.read", "/app/team"),
  domainSignal("performance_reviews", "hr", "attention", "hr.view", "performance.read", "/app/hr"),
  domainSignal("low_stock", "warehouse", "attention", "warehouse.view", "inventory.read", "/app/warehouse"),
  domainSignal("delayed_receipt", "warehouse", "attention", "warehouse.view", "receipt.read", "/app/warehouse"),
  domainSignal("transfer_in_progress", "warehouse", "attention", "warehouse.view", "transfer.read", "/app/warehouse"),
  domainSignal("inventory_discrepancy", "warehouse", "anomaly", "warehouse.view", "inventory.read", "/app/warehouse"),
  domainSignal("replenishment_required", "warehouse", "attention", "warehouse.view", "inventory.read", "/app/warehouse"),
  domainSignal("unresolved_support_case", "support", "attention", "support.view", "support_queue.read", "/app/settings/support-operations"),
  signal("unassigned_support_case", {
    signal_type: "attention",
    category: "support",
    title_key: `${SUPPORT_PREFIX}.unassignedSupportCase`,
    summary_key: `${SUPPORT_PREFIX}.unassignedSupportCase`,
    default_severity: "high",
    source_tier: "exact_live",
    related_capability: "support_queue.read",
    required_permission: "support.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/settings/support-operations",
      capability_key: "support_queue.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("urgent_support_case", {
    signal_type: "risk",
    category: "support",
    title_key: `${SUPPORT_PREFIX}.urgentSupportCase`,
    summary_key: `${SUPPORT_PREFIX}.urgentSupportCase`,
    default_severity: "high",
    source_tier: "exact_live",
    related_capability: "support_case.read",
    required_permission: "support.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/settings/support-operations",
      capability_key: "support_case.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("support_sla_warning", {
    signal_type: "risk",
    category: "support",
    title_key: `${SUPPORT_PREFIX}.supportSlaWarning`,
    summary_key: `${SUPPORT_PREFIX}.supportSlaWarning`,
    default_severity: "medium",
    source_tier: "exact_live",
    related_capability: "support_sla.read",
    required_permission: "support.view_metrics",
    related_action: {
      kind: "view_details",
      label_key: `${CORE_PREFIX}.actions.viewDetails`,
      href: "/app/settings/support-operations",
      capability_key: "support_sla.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("support_sla_breached", {
    signal_type: "alert",
    category: "support",
    title_key: `${SUPPORT_PREFIX}.supportSlaBreached`,
    summary_key: `${SUPPORT_PREFIX}.supportSlaBreached`,
    default_severity: "high",
    source_tier: "exact_live",
    related_capability: "support_sla.read",
    required_permission: "support.view_metrics",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/settings/support-operations",
      capability_key: "support_sla.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("support_escalation_required", {
    signal_type: "alert",
    category: "support",
    title_key: `${SUPPORT_PREFIX}.supportEscalationRequired`,
    summary_key: `${SUPPORT_PREFIX}.supportEscalationRequired`,
    default_severity: "high",
    source_tier: "exact_live",
    related_capability: "support_escalation.read",
    required_permission: "support.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/settings/support-operations",
      capability_key: "support_escalation.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("support_waiting_for_customer", {
    signal_type: "attention",
    category: "support",
    title_key: `${SUPPORT_PREFIX}.supportWaitingForCustomer`,
    summary_key: `${SUPPORT_PREFIX}.supportWaitingForCustomer`,
    default_severity: "medium",
    source_tier: "exact_live",
    related_capability: "support_case_status.read",
    required_permission: "support.view",
    related_action: {
      kind: "view_details",
      label_key: `${CORE_PREFIX}.actions.viewDetails`,
      href: "/app/settings/support-operations",
      capability_key: "support_case_status.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("support_waiting_for_agent", {
    signal_type: "attention",
    category: "support",
    title_key: `${SUPPORT_PREFIX}.supportWaitingForAgent`,
    summary_key: `${SUPPORT_PREFIX}.supportWaitingForAgent`,
    default_severity: "medium",
    source_tier: "exact_live",
    related_capability: "support_case_status.read",
    required_permission: "support.view",
    related_action: {
      kind: "view_details",
      label_key: `${CORE_PREFIX}.actions.viewDetails`,
      href: "/app/settings/support-operations",
      capability_key: "support_case_status.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("reopened_support_case", {
    signal_type: "attention",
    category: "support",
    title_key: `${SUPPORT_PREFIX}.reopenedSupportCase`,
    summary_key: `${SUPPORT_PREFIX}.reopenedSupportCase`,
    default_severity: "medium",
    source_tier: "exact_live",
    related_capability: "support_case.read",
    required_permission: "support.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/settings/support-operations",
      capability_key: "support_case.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("oldest_support_case", {
    signal_type: "attention",
    category: "support",
    title_key: `${SUPPORT_PREFIX}.oldestSupportCase`,
    summary_key: `${SUPPORT_PREFIX}.oldestSupportCase`,
    default_severity: "medium",
    source_tier: "exact_live",
    related_capability: "support_queue.read",
    required_permission: "support.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/settings/support-operations",
      capability_key: "support_queue.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("support_response_draft_ready", {
    signal_type: "opportunity",
    category: "support",
    title_key: `${SUPPORT_PREFIX}.supportResponseDraftReady`,
    summary_key: `${SUPPORT_PREFIX}.supportResponseDraftReady`,
    default_severity: "medium",
    source_tier: "exact_live",
    related_capability: "support_response.draft",
    required_permission: "support.reply",
    related_action: {
      kind: "view_details",
      label_key: `${CORE_PREFIX}.actions.viewDetails`,
      href: "/app/support-ai-engine",
      capability_key: "support_response.draft",
      executable: true,
    },
    panels: ["app"],
  }),
  domainSignal("sla_risk", "support", "risk", "support.view", "support_sla.read", "/app/settings/support-operations"),
  domainSignal("escalation_required", "support", "alert", "support.view", "support_escalation.read", "/app/settings/support-operations"),
  signal("arrival_today", {
    signal_type: "attention",
    category: "hosts",
    title_key: `${HOSTS_PREFIX}.arrivalToday`,
    summary_key: `${HOSTS_PREFIX}.arrivalToday`,
    default_severity: "medium",
    source_tier: "exact_live",
    related_capability: "arrival.read",
    required_permission: "aipify_hosts.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/aipify-hosts/operations",
      capability_key: "arrival.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("departure_today", {
    signal_type: "attention",
    category: "hosts",
    title_key: `${HOSTS_PREFIX}.departureToday`,
    summary_key: `${HOSTS_PREFIX}.departureToday`,
    default_severity: "medium",
    source_tier: "exact_live",
    related_capability: "departure.read",
    required_permission: "aipify_hosts.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/aipify-hosts/operations",
      capability_key: "departure.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("property_not_ready", {
    signal_type: "risk",
    category: "hosts",
    title_key: `${HOSTS_PREFIX}.propertyNotReady`,
    summary_key: `${HOSTS_PREFIX}.propertyNotReady`,
    default_severity: "high",
    source_tier: "exact_live",
    related_capability: "property.read",
    required_permission: "aipify_hosts.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/aipify-hosts/operations",
      capability_key: "property.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("cleaning_due", {
    signal_type: "attention",
    category: "hosts",
    title_key: `${HOSTS_PREFIX}.cleaningDue`,
    summary_key: `${HOSTS_PREFIX}.cleaningDue`,
    default_severity: "medium",
    source_tier: "exact_live",
    related_capability: "cleaning.read",
    required_permission: "aipify_hosts.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/aipify-hosts/operations",
      capability_key: "cleaning.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("cleaning_overdue", {
    signal_type: "risk",
    category: "hosts",
    title_key: `${HOSTS_PREFIX}.cleaningOverdue`,
    summary_key: `${HOSTS_PREFIX}.cleaningOverdue`,
    default_severity: "high",
    source_tier: "exact_live",
    related_capability: "cleaning.read",
    required_permission: "aipify_hosts.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/aipify-hosts/operations",
      capability_key: "cleaning.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("maintenance_open", {
    signal_type: "attention",
    category: "hosts",
    title_key: `${HOSTS_PREFIX}.maintenanceOpen`,
    summary_key: `${HOSTS_PREFIX}.maintenanceOpen`,
    default_severity: "medium",
    source_tier: "exact_live",
    related_capability: "maintenance.read",
    required_permission: "aipify_hosts.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/aipify-hosts/operations",
      capability_key: "maintenance.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("maintenance_urgent", {
    signal_type: "risk",
    category: "hosts",
    title_key: `${HOSTS_PREFIX}.maintenanceUrgent`,
    summary_key: `${HOSTS_PREFIX}.maintenanceUrgent`,
    default_severity: "high",
    source_tier: "exact_live",
    related_capability: "maintenance.read",
    required_permission: "aipify_hosts.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/aipify-hosts/operations",
      capability_key: "maintenance.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("guest_attention_required", {
    signal_type: "attention",
    category: "hosts",
    title_key: `${HOSTS_PREFIX}.guestAttentionRequired`,
    summary_key: `${HOSTS_PREFIX}.guestAttentionRequired`,
    default_severity: "high",
    source_tier: "exact_live",
    related_capability: "guest.read",
    required_permission: "aipify_hosts.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/aipify-hosts/operations",
      capability_key: "guest.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("reservation_change", {
    signal_type: "attention",
    category: "hosts",
    title_key: `${HOSTS_PREFIX}.reservationChange`,
    summary_key: `${HOSTS_PREFIX}.reservationChange`,
    default_severity: "medium",
    source_tier: "exact_live",
    related_capability: "reservation.read",
    required_permission: "aipify_hosts.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/aipify-hosts/bookings",
      capability_key: "reservation.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("cancellation_attention", {
    signal_type: "attention",
    category: "hosts",
    title_key: `${HOSTS_PREFIX}.cancellationAttention`,
    summary_key: `${HOSTS_PREFIX}.cancellationAttention`,
    default_severity: "medium",
    source_tier: "exact_live",
    related_capability: "reservation.read",
    required_permission: "aipify_hosts.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/aipify-hosts/bookings",
      capability_key: "reservation.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("payout_due", {
    signal_type: "attention",
    category: "hosts",
    title_key: `${HOSTS_PREFIX}.payoutDue`,
    summary_key: `${HOSTS_PREFIX}.payoutDue`,
    default_severity: "medium",
    source_tier: "exact_live",
    related_capability: "host_payout.read",
    required_permission: "aipify_hosts.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/aipify-hosts/finance",
      capability_key: "host_payout.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("payout_overdue", {
    signal_type: "risk",
    category: "hosts",
    title_key: `${HOSTS_PREFIX}.payoutOverdue`,
    summary_key: `${HOSTS_PREFIX}.payoutOverdue`,
    default_severity: "high",
    source_tier: "exact_live",
    related_capability: "host_payout.read",
    required_permission: "aipify_hosts.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/aipify-hosts/finance",
      capability_key: "host_payout.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("expense_attention", {
    signal_type: "attention",
    category: "hosts",
    title_key: `${HOSTS_PREFIX}.expenseAttention`,
    summary_key: `${HOSTS_PREFIX}.expenseAttention`,
    default_severity: "medium",
    source_tier: "exact_live",
    related_capability: "host_expense.read",
    required_permission: "aipify_hosts.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/aipify-hosts/finance",
      capability_key: "host_expense.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("occupancy_deviation", {
    signal_type: "anomaly",
    category: "hosts",
    title_key: `${HOSTS_PREFIX}.occupancyDeviation`,
    summary_key: `${HOSTS_PREFIX}.occupancyDeviation`,
    default_severity: "medium",
    source_tier: "exact_live",
    related_capability: "occupancy.read",
    required_permission: "aipify_hosts.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/aipify-hosts",
      capability_key: "occupancy.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("forecast_warning", {
    signal_type: "forecast_warning",
    category: "hosts",
    title_key: `${HOSTS_PREFIX}.forecastWarning`,
    summary_key: `${HOSTS_PREFIX}.forecastWarning`,
    default_severity: "medium",
    source_tier: "exact_live",
    related_capability: "host_forecast.read",
    required_permission: "aipify_hosts.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/aipify-hosts/finance",
      capability_key: "host_forecast.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("unresolved_host_task", {
    signal_type: "attention",
    category: "hosts",
    title_key: `${HOSTS_PREFIX}.unresolvedHostTask`,
    summary_key: `${HOSTS_PREFIX}.unresolvedHostTask`,
    default_severity: "medium",
    source_tier: "exact_live",
    related_capability: "host_task.read",
    required_permission: "aipify_hosts.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/aipify-hosts/operations",
      capability_key: "host_task.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("new_employee", {
    signal_type: "opportunity",
    category: "directory_employee",
    title_key: `${DIRECTORY_EMPLOYEE_PREFIX}.newEmployee`,
    summary_key: `${DIRECTORY_EMPLOYEE_PREFIX}.newEmployee`,
    default_severity: "low",
    source_tier: "exact_live",
    related_capability: "employee.read",
    required_permission: "employees.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/employees",
      capability_key: "employee.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("pending_employee_invitation", {
    signal_type: "attention",
    category: "directory_employee",
    title_key: `${DIRECTORY_EMPLOYEE_PREFIX}.pendingInvitation`,
    summary_key: `${DIRECTORY_EMPLOYEE_PREFIX}.pendingInvitation`,
    default_severity: "medium",
    source_tier: "exact_live",
    related_capability: "employee.search",
    required_permission: "employees.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/employees",
      capability_key: "employee.search",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("inactive_employee", {
    signal_type: "attention",
    category: "directory_employee",
    title_key: `${DIRECTORY_EMPLOYEE_PREFIX}.inactiveEmployee`,
    summary_key: `${DIRECTORY_EMPLOYEE_PREFIX}.inactiveEmployee`,
    default_severity: "medium",
    source_tier: "exact_live",
    related_capability: "employee.read",
    required_permission: "employees.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/employees",
      capability_key: "employee.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("access_review_required", {
    signal_type: "risk",
    category: "directory_employee",
    title_key: `${DIRECTORY_EMPLOYEE_PREFIX}.accessReviewRequired`,
    summary_key: `${DIRECTORY_EMPLOYEE_PREFIX}.accessReviewRequired`,
    default_severity: "high",
    source_tier: "exact_live",
    related_capability: "role.read",
    required_permission: "permissions.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/employees/access",
      capability_key: "role.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("missing_team_assignment", {
    signal_type: "attention",
    category: "directory_employee",
    title_key: `${DIRECTORY_EMPLOYEE_PREFIX}.missingTeamAssignment`,
    summary_key: `${DIRECTORY_EMPLOYEE_PREFIX}.missingTeamAssignment`,
    default_severity: "low",
    source_tier: "exact_live",
    related_capability: "team.read",
    required_permission: "employees.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/organization",
      capability_key: "team.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("employee_role_change", {
    signal_type: "attention",
    category: "directory_employee",
    title_key: `${DIRECTORY_EMPLOYEE_PREFIX}.roleChange`,
    summary_key: `${DIRECTORY_EMPLOYEE_PREFIX}.roleChange`,
    default_severity: "medium",
    source_tier: "exact_live",
    related_capability: "role.read",
    required_permission: "roles.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/employees",
      capability_key: "role.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("new_lead", {
    signal_type: "opportunity",
    category: "directory_crm",
    title_key: `${DIRECTORY_CRM_PREFIX}.newLead`,
    summary_key: `${DIRECTORY_CRM_PREFIX}.newLead`,
    default_severity: "low",
    source_tier: "exact_live",
    related_capability: "lead.search",
    required_permission: "sales.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/customers/leads",
      capability_key: "lead.search",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("lead_without_follow_up", {
    signal_type: "attention",
    category: "directory_crm",
    title_key: `${DIRECTORY_CRM_PREFIX}.leadWithoutFollowUp`,
    summary_key: `${DIRECTORY_CRM_PREFIX}.leadWithoutFollowUp`,
    default_severity: "medium",
    source_tier: "exact_live",
    related_capability: "lead.search",
    required_permission: "sales.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/customers/leads",
      capability_key: "lead.search",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("recommended_follow_up", {
    signal_type: "follow_up",
    category: "directory_crm",
    title_key: `${DIRECTORY_CRM_PREFIX}.recommendedFollowUp`,
    summary_key: `${DIRECTORY_CRM_PREFIX}.recommendedFollowUp`,
    default_severity: "medium",
    source_tier: "exact_live",
    related_capability: "lead.read",
    required_permission: "sales.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/customers/leads",
      capability_key: "lead.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("customer_health_warning", {
    signal_type: "attention",
    category: "directory_crm",
    title_key: `${DIRECTORY_CRM_PREFIX}.customerHealthWarning`,
    summary_key: `${DIRECTORY_CRM_PREFIX}.customerHealthWarning`,
    default_severity: "high",
    source_tier: "exact_live",
    related_capability: "customer.read",
    required_permission: "customers.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/customers",
      capability_key: "customer.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("churn_risk", {
    signal_type: "risk",
    category: "directory_crm",
    title_key: `${DIRECTORY_CRM_PREFIX}.churnRisk`,
    summary_key: `${DIRECTORY_CRM_PREFIX}.churnRisk`,
    default_severity: "high",
    source_tier: "exact_live",
    related_capability: "customer.read",
    required_permission: "sales.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/customers",
      capability_key: "customer.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("deal_status_change", {
    signal_type: "attention",
    category: "directory_crm",
    title_key: `${DIRECTORY_CRM_PREFIX}.dealStatusChange`,
    summary_key: `${DIRECTORY_CRM_PREFIX}.dealStatusChange`,
    default_severity: "medium",
    source_tier: "exact_live",
    related_capability: "opportunity.read",
    required_permission: "sales.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/customers/opportunities",
      capability_key: "opportunity.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("conversion_deviation", {
    signal_type: "anomaly",
    category: "directory_crm",
    title_key: `${DIRECTORY_CRM_PREFIX}.conversionDeviation`,
    summary_key: `${DIRECTORY_CRM_PREFIX}.conversionDeviation`,
    default_severity: "medium",
    source_tier: "exact_live",
    related_capability: "pipeline.read",
    required_permission: "sales.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/customers",
      capability_key: "pipeline.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("unassigned_customer", {
    signal_type: "attention",
    category: "directory_crm",
    title_key: `${DIRECTORY_CRM_PREFIX}.unassignedCustomer`,
    summary_key: `${DIRECTORY_CRM_PREFIX}.unassignedCustomer`,
    default_severity: "medium",
    source_tier: "exact_live",
    related_capability: "customer_owner.read",
    required_permission: "customers.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/customers",
      capability_key: "customer_owner.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("duplicate_customer_candidate", {
    signal_type: "attention",
    category: "directory_crm",
    title_key: `${DIRECTORY_CRM_PREFIX}.duplicateCustomerCandidate`,
    summary_key: `${DIRECTORY_CRM_PREFIX}.duplicateCustomerCandidate`,
    default_severity: "low",
    source_tier: "exact_live",
    related_capability: "customer.search",
    required_permission: "customers.view",
    related_action: {
      kind: "view_details",
      label_key: `${CORE_PREFIX}.actions.viewDetails`,
      href: "/app/customers",
      capability_key: "customer.search",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("supplier_contract_expiring", {
    signal_type: "attention",
    category: "directory_supplier",
    title_key: `${DIRECTORY_SUPPLIER_PREFIX}.supplierContractExpiring`,
    summary_key: `${DIRECTORY_SUPPLIER_PREFIX}.supplierContractExpiring`,
    default_severity: "medium",
    source_tier: "exact_live",
    related_capability: "supplier_contract_status.read",
    required_permission: "contracts.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/procurement",
      capability_key: "supplier_contract_status.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("supplier_contract_expired", {
    signal_type: "attention",
    category: "directory_supplier",
    title_key: `${DIRECTORY_SUPPLIER_PREFIX}.supplierContractExpired`,
    summary_key: `${DIRECTORY_SUPPLIER_PREFIX}.supplierContractExpired`,
    default_severity: "high",
    source_tier: "exact_live",
    related_capability: "supplier_contract_status.read",
    required_permission: "contracts.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/procurement",
      capability_key: "supplier_contract_status.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("supplier_delivery_delay", {
    signal_type: "attention",
    category: "directory_supplier",
    title_key: `${DIRECTORY_SUPPLIER_PREFIX}.supplierDeliveryDelay`,
    summary_key: `${DIRECTORY_SUPPLIER_PREFIX}.supplierDeliveryDelay`,
    default_severity: "medium",
    source_tier: "compatible_live",
    related_capability: "supplier_performance.read",
    required_permission: "procurement.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/procurement",
      capability_key: "supplier_performance.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("supplier_performance_warning", {
    signal_type: "risk",
    category: "directory_supplier",
    title_key: `${DIRECTORY_SUPPLIER_PREFIX}.supplierPerformanceWarning`,
    summary_key: `${DIRECTORY_SUPPLIER_PREFIX}.supplierPerformanceWarning`,
    default_severity: "medium",
    source_tier: "compatible_live",
    related_capability: "supplier_performance.read",
    required_permission: "supplier_performance.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/procurement/suppliers",
      capability_key: "supplier_performance.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("supplier_risk_attention", {
    signal_type: "risk",
    category: "directory_supplier",
    title_key: `${DIRECTORY_SUPPLIER_PREFIX}.supplierRiskAttention`,
    summary_key: `${DIRECTORY_SUPPLIER_PREFIX}.supplierRiskAttention`,
    default_severity: "high",
    source_tier: "compatible_live",
    related_capability: "supplier_performance.read",
    required_permission: "supplier_performance.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/procurement/suppliers",
      capability_key: "supplier_performance.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("supplier_contact_missing", {
    signal_type: "attention",
    category: "directory_supplier",
    title_key: `${DIRECTORY_SUPPLIER_PREFIX}.supplierContactMissing`,
    summary_key: `${DIRECTORY_SUPPLIER_PREFIX}.supplierContactMissing`,
    default_severity: "low",
    source_tier: "exact_live",
    related_capability: "supplier_contact.read",
    required_permission: "suppliers.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/procurement/suppliers",
      capability_key: "supplier_contact.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("supplier_without_category", {
    signal_type: "attention",
    category: "directory_supplier",
    title_key: `${DIRECTORY_SUPPLIER_PREFIX}.supplierWithoutCategory`,
    summary_key: `${DIRECTORY_SUPPLIER_PREFIX}.supplierWithoutCategory`,
    default_severity: "low",
    source_tier: "exact_live",
    related_capability: "supplier_category.read",
    required_permission: "suppliers.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/procurement/suppliers",
      capability_key: "supplier_category.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("duplicate_supplier_candidate", {
    signal_type: "attention",
    category: "directory_supplier",
    title_key: `${DIRECTORY_SUPPLIER_PREFIX}.duplicateSupplierCandidate`,
    summary_key: `${DIRECTORY_SUPPLIER_PREFIX}.duplicateSupplierCandidate`,
    default_severity: "low",
    source_tier: "exact_live",
    related_capability: "supplier.search",
    required_permission: "suppliers.view",
    related_action: {
      kind: "view_details",
      label_key: `${CORE_PREFIX}.actions.viewDetails`,
      href: "/app/procurement/suppliers",
      capability_key: "supplier.search",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("preferred_supplier_unavailable", {
    signal_type: "attention",
    category: "directory_supplier",
    title_key: `${DIRECTORY_SUPPLIER_PREFIX}.preferredSupplierUnavailable`,
    summary_key: `${DIRECTORY_SUPPLIER_PREFIX}.preferredSupplierUnavailable`,
    default_severity: "medium",
    source_tier: "compatible_live",
    related_capability: "supplier.read",
    required_permission: "suppliers.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/procurement/suppliers",
      capability_key: "supplier.read",
      executable: true,
    },
    panels: ["app"],
  }),
  signal("purchase_order_attention", {
    signal_type: "attention",
    category: "directory_supplier",
    title_key: `${DIRECTORY_SUPPLIER_PREFIX}.purchaseOrderAttention`,
    summary_key: `${DIRECTORY_SUPPLIER_PREFIX}.purchaseOrderAttention`,
    default_severity: "medium",
    source_tier: "compatible_live",
    related_capability: "supplier.read",
    required_permission: "procurement.view",
    related_action: {
      kind: "open_page",
      label_key: `${CORE_PREFIX}.actions.openDetails`,
      href: "/app/procurement",
      capability_key: "supplier.read",
      executable: true,
    },
    panels: ["app"],
  }),
];

const catalogByKey = new Map(
  COMMAND_BRIEF_SIGNAL_CATALOG.map((entry) => [entry.signal_key, entry]),
);

export function getCommandBriefCatalogEntry(signalKey: string): CommandBriefCatalogEntry | null {
  return catalogByKey.get(signalKey) ?? null;
}

export function listCommandBriefCatalogKeys(): readonly string[] {
  return COMMAND_BRIEF_SIGNAL_CATALOG.map((entry) => entry.signal_key);
}

...
import {
  runP1_06LiveAppSupportCaseWriteE2eCertification,
  writeP1_06LiveAppSupportCaseWriteE2eCertificationArtifact,
  P1_06_LIVE_E2E_ARTIFACT_FILENAME,
  buildBlockedP1_06LiveAppSupportCaseWriteE2eArtifact,
  readP1_06LiveAppSupportCaseWriteE2eCertificationArtifact,
} from "@/lib/companion-runtime/p1-06-live-app-support-case-write-e2e-certification";
import { P1_06_LIVE_E2E_MODULE_REQUIREMENTS } from "@/lib/companion-runtime/p1-06-live-app-support-case-write-e2e-coverage";
import {
  P1_06_ASSIGN_WRITE_SOURCE,
  P1_06_ESCALATE_WRITE_SOURCE,
  P1_06_LIVE_E2E_CERTIFICATION_VERSION,
} from "@/lib/companion-runtime/p1-06-live-app-support-case-write-e2e-types";
...
async function main() {
  ...
  if (p1_01?.overall_status !== "pass") {
    console.log("phase-p1-06.test.ts blocked — P1.01 prerequisite artifact is not pass.");
    process.exit(0);
  }

  const { config } = resolveP1LiveE2eConfig();
  const artifact = config
    ? await runP1_06LiveAppSupportCaseWriteE2eCertification(repoRoot)
    : buildBlockedP1_06LiveAppSupportCaseWriteE2eArtifact();

  assert.equal(artifact.version, P1_06_LIVE_E2E_CERTIFICATION_VERSION);
  assert.equal(artifact.assign_write_source, P1_06_ASSIGN_WRITE_SOURCE);
  assert.equal(artifact.escalate_write_source, P1_06_ESCALATE_WRITE_SOURCE);
  const artifactPath = writeP1_06LiveAppSupportCaseWriteE2eCertificationArtifact(artifact, repoRoot);
  assert.ok(fs.existsSync(artifactPath));
  assert.equal(path.basename(artifactPath), P1_06_LIVE_E2E_ARTIFACT_FILENAME);
  ...
  if (!config) {
    ...
    console.log("phase-p1-06.test.ts blocked — live E2E environment not configured.");
    process.exit(0);
  }

  if (artifact.overall_status === "blocked") {
    console.log("phase-p1-06.test.ts blocked — prerequisite or authenticated session unavailable.");
  ...
  assert.equal(artifact.controlled_probe_case_created, true);
  ...
  assert.ok(
    artifact.idempotency.some((check) => check.check_id === "duplicate_assign_retry"),
    "missing idempotency check duplicate_assign_retry",
  );
  ...
  if (artifact.overall_status !== "pass") {
    ...
    console.error("phase-p1-06.test.ts failed — live support case write certification did not pass.");
  ...
  const writeModule = registry.find((row) => row.module_id === "support.case_write");
  assert.ok(writeModule, "missing support.case_write registry entry");
  assert.equal(writeModule!.readiness, "production_ready_candidate");
  assert.ok(artifact.coverage_updates_applied.includes("support.case_write"));

  const persisted = readP1_06LiveAppSupportCaseWriteE2eCertificationArtifact(repoRoot);
  assert.ok(persisted);
  assert.equal(persisted!.overall_status, "pass");

  console.log("phase-p1-06.test.ts passed");
  console.log(`  environment: ${artifact.environment}`);
  console.log(`  organization_reference: ${artifact.organization_reference}`);
  console.log(`  assign_write_source: ${artifact.assign_write_source}`);
  console.log(`  escalate_write_source: ${artifact.escalate_write_source}`);
  console.log(`  controlled_probe_case_created: ${artifact.controlled_probe_case_created}`);

...
const WORKSPACE_PACK_KEY = "workspace_provider";

export function mapWorkspaceCapabilitiesToRefs(
  workspaceContext: CompanionWorkspaceContext,
): CompanionCapabilityRef[] {
  return filterWorkspaceCapabilitiesForPrivacy(workspaceContext).map((capability) => ({
...
export function mergeWorkspaceCapabilities(
  ...
  const workspaceRefs = mapWorkspaceCapabilitiesToRefs(workspaceContext);
...
export function buildWorkspaceSchemaEntities(
  workspaceContext: CompanionWorkspaceContext,
  effectivePermissions: string[],
): CompanionSchemaContext[] {
  if (workspaceContext.permission_denied || workspaceContext.app_entitlement_blocked) {
    return [];
  }

  const entities: CompanionSchemaContext[] = [];

  for (const capability of filterWorkspaceCapabilitiesForPrivacy(workspaceContext)) {
    ...
    entities.push({
      ...
      field_types: { [capability.capability_key]: "workspace_capability" },
      ...
      source_provider: capability.provider_key,
      pack_key: WORKSPACE_PACK_KEY,
      schema_version: "workspace_provider:v1",
      freshness: "unknown",
...
export function mergeWorkspaceSchemaCollection(
  collection: CompanionSchemaCollection,
  workspaceContext: CompanionWorkspaceContext,
  effectivePermissions: string[],
): CompanionSchemaCollection {
  const workspaceEntities = buildWorkspaceSchemaEntities(workspaceContext, effectivePermissions);
  if (workspaceEntities.length === 0) return collection;
...
export function buildWorkspaceReadToolDefinitions(input: {
  workspaceContext: CompanionWorkspaceContext;
  effectivePermissions: string[];
}): CompanionToolDefinition[] {
  const tools: CompanionToolDefinition[] = [];

  for (const capability of filterWorkspaceCapabilitiesForPrivacy(input.workspaceContext)) {
    if (capability.operation !== "read") continue;
    ...
    tools.push({
      ...
      source_label: `workspace:${capability.provider_key}`,
      freshness: "unknown",
      ...
      enabled:
        ...
        !input.workspaceContext.app_entitlement_blocked,
    });
  }
...
export function mergeWorkspaceToolRegistry(
  registry: CompanionToolRegistry,
  workspaceContext: CompanionWorkspaceContext,
  effectivePermissions: string[],
): CompanionToolRegistry {
  const workspaceTools = buildWorkspaceReadToolDefinitions({
    workspaceContext,
    effectivePermissions,
  });

  if (workspaceTools.length === 0) return registry;
...
export function buildWorkspaceActionDefinitions(input: {
  workspaceContext: CompanionWorkspaceContext;
  effectivePermissions: string[];
...
export function mergeWorkspaceActionDefinitions(
  ...
  const workspaceActions = buildWorkspaceActionDefinitions({
    workspaceContext,
    ...input,
  });
  if (workspaceActions.length === 0) return actions;

...
import {
  ...
  buildFinanceProviderUnavailableAnswer,
  hasBlockedFinanceOperationIntent,
  hasExternalFinanceAdapterIntent,
...
import {
  ...
  buildSecurityProviderUnavailableAnswer,
  hasBlockedSecurityOperationIntent,
  hasExternalSecurityAdapterIntent,
...
function resolveFinanceProviderAnswer(
  query: string,
  t: Translator,
  tenantContext: CompanionTenantContext,
): PlatformSearchResult | null {
  if (hasBlockedFinanceOperationIntent(query)) {
    return {
      answer: buildBlockedFinanceOperationAnswer(t),
    };
  }

  if (!hasFinanceProviderIntent(query)) {
    return null;
  }

  if (tenantContext.financeContext.permission_denied) {
    return {
      answer: buildFinanceProviderUnavailableAnswer(t, tenantContext.financeContext),
    };
  }

  if (hasExternalFinanceAdapterIntent(query)) {
    return {
      answer: buildExternalFinanceUnavailableAnswer(t),
    };
  }

  const match = matchFinanceProviderQuery(query, tenantContext);
  if (!match) {
    return {
      answer: buildFinanceProviderUnavailableAnswer(t, tenantContext.financeContext),
    };
  ...
  return {
    answer: buildFinanceProviderDiscoveryAnswer(match, tenantContext.financeContext, t),
  };
}
...
function resolveSecurityProviderAnswer(
  query: string,
  t: Translator,
  tenantContext: CompanionTenantContext,
): PlatformSearchResult | null {
  if (hasBlockedSecurityOperationIntent(query)) {
    return {
      answer: buildBlockedSecurityOperationAnswer(t),
    };
  }

  if (!hasSecurityProviderIntent(query)) {
    return null;
  }

  if (tenantContext.securityContext.permission_denied) {
    return {
      answer: buildSecurityProviderUnavailableAnswer(t, tenantContext.securityContext),
    };
  }

  if (hasExternalSecurityAdapterIntent(query)) {
    return {
      answer: buildExternalSecurityUnavailableAnswer(t),
    };
  }

  const match = matchSecurityProviderQuery(query, tenantContext);
  if (!match) {
    return {
      answer: buildSecurityProviderUnavailableAnswer(t, tenantContext.securityContext),
    };
  ...
  return {
    answer: buildSecurityProviderDiscoveryAnswer(match, tenantContext.securityContext, t),
  };
}

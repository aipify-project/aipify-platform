...
const SERVICES_PACK_KEY = "services_provider";

export function mapServicesCapabilitiesToRefs(
  servicesContext: CompanionServicesContext,
): CompanionCapabilityRef[] {
  return filterServicesCapabilitiesForPrivacy(servicesContext).map((capability) => ({
    ...
    pack_key: SERVICES_PACK_KEY,
  }));
}

export function mergeServicesCapabilities(
  entitledCapabilities: CompanionCapabilityRef[],
  servicesContext: CompanionServicesContext,
): CompanionCapabilityRef[] {
  const servicesRefs = mapServicesCapabilitiesToRefs(servicesContext);
  const seen = new Set(entitledCapabilities.map((capability) => capability.capability_id));
  const merged = [...entitledCapabilities];
  for (const ref of servicesRefs) {
...
export function buildServicesSchemaEntities(
  servicesContext: CompanionServicesContext,
  effectivePermissions: string[],
): CompanionSchemaContext[] {
  if (servicesContext.permission_denied || servicesContext.app_entitlement_blocked) {
    return [];
  }

  const entities: CompanionSchemaContext[] = [];

  for (const capability of filterServicesCapabilitiesForPrivacy(servicesContext)) {
    ...
    entities.push({
      ...
      field_types: { [capability.capability_key]: "services_capability" },
      ...
      source_provider: capability.provider_key,
      pack_key: SERVICES_PACK_KEY,
      schema_version: "services_provider:v1",
      freshness: "unknown",
...
export function mergeServicesSchemaCollection(
  collection: CompanionSchemaCollection,
  servicesContext: CompanionServicesContext,
  effectivePermissions: string[],
): CompanionSchemaCollection {
  const servicesEntities = buildServicesSchemaEntities(servicesContext, effectivePermissions);
  if (servicesEntities.length === 0) return collection;

  const entityMap = new Map(collection.entities.map((entity) => [entity.entity_key, entity]));
  for (const entity of servicesEntities) {
...
export function buildServicesReadToolDefinitions(input: {
  servicesContext: CompanionServicesContext;
  effectivePermissions: string[];
}): CompanionToolDefinition[] {
  const tools: CompanionToolDefinition[] = [];

  for (const capability of filterServicesCapabilitiesForPrivacy(input.servicesContext)) {
    if (capability.operation !== "read") continue;
    ...
    tools.push({
      ...
      source_label: `services:${capability.provider_key}`,
      freshness: "unknown",
      ...
      enabled:
        ...
        !input.servicesContext.app_entitlement_blocked,
    });
  }
...
export function mergeServicesToolRegistry(
  registry: CompanionToolRegistry,
  servicesContext: CompanionServicesContext,
  effectivePermissions: string[],
): CompanionToolRegistry {
  const servicesTools = buildServicesReadToolDefinitions({
    servicesContext,
    effectivePermissions,
  });

  if (servicesTools.length === 0) return registry;

  const tools = [...registry.tools];
  const seen = new Set(tools.map((tool) => tool.tool_id));
  for (const tool of servicesTools) {
...
export function buildServicesActionDefinitions(input: {
  servicesContext: CompanionServicesContext;
  effectivePermissions: string[];
...
export function mergeServicesActionDefinitions(
  ...
  const servicesActions = buildServicesActionDefinitions({
    servicesContext,
    ...input,
  });
  if (servicesActions.length === 0) return actions;

  const seen = new Set(actions.map((action) => action.action_id));
  const merged = [...actions];
  for (const action of servicesActions) {

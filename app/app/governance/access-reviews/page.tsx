...
export default async function GovernanceAccessReviewsPage() {
  const locale = await getLocale();
  const dict = await getCustomerAppDictionaryForSplits(locale, ["settings"]);
  const t = createTranslator(dict);
  const labels = buildGovernanceManagementLabels(t);
  return <GovernanceManagementPanel labels={labels} initialTab="accessReviews" titleOverride={labels.accessReviewsTitle} />;
}

...
export default async function DesktopFilesPage() {
  const labels = await getDesktopCompanionPageLabels();
  return (
    <DesktopCompanionFoundationShell labels={labels}>
      <section className="rounded-xl border border-gray-200 bg-white p-6">
        <h1 className="text-xl font-semibold">{labels["nav.files"]}</h1>
        <p className="mt-2 text-sm text-gray-600">{labels.permissionFiles}</p>
      </section>

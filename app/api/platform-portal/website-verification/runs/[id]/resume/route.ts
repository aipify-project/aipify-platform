...
import { resumeWebsiteReleaseVerificationRun } from "@/lib/website-staging-verification/runs";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await resumeWebsiteReleaseVerificationRun(supabase, runId);
    if (!result.ok) {
      const mapped = mapWebsiteStagingRpcError(result.rawMessage);
      return jsonError("Unable to resume the verification run.", mapped.status, mapped.code);
    }

    return NextResponse.json(result.value, { headers: NO_STORE });
  } catch {
    return jsonError("Unable to resume the verification run.", 500, "unknown");
  }
}

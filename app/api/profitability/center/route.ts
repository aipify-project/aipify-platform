...
export async function GET(request: Request) {
  try {
    ...
    const { data, error } = await supabase.rpc("get_organization_profitability_center", {
      p_section: section,
    });
    ...
    const message = err instanceof Error ? err.message : "Failed to load Profitability Center";
    const access_state = isDatabaseExecutionError(message)
      ? "database_execution_error"
      : classifyAppPortalError(message);
    console.error("[profitability/center]", message);
    return NextResponse.json(

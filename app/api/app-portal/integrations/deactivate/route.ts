...
export async function POST(request: Request) {
  try {
    ...
    const { data, error } = await supabase.rpc("deactivate_app_portal_integration_connection", {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: "Deactivation could not be completed" }, { status, headers: NO_STORE });
    }

    revalidateAppPortalIntegrationSurfaces();
    return NextResponse.json(data, { headers: NO_STORE });
  } catch {
    return NextResponse.json({ error: "Deactivation could not be completed" }, { status: 500, headers: NO_STORE });
  }
}

...
export async function GET() {
  try {
    ...
    const { data, error } = await supabase.rpc("get_real_world_actions_execution_center");
...
export async function POST(request: Request) {
  try {
    ...
    if (body.action === "manage") {
      ...
      const { data, error } = await supabase.rpc("manage_real_world_actions_execution_item", {

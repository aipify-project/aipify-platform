  ...
  try {
    ...
    if (!passwordOk) {
      await logTwoFactorAuditEvent(
        supabase,
        "verification_failed",
        "Sign-in verification setup blocked — invalid password",
    ...
    if (secretError || !encrypted) {
      return NextResponse.json({ error: "noPendingEnrollment" }, { status: 400 });
    }
    ...
    if (!(await verifyTotpCode(secret, code))) {
      await logTwoFactorAuditEvent(
        supabase,
        "verification_failed",
        "Sign-in verification setup failed — invalid authenticator code",
    ...
    const { data, error } = await supabase.rpc("confirm_two_factor_enrollment", {

...
import {
  createOrganizationProviderAccessRequest,
  getOrganizationAccessApprovalCenter,
...
export async function GET(request: Request) {
  try {
    ...
    const center = await getOrganizationAccessApprovalCenter(supabase, status);
    ...
    console.error("[app/organization-access/requests]", message);
...
export async function POST(request: Request) {
  try {
    ...
    const created = await createOrganizationProviderAccessRequest(supabase, {
      provider_key: providerKey,
    ...
    const access_state = isDatabaseExecutionError(message)
      ? "database_execution_error"
      : classifyAppPortalError(message);
    console.error("[app/organization-access/requests POST]", message);
    return NextResponse.json(
      { error: message, access_state },
      { status: rpcErrorStatus(message, access_state) },
    );

...
export async function GET(
  ...
  try {
    ...
    const { data, error } = await supabase.rpc("get_app_kompis_operator_run", { p_run_id: id });
    if (error) {
      const mapped = mapKompisOperatorRpcError(error.message);
      return NextResponse.json(
        { error: "Unable to load run.", code: mapped.code },
        { status: mapped.status, headers },
      );
    }
    return NextResponse.json(data, { headers });
  } catch {
    return NextResponse.json({ error: "Unable to load run.", code: "unknown" }, { status: 500, headers });
  }
}

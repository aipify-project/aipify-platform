...
export async function GET(request: Request) {
  try {
    ...
    const { data, error } = await supabase.rpc("get_platform_business_pack_runtime_center", {
      p_section: section,
    });
    ...
    return NextResponse.json({ error: "Failed to load Business Pack Runtime Center" }, { status: 500 });
  }
}

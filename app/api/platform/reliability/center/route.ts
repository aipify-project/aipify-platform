...
export async function GET(request: Request) {
  try {
    ...
    const { data, error } = await supabase.rpc("get_platform_reliability_center", { p_section: section });
    ...
    return NextResponse.json({ error: "Failed to load Reliability Center" }, { status: 500 });
  }
}

...
const block = en.portalStructure.strategyExecution;
const navVal = en.portalStructure.nav.strategyExecution;

for (const loc of ["no", "sv", "da", "es", "pl", "uk"]) {
  const file = path.join(root, `locales/${loc}/customerApp.json`);
  const data = JSON.parse(fs.readFileSync(file, "utf8"));
  data.portalStructure.nav.strategyExecution = navVal;
  data.portalStructure.strategyExecution = block;

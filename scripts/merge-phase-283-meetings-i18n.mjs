...
const block = en.portalStructure.meetings;
const navVal = en.portalStructure.nav.meetings;

for (const loc of ["no", "sv", "da", "es", "pl", "uk"]) {
  const file = path.join(root, `locales/${loc}/customerApp.json`);
  const data = JSON.parse(fs.readFileSync(file, "utf8"));
  data.portalStructure.nav.meetings = navVal;
  data.portalStructure.meetings = block;

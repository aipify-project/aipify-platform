...
const block = en.portalStructure.playbooks;
const navVal = en.portalStructure.nav.playbooks;

for (const loc of ["no", "sv", "da", "es", "pl", "uk"]) {
  const file = path.join(root, `locales/${loc}/customerApp.json`);
  const data = JSON.parse(fs.readFileSync(file, "utf8"));
  data.portalStructure.nav.playbooks = navVal;
  data.portalStructure.playbooks = block;

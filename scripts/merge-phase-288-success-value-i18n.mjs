...
const block = en.portalStructure.successValue;
const navVal = en.portalStructure.nav.successValue;

for (const loc of ["no", "sv", "da", "es", "pl", "uk"]) {
  const file = path.join(root, `locales/${loc}/customerApp.json`);
  const data = JSON.parse(fs.readFileSync(file, "utf8"));
  data.portalStructure.nav.successValue = navVal;
  data.portalStructure.successValue = block;

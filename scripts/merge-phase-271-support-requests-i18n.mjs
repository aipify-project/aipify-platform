...
const block = en.portalStructure.supportRequests;
const navVal = en.portalStructure.nav.supportRequests;

for (const loc of ["no", "sv", "da", "es", "pl", "uk"]) {
  const file = path.join(root, `locales/${loc}/customerApp.json`);
  const data = JSON.parse(fs.readFileSync(file, "utf8"));
  data.portalStructure.nav.supportRequests = navVal;
  data.portalStructure.supportRequests = block;

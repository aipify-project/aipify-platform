...
const block = en.portalStructure.gettingStarted;
const navVal = en.portalStructure.nav.gettingStarted;

for (const loc of ["no", "sv", "da", "es", "pl", "uk"]) {
  const file = path.join(root, `locales/${loc}/customerApp.json`);
  const data = JSON.parse(fs.readFileSync(file, "utf8"));
  data.portalStructure.nav.gettingStarted = navVal;
  data.portalStructure.gettingStarted = block;

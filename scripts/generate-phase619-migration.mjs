#!/usr/bin/env node
/**
 * Generates Phase 619 migration SQL (compact Services V1 pattern).
 */
...
const out = path.join(
  process.cwd(),
  "supabase/migrations/20261861900000_customer_communication_rebooking_reviews_retention_service_quality_engine_phase619.sql"
);

const recordTable = (name, domain) =>
  `create table if not exists public.organization_sx619_${name} (id uuid primary key default gen_random_uuid(), organization_id uuid not null references public.organizations(id) on delete cascade, record_key text not null, record_title text not null, record_status text not null default 'active', status_icon text not null default 'circle', status_label text not null default 'Active', domain_key text not null default '${domain}', booking_key text not null default '', customer_label text not null default '', location_label text not null default '', provider_label text not null default '', service_label text not null default '', channel_key text not null default '', category_key text not null default '', communication_status text not null default '', direction_key text not null default 'outbound', rating integer, severity_key text not null default '', scheduled_at timestamptz, sent_at timestamptz, summary text not null default '', metadata jsonb not null default '{}'::jsonb, created_at timestamptz not null default now(), unique (organization_id, record_key)); create index if not exists idx_organization_sx619_${name}_org on public.organization_sx619_${name}(organization_id); create index if not exists idx_organization_sx619_${name}_org_status on public.organization_sx619_${name}(organization_id, communication_status, scheduled_at); alter table public.organization_sx619_${name} enable row level security; revoke all on public.organization_sx619_${name} from authenticated, anon;`;

const sections = [
  ["communications_timeline", 1, "communications", "Unified Services communication timeline"],
  ["communication_deliveries", 2, "communications", "Channel delivery attempts"],
  ["communication_templates", 3, "communications", "Versioned communication templates"],
  ["communication_template_versions", 4, "communications", "Published immutable template versions"],
  ["communication_preferences", 5, "communications", "Customer communication preferences"],
  ["communication_suppressions", 6, "communications", "Suppression rules and outcomes"],
  ["rebooking_recommendations", 7, "rebooking", "Governed rebooking recommendations"],
  ["rebooking_actions", 8, "rebooking", "Rebooking actions and reminders"],
  ["feedback_records", 9, "feedback", "Private customer feedback"],
  ["service_recovery_records", 10, "service_recovery", "Service recovery workflow"],
  ["review_requests", 11, "reviews", "Optional public review requests"],
  ["quality_alerts", 12, "quality", "Operational quality alerts"],
  ["quality_snapshots", 13, "quality", "Aggregated quality snapshots"],
  ["phase616_location_connection", 14, "integration", "Phase 616 multi-location scope"],
  ["phase617_payment_connection", 15, "integration", "Phase 617 payment reminder state"],
  ["phase618_intake_connection", 16, "integration", "Phase 618 preparation and aftercare"],
  ["phase611_crm_connection", 17, "integration", "Phase 611 CRM rebooking extension"],
  ["companion_advisor", 18, "companion_advisor", "Companion Service Experience Advisor"],
  ["mobile_summary", 19, "mobile_summary", "Mobile communication summary"],
];

const sectionInserts = sections
  .map(
    ([key, num, domain, title]) =>
      `insert into public.sx619_section_defs values ('${key}', ${num}, '${domain}', '${title.replace(/'/g, "''")}', '${title.replace(/'/g, "''")}') on conflict do nothing;`
  )
  .join("\n");
...
const tableNames = [
  ...
  ["communication_template_versions", "communication_template_versions"],
  ["communication_preferences", "communication_preferences"],
  ["communication_suppressions", "communication_suppressions"],
  ["rebooking_recommendations", "rebooking_recommendations"],
  ["rebooking_actions", "rebooking_actions"],
  ["feedback_records", "feedback_records"],
  ["service_recovery_records", "service_recovery_records"],
  ["review_requests", "review_requests"],
  ["quality_alerts", "quality_alerts"],
  ["quality_snapshots", "quality_snapshots"],
  ["phase616_location_connection", "phase616_location_connection"],
  ["phase617_payment_connection", "phase617_payment_connection"],
  ["phase618_intake_connection", "phase618_intake_connection"],
  ["phase611_crm_connection", "phase611_crm_connection"],
  ["companion_advisor", "companion_advisor"],
  ["mobile_summary", "mobile_summary"],
];

const sql = `-- Phase 619 — Customer Communication, Rebooking, Reviews, Retention & Service Quality Engine
...
${tableNames.map(([n, d]) => recordTable(n, d)).join("\n\n")}

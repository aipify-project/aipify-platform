...
const block = en.portalStructure.customerSuccess;
const navVal = en.portalStructure.nav.customerSuccess;

for (const loc of ["no", "sv", "da", "es", "pl", "uk"]) {
  const file = path.join(root, `locales/${loc}/customerApp.json`);
  const data = JSON.parse(fs.readFileSync(file, "utf8"));
  data.portalStructure.nav.customerSuccess = navVal;
  data.portalStructure.customerSuccess = block;

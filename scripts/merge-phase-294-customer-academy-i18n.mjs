...
const block = en.portalStructure.customerAcademy;
const navVal = en.portalStructure.nav.customerAcademy;

for (const loc of ["no", "sv", "da", "es", "pl", "uk"]) {
  const file = path.join(root, `locales/${loc}/customerApp.json`);
  const data = JSON.parse(fs.readFileSync(file, "utf8"));
  data.portalStructure.nav.customerAcademy = navVal;
  data.portalStructure.customerAcademy = block;

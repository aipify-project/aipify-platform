...
const block = en.portalStructure.resilience;
const navVal = en.portalStructure.nav.resilience;

for (const loc of ["no", "sv", "da", "es", "pl", "uk"]) {
  const file = path.join(root, `locales/${loc}/customerApp.json`);
  const data = JSON.parse(fs.readFileSync(file, "utf8"));
  data.portalStructure.nav.resilience = navVal;
  data.portalStructure.resilience = block;

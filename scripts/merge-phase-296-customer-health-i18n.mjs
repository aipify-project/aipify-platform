...
const block = en.portalStructure.customerHealth;
const navVal = en.portalStructure.nav.customerHealth;

for (const loc of ["no", "sv", "da", "es", "pl", "uk"]) {
  const file = path.join(root, `locales/${loc}/customerApp.json`);
  const data = JSON.parse(fs.readFileSync(file, "utf8"));
  data.portalStructure.nav.customerHealth = navVal;
  data.portalStructure.customerHealth = block;

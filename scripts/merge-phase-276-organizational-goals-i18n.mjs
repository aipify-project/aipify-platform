...
const block = en.portalStructure.organizationalGoals;
const navVal = en.portalStructure.nav.goalsObjectives;

for (const loc of ["no", "sv", "da", "es", "pl", "uk"]) {
  const file = path.join(root, `locales/${loc}/customerApp.json`);
  const data = JSON.parse(fs.readFileSync(file, "utf8"));
  data.portalStructure.nav.goalsObjectives = navVal;
  data.portalStructure.organizationalGoals = block;

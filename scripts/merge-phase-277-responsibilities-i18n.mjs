...
const block = en.portalStructure.responsibilities;
const navVal = en.portalStructure.nav.responsibilities;

for (const loc of ["no", "sv", "da", "es", "pl", "uk"]) {
  const file = path.join(root, `locales/${loc}/customerApp.json`);
  const data = JSON.parse(fs.readFileSync(file, "utf8"));
  data.portalStructure.nav.responsibilities = navVal;
  data.portalStructure.responsibilities = block;

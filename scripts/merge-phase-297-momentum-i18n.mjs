...
const block = en.portalStructure.momentum;
const navVal = en.portalStructure.nav.momentum;

for (const loc of ["no", "sv", "da", "es", "pl", "uk"]) {
  const file = path.join(root, `locales/${loc}/customerApp.json`);
  const data = JSON.parse(fs.readFileSync(file, "utf8"));
  data.portalStructure.nav.momentum = navVal;
  data.portalStructure.momentum = block;

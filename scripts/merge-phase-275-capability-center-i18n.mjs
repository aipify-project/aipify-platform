...
const block = en.portalStructure.capabilityCenter;
const navVal = en.portalStructure.nav.capabilityCenter;

for (const loc of ["no", "sv", "da", "es", "pl", "uk"]) {
  const file = path.join(root, `locales/${loc}/customerApp.json`);
  const data = JSON.parse(fs.readFileSync(file, "utf8"));
  data.portalStructure.nav.capabilityCenter = navVal;
  data.portalStructure.capabilityCenter = block;

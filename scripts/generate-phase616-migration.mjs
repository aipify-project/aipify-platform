...
const out = path.join(
  root,
  "supabase/migrations/20261861600000_multi_location_chair_room_rental_service_network_engine_phase616.sql",
);

/** 22 sections — settings + 20 domains + audit_logs */
const SECTIONS = [
  ["settings", "Service network engine settings"],
  ["service_networks", "APP organization service network overview"],
  ["service_locations", "Multi-location branches and salons"],
  ["location_groups", "Optional location groupings for filtering and reporting"],
  ["service_resources", "Shared resource engine — chairs, rooms, workstations"],
  ["service_providers", "Employees, renters, and governed external providers"],
  ["rental_agreements", "Chair and room rental operational agreements"],
  ["provider_assignments", "Provider to location, resource, and service assignments"],
  ["location_services", "Location-specific service availability and overrides"],
  ["availability_rules", "Multi-location availability validation rules"],
  ["resource_blocks", "Resource maintenance and manual blocks"],
  ["location_hours", "Location opening hours"],
  ["location_closures", "Location closure periods"],
  ["customer_access_rules", "Renter customer visibility governance"],
  ["conflict_prevention", "Provider and resource double-booking prevention"],
  ["booking_validation", "Server-side booking validation metadata"],
  ["permission_scopes", "Location and renter permission scopes"],
  ["phase610_booking_connection", "Phase 610 booking engine — extend, do not duplicate"],
  ["phase606_absence_connection", "Phase 606 absence and vacation mode integration"],
  ["phase615_profitability_connection", "Phase 615 room and chair cost signals — interfaces only"],
  ["companion_advisor", "Companion Service Network Advisor metadata"],
  ["mobile_summary", "Mobile service network summary"],
  ["integration_hub", "Cross-phase integration hub references"],
  ["audit_logs", "Service network audit log"],
];

if (SECTIONS.length !== 24) {
  console.error(`Expected 24 sections, got ${SECTIONS.length}`);
  process.exit(1);
}

const UI_SECTION_MAP = {
  network: "service_networks",
...
let sql = `-- Phase 616 — Multi-Location, Chair Rental, Room Rental & Service Network Engine
...
SECTIONS.forEach(([key, desc]) => {
  ...
  sql += `insert into public.net616_section_defs (section_key, section_number, domain_key, section_title, summary) values
...
sql += `create table if not exists public.net616_status_defs (
...
for (const [key, desc] of SECTIONS) {
  ...
  const table = `organization_net616_${key}`;
  sql += `-- ${desc}
...
create index if not exists idx_${table}_org_status on public.${table} (organization_id, record_status);
create index if not exists idx_${table}_org_location on public.${table} (organization_id, location_key) where location_key <> '';
...
sql += `create table if not exists public.organization_net616_audit_logs (
  id uuid primary key default gen_random_uuid(),
  organization_id uuid not null references public.organizations (id) on delete cascade,
...
sql += `create or replace function public._net616_seed(p_org_id uuid)
returns void language plpgsql security definer set search_path = public as $$
begin
...
for (const [key] of SECTIONS) {
  ...
  sql += `  insert into public.${table} (
    ...
    p_org_id, '${seedKey}_base', initcap(replace('${key}', '_', ' ')), 'active', 'circle', 'Active', '${key}', 'organization',
    'Baseline seed — Phase 616 ${key}.', '{}'::jsonb
  ) on conflict (organization_id, record_key) do nothing;

`;
}

sql += `  insert into public.organization_net616_service_networks (
...
sql += `create or replace function public._net616_section_rows(p_org_id uuid, p_domain text)
...
const rowsCase = Object.entries(UI_SECTION_MAP)
  .map(([ui, domain]) => `    when '${ui}' then v_rows := public._net616_section_rows(v_org_id, '${domain}');`)
  .join("\n");

sql += `create or replace function public.get_organization_service_network_center(p_section text default 'overview')
returns jsonb language plpgsql stable security definer set search_path = public as $$
declare
  v_org_id uuid;

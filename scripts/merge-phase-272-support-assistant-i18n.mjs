...
const block = en.portalStructure.supportAssistant;
const navVal = en.portalStructure.nav.supportAssistant;

for (const loc of ["no", "sv", "da", "es", "pl", "uk"]) {
  const file = path.join(root, `locales/${loc}/customerApp.json`);
  const data = JSON.parse(fs.readFileSync(file, "utf8"));
  data.portalStructure.nav.supportAssistant = navVal;
  data.portalStructure.supportAssistant = block;

...
const block = en.portalStructure.capacityWorkload;
const navVal = en.portalStructure.nav.capacityWorkload;

for (const loc of ["no", "sv", "da", "es", "pl", "uk"]) {
  const file = path.join(root, `locales/${loc}/customerApp.json`);
  const data = JSON.parse(fs.readFileSync(file, "utf8"));
  data.portalStructure.nav.capacityWorkload = navVal;
  data.portalStructure.capacityWorkload = block;

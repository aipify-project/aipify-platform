...
const block = en.portalStructure.communications;
const navVal = en.portalStructure.nav.communications;

for (const loc of ["no", "sv", "da", "es", "pl", "uk"]) {
  const file = path.join(root, `locales/${loc}/customerApp.json`);
  const data = JSON.parse(fs.readFileSync(file, "utf8"));
  data.portalStructure.nav.communications = navVal;
  data.portalStructure.communications = block;

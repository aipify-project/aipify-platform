-- P1.05 — Support SLA exact source on ASO (no parallel SLA engine)
-- Extends aso_settings + support_cases + get_customer_support_operations_center.

alter table public.aso_settings
  add column if not exists sla_policy_enabled boolean not null default false,
  add column if not exists sla_first_response_minutes integer,
  add column if not exists sla_resolution_minutes integer,
  add column if not exists sla_warning_minutes_before_due integer,
  add column if not exists sla_at_risk_minutes_before_due integer;

do $$
begin
  if not exists (
    select 1 from pg_constraint where conname = 'aso_settings_sla_first_response_minutes_check'
  ) then
    alter table public.aso_settings
      add constraint aso_settings_sla_first_response_minutes_check
      check (sla_first_response_minutes is null or sla_first_response_minutes > 0);
  end if;
  if not exists (
    select 1 from pg_constraint where conname = 'aso_settings_sla_resolution_minutes_check'
  ) then
    alter table public.aso_settings
      add constraint aso_settings_sla_resolution_minutes_check
      check (sla_resolution_minutes is null or sla_resolution_minutes > 0);
  end if;
  if not exists (
    select 1 from pg_constraint where conname = 'aso_settings_sla_warning_minutes_check'
  ) then
    alter table public.aso_settings
      add constraint aso_settings_sla_warning_minutes_check
      check (sla_warning_minutes_before_due is null or sla_warning_minutes_before_due > 0);
  end if;
  if not exists (
    select 1 from pg_constraint where conname = 'aso_settings_sla_at_risk_minutes_check'
  ) then
    alter table public.aso_settings
      add constraint aso_settings_sla_at_risk_minutes_check
      check (sla_at_risk_minutes_before_due is null or sla_at_risk_minutes_before_due > 0);
  end if;
end $$;
...
create or replace function public._aso_sla_policy_ready(p_settings public.aso_settings)
returns boolean
language sql
stable
as $$
  select coalesce(p_settings.sla_policy_enabled, false)
    and p_settings.sla_first_response_minutes is not null
    and p_settings.sla_resolution_minutes is not null
    and p_settings.sla_warning_minutes_before_due is not null
    and p_settings.sla_at_risk_minutes_before_due is not null
    and p_settings.sla_warning_minutes_before_due >= p_settings.sla_at_risk_minutes_before_due;
$$;

create or replace function public._aso_sla_status_rank(p_status text)
returns integer
language sql
immutable
as $$
  select case lower(coalesce(p_status, 'unavailable'))
...
create or replace function public._aso_compute_deadline_sla_status(
  p_now timestamptz,
  p_due_at timestamptz,
  p_completed_at timestamptz,
  p_warning_minutes integer,
  p_at_risk_minutes integer
)
returns text
language plpgsql
stable
...
begin
  ...
  if p_completed_at is not null then
    if p_completed_at <= p_due_at then
      return 'on_track';
    end if;
    return 'breached';
  end if;
  ...
  if p_now >= p_due_at - make_interval(mins => p_at_risk_minutes) then
    return 'at_risk';
  end if;

  if p_now >= p_due_at - make_interval(mins => p_warning_minutes) then
    return 'warning';
  end if;
...
create or replace function public._aso_compute_case_sla_status(
  p_now timestamptz,
  p_case public.support_cases,
  p_settings public.aso_settings
)
returns text
language plpgsql
stable
...
begin
  if not public._aso_sla_policy_ready(p_settings) then
    return 'unavailable';
  end if;

  v_first_due := coalesce(
    p_case.first_response_due_at,
    p_case.created_at + make_interval(mins => p_settings.sla_first_response_minutes)
  );
  v_resolution_due := coalesce(
    p_case.resolution_due_at,
    p_case.created_at + make_interval(mins => p_settings.sla_resolution_minutes)
  );

  v_first_status := public._aso_compute_deadline_sla_status(
    p_now,
    v_first_due,
    p_case.first_responded_at,
    p_settings.sla_warning_minutes_before_due,
    p_settings.sla_at_risk_minutes_before_due
  );

  v_resolution_status := public._aso_compute_deadline_sla_status(
    p_now,
    v_resolution_due,
    p_case.resolved_at,
    p_settings.sla_warning_minutes_before_due,
  ...
  if public._aso_sla_status_rank(v_first_status) >= public._aso_sla_status_rank(v_resolution_status) then
    return v_first_status;
  end if;

  return v_resolution_status;
end;
$$;

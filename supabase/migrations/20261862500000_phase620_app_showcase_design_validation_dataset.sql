-- Phase 620 — APP design validation showcase dataset (internal tooling only).
-- Dataset key: phase620_app_showcase_v1

create table if not exists public.app_showcase_data_registry (
  ...
  cleanup_priority integer not null default 50,
  notes text,
  unique (dataset_key, organization_id, table_name, record_id)
);

create index if not exists app_showcase_data_registry_org_dataset_idx
  on public.app_showcase_data_registry (organization_id, dataset_key, cleanup_priority, created_at desc);

create index if not exists app_showcase_data_registry_notes_idx
  on public.app_showcase_data_registry (organization_id, dataset_key, notes)
  where notes is not null;

alter table public.app_showcase_data_registry enable row level security;
revoke all on public.app_showcase_data_registry from authenticated, anon;

create table if not exists public.app_showcase_integrity_snapshots (
  organization_id uuid not null references public.organizations (id) on delete cascade,
  dataset_key text not null,
  snapshot jsonb not null default '{}'::jsonb,
  captured_at timestamptz not null default now(),
  primary key (organization_id, dataset_key)
);

alter table public.app_showcase_integrity_snapshots enable row level security;
revoke all on public.app_showcase_integrity_snapshots from authenticated, anon;

create or replace function public._ps620_registered(
...
as $$
  select exists (
    ...
    where r.organization_id = p_organization_id
      and r.dataset_key = p_dataset_key
      and r.notes = p_showcase_key
  );
$$;

create or replace function public._ps620_register(
  p_dataset_key text,
  p_organization_id uuid,
  p_table_name text,
  p_record_id uuid,
...
as $$
begin
  insert into public.app_showcase_data_registry (
    dataset_key, organization_id, table_name, record_id, record_type, cleanup_priority, notes
  ) values (
    p_dataset_key, p_organization_id, p_table_name, p_record_id, p_record_type, p_cleanup_priority, p_showcase_key
  )
  on conflict (dataset_key, organization_id, table_name, record_id) do nothing;
end;
$$;

create or replace function public._ps620_capture_integrity(
  p_organization_id uuid,
  p_dataset_key text
)
returns jsonb
...
begin
  select jsonb_build_object(
    ...
    'organization_role_permissions_count', (
      select count(*)::int
      from public.organization_role_permissions rp
      where rp.organization_id = p_organization_id
    ),
    'organization_user_permissions_count', (
      select count(*)::int
      from public.organization_user_permissions oup
      where oup.organization_id = p_organization_id
    ),
    'license_status', (
      select als.app_license_status
      from public.organization_app_license_state als
      where als.organization_id = p_organization_id
      limit 1
    )
  ) into v_snapshot;

  insert into public.app_showcase_integrity_snapshots (organization_id, dataset_key, snapshot, captured_at)
  values (p_organization_id, p_dataset_key, v_snapshot, now())
  on conflict (organization_id, dataset_key) do update
...
create or replace function public._ps620_seed_employees(
  p_org_id uuid,
  p_company_id uuid,
  p_dataset_key text,
  p_dense boolean
...
declare
  v_id uuid;
  v_count int := 0;
  v_suffix text := case when p_dense then ' — extended design-validation narrative for wrapping, badge width, and mobile layout stress across Nordic and international character sets Æøå Österreich 日本語' else '' end;
  rec record;
begin
  for rec in
    select *
    from (
      values
        ('ps620:emp:owner', 'Eira Nordahl', 'eira.nordahl@example.aipify-showcase.invalid', 'Organization Owner', 'owner', 'active'),
        ('ps620:emp:ops', 'Jonas Bergström', 'jonas.bergstrom@example.aipify-showcase.invalid', 'Operations Manager', 'manager', 'active'),
        ('ps620:emp:cs', 'Sofia Müller', 'sofia.muller@example.aipify-showcase.invalid', 'Customer Success Lead', 'manager', 'active'),
        ('ps620:emp:finance', 'Henrik Olsen', 'henrik.olsen@example.aipify-showcase.invalid', 'Finance Manager', 'manager', 'active'),
        ('ps620:emp:support', 'Maja Kowalska', 'maja.kowalska@example.aipify-showcase.invalid', 'Support Specialist', 'employee', 'active'),
        ('ps620:emp:compliance', 'Linnea Åström', 'linnea.astrom@example.aipify-showcase.invalid', 'Compliance Coordinator', 'employee', 'active'),
        ('ps620:emp:sales', 'Ahmed Hassan', 'ahmed.hassan@example.aipify-showcase.invalid', 'Sales Manager', 'manager', 'active'),
        ('ps620:emp:service', 'Emma van Dijk', 'emma.vandijk@example.aipify-showcase.invalid', 'Service Coordinator', 'employee', 'active'),
        ('ps620:emp:parttime', 'Noah Sørensen', 'noah.sorensen@example.aipify-showcase.invalid', 'Part-time Operations Assistant', 'employee', 'active'),
        ('ps620:emp:inactive', 'Former Employee Demo', 'former.employee@example.aipify-showcase.invalid', 'Former Analyst', 'employee', 'offboarded')
    ) as t(showcase_key, full_name, email, job_title, org_role, employee_status)
  loop
    if public._ps620_registered(p_org_id, p_dataset_key, rec.showcase_key) then
      continue;
    end if;

    insert into public.organization_employee_profiles (
      organization_id, full_name, email, job_title, org_role, employee_status,
      metadata
    ) values (
      p_org_id,
      rec.full_name || v_suffix,
      rec.email,
      rec.job_title,
      rec.org_role,
      rec.employee_status,
    ...
    perform public._ps620_register(
      p_dataset_key, p_org_id, 'organization_employee_profiles', v_id, 'employee_profile', 60, rec.showcase_key
    );
    v_count := v_count + 1;
  end loop;

  if not public._ps620_registered(p_org_id, p_dataset_key, 'ps620:invite:1') then
    insert into public.organization_employee_invitations (
      organization_id, email, full_name, org_role, status, welcome_message
    ) values (
      p_org_id,
      'pending.invite.one@example.aipify-showcase.invalid',
      'Pending Invite — Analytics Lead',
      'employee',
      'pending',
      'Synthetic invitation for design validation only.'
    ) returning id into v_id;
    perform public._ps620_register(
      p_dataset_key, p_org_id, 'organization_employee_invitations', v_id, 'invitation', 55, 'ps620:invite:1'
    );
    v_count := v_count + 1;
  end if;

  if not public._ps620_registered(p_org_id, p_dataset_key, 'ps620:invite:2') then
    insert into public.organization_employee_invitations (
      organization_id, email, full_name, org_role, status, welcome_message
    ) values (
      p_org_id,
      'pending.invite.two@example.aipify-showcase.invalid',
      'Pending Invite — Weekend Support Coverage',
      'support',
      'pending',
      'Second synthetic invitation for filter and badge testing.'
    ) returning id into v_id;
    perform public._ps620_register(
      p_dataset_key, p_org_id, 'organization_employee_invitations', v_id, 'invitation', 55, 'ps620:invite:2'
    );
    v_count := v_count + 1;
  end if;
...
create or replace function public.remove_app_showcase_data(
  p_organization_id uuid,
  p_dataset_key text default 'phase620_app_showcase_v1',
...
begin
  ...
  if not p_dry_run and not p_confirm then
    raise exception 'Destructive remove requires p_confirm = true or p_dry_run = true';
  end if;

  for r in
    select table_name, count(*)::int as cnt
    from public.app_showcase_data_registry
    where organization_id = p_organization_id and dataset_key = p_dataset_key
    group by table_name
    order by table_name
  loop
    v_deleted := v_deleted || jsonb_build_object(r.table_name, r.cnt);
  ...
  for r in
    ...
    from public.app_showcase_data_registry
    where organization_id = p_organization_id and dataset_key = p_dataset_key
    order by cleanup_priority asc, created_at desc
  loop
    v_sql := format('delete from public.%I where id = $1', r.table_name);
    execute v_sql using r.record_id;
  end loop;

  delete from public.app_showcase_data_registry
  where organization_id = p_organization_id and dataset_key = p_dataset_key;

  delete from public.app_showcase_integrity_snapshots
  where organization_id = p_organization_id and dataset_key = p_dataset_key;
...
create or replace function public.verify_app_showcase_data(
  p_organization_id uuid,
  p_dataset_key text default 'phase620_app_showcase_v1'
)
returns jsonb
...
begin
  for r in
    ...
    order by table_name
  loop
    v_counts := v_counts || jsonb_build_object(r.table_name, r.cnt);
    v_modules := array_append(v_modules, r.table_name);
  end loop;
  ...
  from (
    select notes, count(*)
    from public.app_showcase_data_registry
    where organization_id = p_organization_id and dataset_key = p_dataset_key and notes is not null
    group by notes
    having count(*) > 1
  ) d;

  select count(*)::int into v_orphans
  from public.app_showcase_data_registry reg
  where reg.organization_id = p_organization_id
    and reg.dataset_key = p_dataset_key
    and not exists (
      select 1
      from public.organizations o
      where o.id = reg.organization_id
    );

  select snapshot into v_before
  from public.app_showcase_integrity_snapshots
  where organization_id = p_organization_id and dataset_key = p_dataset_key;
  ...
  return jsonb_build_object(
    'ok', v_orphans = 0 and v_dupes = 0,
    'dataset_key', p_dataset_key,
    'organization_id', p_organization_id,
    'modules_populated', to_jsonb(v_modules),
    'record_counts', v_counts,
    'duplicate_showcase_keys', v_dupes,
    'orphan_registry_rows', v_orphans,
    'records_missing_organization_id', 0,
    'records_without_registry_entries', v_missing_registry,
    'external_side_effects_triggered', 0,
    'subscription_changed', v_before is not null and (v_before->'organization_subscriptions') is distinct from (v_after->'organization_subscriptions'),
    'license_changed', v_before is not null and (v_before->>'license_status') is distinct from (v_after->>'license_status'),
    'permissions_changed', v_before is not null and (
      (v_before->>'organization_role_permissions_count') is distinct from (v_after->>'organization_role_permissions_count')
      or (v_before->>'organization_user_permissions_count') is distinct from (v_after->>'organization_user_permissions_count')
    ),
    'direct_user_grants_created', 0,
    'integrity_snapshot', v_before
  );
end;
$$;

revoke all on function public.seed_app_showcase_data(uuid, text, text) from public;
revoke all on function public.remove_app_showcase_data(uuid, text, boolean, boolean) from public;
revoke all on function public.verify_app_showcase_data(uuid, text) from public;

grant execute on function public.seed_app_showcase_data(uuid, text, text) to service_role;
grant execute on function public.remove_app_showcase_data(uuid, text, boolean, boolean) to service_role;
grant execute on function public.verify_app_showcase_data(uuid, text) to service_role;

grant execute on function public._ps620_registered(uuid, text, text) to service_role;
grant execute on function public._ps620_register(text, uuid, text, uuid, text, integer, text) to service_role;
grant execute on function public._ps620_capture_integrity(uuid, text) to service_role;
grant execute on function public._ps620_seed_employees(uuid, uuid, text, boolean) to service_role;

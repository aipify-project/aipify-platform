-- Platform portal Customer Detail V1 (read-only).
-- Authoritative customer orgs: organizations → customers → non-platform companies.
-- Excludes Platform infrastructure and UNKNOWN_STOP my-company-1.
...
revoke all on function public.get_platform_portal_customer_detail(uuid) from public, anon;
grant execute on function public.get_platform_portal_customer_detail(uuid) to authenticated;
